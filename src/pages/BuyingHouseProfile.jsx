...
export default function BuyingHouseProfile() {
  ...
  const partnerTotal =
    partnerNetwork?.total_connected ??
            ...
            <div className="absolute bottom-0 left-0 right-0 px-4 pb-4 sm:px-6 sm:pb-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
                  ...
                  <div className="pb-1">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {user?.verified ? (
                        <BadgeCheck className="text-sky-300" size={22} />
                      ) : null}
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {notice ? (
                    <p className="text-xs text-sky-600">{notice}</p>
                  ) : null}
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  <SoftCard>
                    ...
                    <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-200">
                      <strong>Review policy:</strong> Reviews can only be edited
                      or deleted by the person who wrote them. Profile owners
                      cannot delete reviews to maintain transparency and trust.
                    </div>
                  </SoftCard>
                  <SoftCard>
                    <SectionTitle
                    ...
                    {(ratingSummary?.recent_reviews || []).length > 0 ? (
                      <div className="space-y-3">
                        {(ratingSummary.recent_reviews || []).map((review) => {
                          const isAuthor =
                            String(review.from_user_id) === String(user?.id);
                          return (
                            <div
                              key={review.id}
