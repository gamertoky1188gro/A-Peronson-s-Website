...
export default function MvpDashboard() {
  ...
  const canManageOrg = useMemo(
    () => user && (user.role === "admin" || user.role === "buying_house"),
    [user],
  );
  ...
  async function completeOnboarding(e) {
    e.preventDefault();
    try {
      await api("/onboarding", "POST", token, {
        ...onboarding,
  ...
  async function createRequest(e) {
    e.preventDefault();
    try {
      await api("/requirements", "POST", token, {
        ...requestForm,
        certifications_required: requestForm.certifications_required
          .split(",")
          .map((x) => x.trim())
          .filter(Boolean),
      });
  ...
  async function loadFeed() {
    try {
      const data = await api(
        `/feed?unique=${unique}&type=${feedType}`,
        "GET",
        token,
      );
      setFeed(data);
  ...
  async function askAssistant(e) {
    e.preventDefault();
    try {
      const res = await api("/assistant/ask", "POST", token, {
      ...
      if (res.forward_to_agent) {
        ...
        setAssistantResponse(
          `${res.source} (${res.confidence}): ${res.matched_answer}`,
        );
      }
    } catch (err) {
      setFeedback(err.message);
  ...
  async function claimConversation() {
    try {
      const r = await api(
        `/conversations/${claimRequestId}/claim`,
        "POST",
        token,
      );
  ...
  async function grantConversation() {
    try {
      await api(`/conversations/${grantRequestId}/grant`, "POST", token, {
        ...
        <main className="grid two-col">
          <section className="card stack">
            ...
            <form onSubmit={completeOnboarding} className="stack">
              <input
                placeholder="1) Profile Image URL"
                value={onboarding.profile_image}
                onChange={(e) =>
                  setOnboarding({
                    ...onboarding,
              ...
              />
              <input
                placeholder="2) Organization Name"
              ...
              />
              <input
                placeholder="3) Category Selection (comma separated)"
                value={onboarding.categories}
                onChange={(e) =>
                  setOnboarding({ ...onboarding, categories: e.target.value })
                }
              />
              <button type="submit">Save Onboarding</button>
            </form>
            <p>
              Role: <strong>{user.role}</strong> | Verified badge:{" "}
              {user.verified ? "🔵 Blue Safe" : "Unverified"}
            </p>
            ...
            <p>
              ...
              {(verificationStatus?.missing_required || []).join(", ") ||
                "none"}
            </p>
            ...
            <section className="card stack">
              ...
              <form onSubmit={createRequest} className="stack">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRequestForm({ ...requestForm, material: e.target.value })
                  }
                  required
                />
                <input
                  placeholder="Timeline Days"
                  value={requestForm.timeline_days}
                  onChange={(e) =>
                    setRequestForm({
                      ...requestForm,
                ...
                <input
                  ...
                  value={requestForm.certifications_required}
                  onChange={(e) =>
                    setRequestForm({
                      ...requestForm,
                      certifications_required: e.target.value,
                    })
                  }
                />
                <input
                  placeholder="Shipping Terms"
                  value={requestForm.shipping_terms}
                  onChange={(e) =>
                    setRequestForm({
                      ...requestForm,
                      shipping_terms: e.target.value,
                    })
                  }
                  required
                />
                <textarea
                  ...
                  value={requestForm.custom_description}
                  onChange={(e) =>
                    setRequestForm({
                      ...requestForm,
            ...
            <section className="card stack">
              ...
              <form onSubmit={createProduct} className="stack">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProductForm({ ...productForm, moq: e.target.value })
                  }
                />
                <input
                ...
                />
                <input
                  placeholder="Video URL (for gallery/reels)"
          ...
          <section className="card full stack">
            ...
            <div className="top-actions">
              ...
              >
                ...
                <option value="requests">Buyer Requests</option>
                <option value="products">Company Products</option>
              </select>
            ...
            <ul>
              {(feed.items || []).map((item) => (
                <li key={item.id}>
                  {item.icon} <strong>{item.feed_type}</strong> |{" "}
                  {item.category} |{" "}
                  {item.custom_description || item.description || item.title}
                </li>
              ))}
            </ul>
          </section>
          ...
          <section className="card stack">
            ...
            <input
              placeholder="Request ID to claim"
              value={claimRequestId}
              onChange={(e) => setClaimRequestId(e.target.value)}
            />
            <button onClick={claimConversation}>
              Start Conversation / Claim & Respond
            </button>
            {canManageOrg ? (
              <>
                <input
                  placeholder="Request ID to grant"
                  value={grantRequestId}
                  onChange={(e) => setGrantRequestId(e.target.value)}
                />
                <input
          ...
          <section className="card stack">
            ...
            <form onSubmit={askAssistant} className="stack">
              <input
                placeholder="Ask about setup, verification, premium, help"
