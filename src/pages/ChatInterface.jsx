...
export default function ChatInterface() {
  ...
  const loadInbox = useCallback(async () => {
    ...
    try {
      ...
      const requests = normalizeThreads(
        data?.request_pool || [],
        currentUserId,
      );
      const allMatchIds = [
      ...
      const friendCounterpartyIds = [
        ...new Set(
          [...priority, ...requests]
            ...
            .map((thread) =>
              friendCounterpartyId(thread.matchId, currentUserId),
            )
            .filter(Boolean),
        ),
      ];
      ...
      const applyFriendDisplay = (threads) =>
        threads.map((thread) => {
          ...
          const counterpartyId = friendCounterpartyId(
            thread.matchId,
            currentUserId,
          );
      ...
      if (allMatchIds.length > 0) {
        ...
        const grouped = (callHistoryResponse?.items || []).reduce(
          (acc, item) => {
  ...
  const filteredPriorityInbox = useMemo(() => {
    ...
    return priorityInbox.filter(
      (thread) =>
        ...
        thread.last.toLowerCase().includes(search),
    );
  }, [priorityInbox, query]);
  ...
  useEffect(() => {
    ...
    if (summaryNote?.note) {
      const parsed = splitSuggestedReply(
        String(summaryNote.note).replace(/^AI Summary:\\s*/i, ""),
      );
      setAiSummary({
        text: parsed.text,
    ...
    const negotiationNote = extractLatestNote(
      leadSummary.notes,
      "AI Negotiation:",
    );
  ...
  function parsePrequal(notes = []) {
    ...
    const match =
      rows.find((n) =>
        String(n.note || "").startsWith("AI Pre-Qual Summary"),
      ) || null;
  ...
  const prequalNeedsInfo = Number.isFinite(prequal?.score)
    ? prequal.score < 0.6
    : false;
  const prequalCanOverride = Boolean(currentUser?.verified || isAdminUser);
  const prequalBlocked =
    prequalNeedsInfo && !isBuyerUser && !prequalOverride && prequalCanOverride;
  const prequalHardBlocked =
    prequalNeedsInfo && !isBuyerUser && !prequalCanOverride;
  ...
  const isLockOwner = Boolean(
    lockMeta &&
    lockMeta?.claimed_by === currentUser?.id &&
    !activeThread?.isFriendThread,
  );
  const canSendMessage =
    !isLockRestricted && !prequalBlocked && !prequalHardBlocked;
  ...
  async function startInstantCall(thread) {
    ...
    if (!token || !thread?.matchId) {
      setScheduleStatus(
        "Please sign in and select a valid thread before starting a call.",
      );
      return;
    }
  ...
  function openAttachmentPreview(attachment, absoluteUrlOverride = "") {
    ...
    setPreviewAttachment({
      url: absoluteUrlOverride
        ? absoluteUrlOverride
        : toAbsoluteAssetUrl(rawUrl),
      ...
      return (
        <div className="space-y-1">
          ...
          <button
            type="button"
            onClick={() =>
              openAttachmentPreview(message?.attachment, attachmentUrl)
            }
  ...
  async function requestNegotiationHelper() {
    ...
    try {
      ...
      setAiNegotiationError(
        err.message || "Unable to generate negotiation help",
      );
    } finally {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const retryAfter = Number(
        err?.details?.policy?.retry_after_seconds ||
            ...
            <div className="flex items-center gap-4">
              {callPromptThread.avatar ? (
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#2a2744] text-lg font-bold">
                  {getInitials(
                    formatDisplayName(
                      callPromptThread.name,
                      callPromptThread.senderId,
                    ),
                  )}
                </div>
              )}
