...
function Pill({ children, tone = "default" }) {
	...
	return (
		<span
			class={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${tones[tone] || tones.default}`}
		>
			{children}
		</span>
	);
				...
				<div class="rounded-3xl border border-slate-200/70 bg-white/80 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/70">
					<div class="flex flex-wrap items-center justify-between gap-4">
						...
						<Link
							...
							class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:text-sky-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200"
						>
							Open full search
					...
					<div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
						...
						<div class="rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/50">
							<div class="flex items-center gap-2 text-sky-600 dark:text-sky-400">
								<Globe2 class="h-4 w-4" />
								<span class="text-xs font-medium uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
									Top buyer regions
							...
							<div class="mt-3 text-sm font-semibold text-slate-900 dark:text-slate-100">
				...
				<div class="rounded-3xl border border-slate-200/70 bg-white/80 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/70">
					<div class="flex items-center justify-between gap-3">
						...
						<button
							...
							disabled={aiLoading}
							class="inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-slate-900"
						>
							<Sparkles size={14} />
					...
					{aiError ? (
						<div class="mt-3 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-700 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-200">
							{aiError}
						</div>
					) : null}
					{aiReply ? (
						<div class="mt-4 rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-100">
							<p class="whitespace-pre-wrap">{aiReply}</p>
							<div class="mt-3 flex items-center gap-3 text-xs text-slate-500">
								<button
									type="button"
									onClick={copyReply}
									class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-sky-300 hover:text-sky-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
								>
									Copy
								</button>
								{copyStatus ? <span class="text-sky-600">{copyStatus}</span> : null}
							</div>
						</div>
					) : null}
				</div>
				...
				<div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
					<div class="rounded-3xl border border-slate-200/70 bg-white/80 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/70">
						<div class="flex items-center gap-2 mb-3">
							<ShoppingCart class="h-4 w-4 text-sky-500" />
							<h3 class="text-sm font-semibold tracking-wide text-slate-900 dark:text-slate-100">
						...
						<div class="space-y-3">
							{(requests || []).slice(0, 6).map((req) => (
								<div
									key={req.id}
									class="rounded-2xl border border-slate-200/70 bg-white p-4 dark:border-slate-800 dark:bg-slate-950/60"
								>
							...
							{requests.length > 0 ? null : (
								<div class="text-sm text-slate-500 dark:text-slate-400">No requests yet.</div>
							)}
						</div>
					</div>
					...
					<div class="rounded-3xl border border-slate-200/70 bg-white/80 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/70">
						...
						<div class="space-y-3">
							...
							{products.length > 0 ? null : (
								<div class="text-sm text-slate-500 dark:text-slate-400">No products yet.</div>
							)}
						</div>
					</div>
				</div>
