        ...
        <header className={`mb-6 overflow-hidden rounded-[28px] border ${cardTheme} backdrop-blur-xl`}>
          <div className="flex flex-col gap-6 p-6 lg:flex-row lg:items-end lg:justify-between lg:p-8">
            <div className="max-w-3xl">
              ...
              <p className={`mt-4 max-w-2xl text-sm leading-6 sm:text-base ${darkMode ? "text-slate-300" : "text-slate-600"}`}>
              ...
              {canDedicatedManager && hasAccountManager ? (
                <div className={`mt-4 rounded-2xl border p-4 ${darkMode ? "border-amber-400/20 bg-amber-500/10" : "border-amber-200 bg-amber-50"}`}>
                  <div className="flex items-start gap-3">
                    <Icon className={`mt-0.5 h-5 w-5 ${darkMode ? "text-amber-300" : "text-amber-600"}`}>👑</Icon>
                    <div>
                      <p className="font-semibold">Dedicated Account Manager</p>
                      <p className={`mt-1 text-sm leading-6 ${darkMode ? "text-amber-100/80" : "text-amber-900/70"}`}>
                        {accountManager.account_manager_name || "Support manager"} — {accountManager.account_manager_email || ""} — {accountManager.account_manager_phone || ""}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                {darkMode ? <Icon className="h-4 w-4">☀️</Icon> : <Icon className="h-4 w-4">🌙</Icon>}
            ...
            <form onSubmit={submitReport} className="space-y-5">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <div>
                  ...
                  {!canPrioritySupport && (
                    <p className="mt-2 text-xs text-slate-500">Upgrade to Premium for priority support</p>
                  )}
                </div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block">
                  <span className={`mb-2 block text-sm font-medium ${darkMode ? "text-slate-200" : "text-slate-700"}`}>
                    Page URL <span className={darkMode ? "text-slate-500" : "text-slate-400"}>(optional)</span>
                  </span>
                  <div className="relative">
                ...
                <label className="block">
                  <span className={`mb-2 block text-sm font-medium ${darkMode ? "text-slate-200" : "text-slate-700"}`}>
                    Contact Email <span className={darkMode ? "text-slate-500" : "text-slate-400"}>(optional)</span>
                  </span>
                  <div className="relative">
              ...
              <div className={`rounded-[24px] border border-dashed p-5 ${darkMode ? "border-white/10 bg-slate-900/30" : "border-slate-200 bg-slate-50"}`}>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-3">
                    <Icon className={`mt-0.5 h-5 w-5 ${darkMode ? "text-sky-300" : "text-sky-600"}`}>📤</Icon>
                    <div>
                      <p className="font-medium">Screenshot / File <span className={darkMode ? "text-slate-500" : "text-slate-400"}>(optional)</span></p>
                      <p className={`mt-1 text-sm ${darkMode ? "text-slate-400" : "text-slate-600"}`}>
                  ...
                  <label className={`inline-flex cursor-pointer items-center gap-2 rounded-2xl border px-4 py-3 text-sm font-medium transition hover:scale-[1.01] active:scale-[0.99] ${darkMode ? 'border-white/10 bg-white/5 text-white hover:bg-white/10' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}>
                    <input
                      type="file"
                      className="hidden"
              ...
              >
                {loading ? <Icon className="h-4 w-4 animate-spin">⏳</Icon> : <Icon className="h-4 w-4">✓</Icon>}
                Submit Report
              </button>
            </form>
          </section>
          ...
          <aside className="space-y-6">
            <section className={`rounded-[28px] border ${cardTheme} p-5 backdrop-blur-xl sm:p-6`}>
              <div className="mb-5 flex items-center justify-between">
                ...
                >
                  <Icon className={`h-4 w-4 ${ticketsLoading ? "animate-spin" : ""}`}>↻</Icon>
                  Refresh
                </button>
              </div>
              ...
              {ticketsLoading ? (
                ...
                <div className={`rounded-[24px] border border-dashed p-8 text-center ${darkMode ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50"}`}>
                  <Icon className={`mx-auto h-10 w-10 ${darkMode ? "text-sky-300" : "text-sky-600"}`}>🎫</Icon>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${darkMode ? "bg-white/5 text-slate-300" : "bg-slate-100 text-slate-600"}`}>
                              {ticket.status || "Open"}
                            </span>
                          </div>
            ...
            <section className={`rounded-[28px] border ${cardTheme} p-5 backdrop-blur-xl sm:p-6`}>
              ...
              <div className="mt-5 space-y-4">
                {[
                  { icon: "📋", title: "Collected", text: "Your report is grouped with the right category and priority." },
                  { icon: "🐞", title: "Reviewed", text: "The issue is checked for clarity, impact, and reproducibility." },
                  { icon: "💬", title: "Responded", text: "A team member can follow up if more details are needed." },
                  { icon: "✅", title: "Resolved", text: "Completed items are tracked until the case is closed." },
                ].map((item, index) => (
                  <div key={item.title} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${darkMode ? "bg-white/5 text-sky-300" : "bg-sky-50 text-sky-600"}`}>
                        <span className="text-lg">{item.icon}</span>
                      </div>
                      {index < 3 && <div className={`mt-2 h-full w-px flex-1 ${darkMode ? "bg-white/10" : "bg-slate-200"}`} />}
                    </div>
                    <div className="pb-3">
            ...
            <section className={`rounded-[28px] border ${cardTheme} p-5 backdrop-blur-xl sm:p-6`}>
              <div className="flex items-start gap-3">
                <Icon className={`mt-0.5 h-5 w-5 ${darkMode ? "text-sky-300" : "text-sky-600"}`}>⚠️</Icon>
                <div>
