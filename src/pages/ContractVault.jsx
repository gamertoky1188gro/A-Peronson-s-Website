...
function canViewBankingReferences(user, contract) {
  ...
  return (
    uid &&
    (uid === String(contract.uploaded_by) ||
      uid === String(contract.buyer_id) ||
      uid === String(contract.factory_id))
  );
}
...
function computeFlow(contract) {
  ...
  const hasPdf = Boolean(
    contract?.artifact?.generated_at && contract?.artifact?.pdf_path,
  );
  const locked = contract?.artifact?.status === "locked";
  const archived =
    contract?.artifact?.status === "archived" || Boolean(contract?.archived_at);
  ...
  if (buyerSigned && factorySigned && !hasPdf)
    blockers.push("PDF generation pending");
...
function canBuyerSign(user, contract) {
  ...
  return (
    isOwnerLevel(user) ||
    (user.role === "buyer" && String(user.id) === String(contract.buyer_id))
  );
}
...
function canFactorySign(user, contract) {
  ...
  return (
    isOwnerLevel(user) ||
    (user.role === "factory" && String(user.id) === String(contract.factory_id))
  );
}
...
function actionBlockers(user, contract) {
  ...
  const factorySigned = contract?.factory_signature_state === "signed";
  const hasPdf = Boolean(
    contract?.artifact?.generated_at &&
    contract?.artifact?.pdf_hash &&
  ...
  if (!canBuyerSign(user, contract))
    blockers.buyerSign = "Only owner/admin or the assigned buyer can sign.";
  else if (buyerSigned) blockers.buyerSign = "Already signed.";

  if (!canFactorySign(user, contract))
    blockers.factorySign = "Only owner/admin or the assigned factory can sign.";
  else if (factorySigned) blockers.factorySign = "Already signed.";

  if (!canFinalizeArtifact(user, contract))
    blockers.lock = "Only owner/admin or the draft uploader can lock the PDF.";
  else if (!buyerSigned || !factorySigned)
    blockers.lock = "Both signatures are required first.";
  else if (!hasPdf)
    blockers.lock =
      "PDF is not generated yet (generated automatically after both signatures).";
  else if (locked) blockers.lock = "Already locked.";

  if (!canArchive(user, contract))
    blockers.archive = "Only owner/admin or the draft uploader can archive.";
...
function StepPill({ done, label }) {
  return (
    <div
      className={`flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ring-1${done ? "bg-[#E8F3FF] text-[#0A66C2] ring-[#BBD8FF]" : "bg-white text-slate-600 ring-slate-200"}`}
    >
      <span
        className={`inline-flex h-5 w-5 items-center justify-center rounded-full${done ? "bg-[#0A66C2] text-white" : "bg-slate-100 text-slate-500"}`}
      >
        {done ? "\u2713" : "\u2022"}
      </span>
      <span>{label}</span>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`inline-flex shrink-0 items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1${statusClass(status)}`}
            >
              {toLabel(status)}
            </span>
          </div>
          <div className="mt-1 truncate text-xs text-slate-600">
            {safeDash(contract.title)}
          </div>
        ...
        <div className="shrink-0 text-right text-xs text-slate-600">
          <div>
            {(contract.updated_at || contract.created_at || "").slice(0, 10) ||
              "\u2014"}
          </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-slate-600">
        <span className="inline-flex items-center rounded-full bg-white px-2 py-1 ring-1 ring-slate-200">
          Buyer: {contract.buyer_signature_state || "pending"}
        </span>
        <span className="inline-flex items-center rounded-full bg-white px-2 py-1 ring-1 ring-slate-200">
          Factory: {contract.factory_signature_state || "pending"}
        </span>
        <span className="inline-flex items-center rounded-full bg-white px-2 py-1 ring-1 ring-slate-200">
          PDF: {flow.hasPdf ? "ready" : "pending"}
        </span>
      </div>
    </button>
  );
...
export default function ContractVault() {
    ...
    try {
      ...
      setError(
        isForbidden
          ...
          : err.message || "Failed to load contracts.",
      );
      setContracts([]);
    } finally {
      setLoadingContracts(false);
  ...
  const loadPaymentProofs = async (contractId) => {
    ...
    try {
      const data = await apiRequest(
        `/payment-proofs?contract_id=${encodeURIComponent(contractId)}`,
        { token },
      );
  ...
  const loadAuditTrail = async (contractId) => {
    ...
    try {
      const data = await apiRequest(
        `/documents/contracts/${encodeURIComponent(contractId)}/audit`,
        { token },
      );
  ...
  const submitPaymentProof = async () => {
    ...
    try {
      ...
      if (paymentForm.type === "lc") {
        ...
        payload.usance_days =
          paymentForm.lc_type === "usance" ? usanceDays : undefined;
      } else {
      ...
      const created = await apiRequest("/payment-proofs", {
        method: "POST",
        token,
        body: payload,
      });
      ...
      resetPaymentForm();
      trackClientEvent("payment_proof_submitted", {
        entityType: "contract",
  ...
  const updatePaymentStatus = async (proofId, status) => {
    ...
    try {
      const updated = await apiRequest(
        `/payment-proofs/${encodeURIComponent(proofId)}`,
        {
          method: "PATCH",
          token,
          body: { status },
      ...
      trackClientEvent("payment_proof_status_updated", {
  ...
  const visibleContracts = useMemo(() => {
    ...
    return contracts
      .filter((c) =>
        statusFilter === "all"
          ? true
          : (c.lifecycle_status || "pending_signature") === statusFilter,
      )
      .filter((c) => {
        if (!q) return true;
        const haystack =
          `${c.contract_number || ""} ${c.title || ""} ${c.buyer_name || ""} ${c.factory_name || ""} ${c.id || ""}`.toLowerCase();
  ...
  useEffect(() => {
    ...
    apiRequest(`/calls/by-contract/${encodeURIComponent(selected.id)}`, {
      token,
    })
      .then((data) =>
        setCallItems(Array.isArray(data?.items) ? data.items : []),
      )
      .catch(() => setCallItems([]))
  ...
  const upsertContract = (nextContract) => {
    ...
    setContracts((prev) => {
      const idx = prev.findIndex(
        (entry) => String(entry.id) === String(nextContract.id),
      );
  ...
  const handleCreateDraft = async () => {
    await runStepAction(
      async (token) =>
        apiRequest("/documents/contracts/draft", {
          method: "POST",
          token,
          body: {
  ...
  const detailPanel = selected ? (
    <div className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <div className="truncate text-lg font-bold text-slate-900">
              {selected.contract_number || selected.id}
            </div>
            <span
              className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ring-1${statusClass(selected.lifecycle_status || "pending_signature")}`}
            >
              {toLabel(selected.lifecycle_status || "pending_signature")}
            </span>
          </div>
          <div className="mt-1 text-sm text-slate-600">
            {safeDash(selected.title)}
          </div>
          <div className="mt-3 text-sm text-slate-700">
            <span className="font-semibold">Buyer:</span>{" "}
            {safeDash(selected.buyer_name)} <span className="mx-1">-</span>
        ...
        <Link
          to="/help"
          className="shrink-0 rounded-full bg-[#E8F3FF] px-3 py-1 text-xs font-semibold text-[#0A66C2] hover:bg-[#D9ECFF]"
        >
          Help
        </Link>
      </div>
      ...
      {actionError ? (
        <div className="mt-4 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-rose-50 p-3 text-sm font-semibold text-rose-700">
          {actionError}
        </div>
      ) : null}
      <div className="mt-4">
      ...
      {selected.requirement_id ? (
        <div className="mt-2">
          <Link
            to={`/search?requirementId=${encodeURIComponent(selected.requirement_id)}`}
      ...
      {!hasRecordedCall ? (
        <div className="mt-4 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-amber-50 p-3 text-sm font-semibold text-amber-900">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span>
              Video calls are recommended before finalizing contracts. No
              recorded call is linked to this contract yet.
            </span>
            <Link
              to="/chat"
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        ...
        <StepPill
          done={selectedFlow.stepState.buyer_signature}
          label="Buyer sign"
        />
        <StepPill
          done={selectedFlow.stepState.factory_signature}
          label="Factory sign"
        />
        <StepPill
          done={selectedFlow.stepState.artifact_finalize}
          label="Lock PDF"
        />
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4">
          ...
          <div className="mt-2 text-sm text-slate-700">
            ...
            <span className="font-semibold">
              {selected.buyer_signature_state || "pending"}
            </span>{" "}
            {selected.buyer_signed_at ? (
          ...
          <div className="mt-1 text-sm text-slate-700">
            Factory:{" "}
            <span className="font-semibold">
              {selected.factory_signature_state || "pending"}
            </span>{" "}
            {selected.factory_signed_at ? (
              <span className="text-xs text-slate-500">
                ({selected.factory_signed_at})
              </span>
            ) : null}
          ...
          {!hasAcceptedProof ? (
            <div className="mt-3 rounded-lg shadow-borderless dark:shadow-borderlessDark bg-amber-50 px-3 py-2 text-[11px] font-semibold text-amber-900">
              Warning: No accepted payment proof yet. You may continue, but
              proof is strongly recommended for safety.
            </div>
          ) : null}
          <div className="mt-4 grid grid-cols-1 gap-2">
            <button
              ...
              onClick={() =>
                runStepActionWithProofWarning(
                  async (token) =>
                    apiRequest(
                      `/documents/contracts/${selected.id}/signatures`,
                      {
                        method: "PATCH",
                        token,
                        body: {
            ...
            {selectedActionBlockers.buyerSign ? (
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await apiRequest(
                    `/documents/contracts/${selected.id}/sign-session`,
                    { method: "POST", token },
                  );
                  if (res?.signing_url) {
                  ...
                  setActionError(
                    err.message || "Unable to create sign session",
                  );
                } finally {
                  setSaving(false);
                }
            ...
            {selectedActionBlockers.factorySign ? (
              <div className="text-xs text-amber-700">
                {selectedActionBlockers.factorySign}
              </div>
            ) : null}
        ...
        <div className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4">
          ...
          <div className="mt-2 text-sm text-slate-700">
            Status:{" "}
            <span className="font-semibold">
              {selected.artifact?.status || "draft"}
            </span>
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-2">
            <button
              ...
              onClick={() =>
                runStepActionWithProofWarning(
                  async (token) =>
                    apiRequest(`/documents/contracts/${selected.id}/artifact`, {
                      method: "PATCH",
                      token,
                      body: { status: "locked" },
                    }),
            ...
            {selectedActionBlockers.archive ? (
              <div className="text-xs text-amber-700">
                {selectedActionBlockers.archive}
              </div>
            ) : null}
            ...
            {canDownload ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-xl bg-[#0A66C2] px-4 py-2 text-center text-sm font-semibold text-white hover:bg-[#0959A8]"
              >
                Download PDF
              </a>
            ) : (
      ...
      <div className="mt-5 rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4 dark:bg-slate-900/50">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-sm font-semibold text-slate-900">
              Banking references (optional)
            </div>
            <div className="mt-1 text-xs text-slate-500">
              For fraud prevention only. No direct payments are processed
              on-platform.
            </div>
          </div>
          <div className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
            {canViewBankingReferences(currentUser, selected)
              ? "Visible"
              : "Masked"}
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 text-sm text-slate-700">
          <div>
            Bank name:{" "}
            {canViewBankingReferences(currentUser, selected)
              ? safeDash(selected.bank_name)
              : maskValue(selected.bank_name)}
          </div>
          <div>
            Beneficiary:{" "}
            {canViewBankingReferences(currentUser, selected)
              ? safeDash(selected.beneficiary_name)
              : maskValue(selected.beneficiary_name)}
          </div>
          <div>
            Transaction reference:{" "}
            {canViewBankingReferences(currentUser, selected)
              ? safeDash(selected.transaction_reference)
              : maskValue(selected.transaction_reference)}
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4 dark:bg-slate-900/50">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500">
              Submit bank transfer or LC documents. Seller review sets status,
              disputes trigger internal admin review.
            </div>
          </div>
          <button
            type="button"
            onClick={() => loadPaymentProofs(selected.id)}
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <select
            ...
            onChange={(e) =>
              setPaymentForm((prev) => ({ ...prev, type: e.target.value }))
            }
            className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
          >
          ...
          {paymentForm.type === "bank_transfer" ? (
            <>
              ...
              <input
                ...
                onChange={(e) =>
                  setPaymentForm((p) => ({ ...p, bank_name: e.target.value }))
                }
              />
              <input
                className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
              ...
              <input
                className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                placeholder="Receiver/company account name"
              ...
              <div className="flex gap-2">
                <input
                  className="flex-1 rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                  placeholder="Amount"
                ...
                />
                <input
                  className="w-24 rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
            ...
            <>
              <input
                ...
                value={paymentForm.lc_number}
                onChange={(e) =>
                  setPaymentForm((p) => ({ ...p, lc_number: e.target.value }))
                }
              />
              <div className="flex flex-wrap gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setPaymentForm((p) => ({ ...p, lc_type: e.target.value }))
                  }
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                >
                ...
                {paymentForm.lc_type === "usance" ? (
                  <>
                    ...
                    {String(paymentForm.usance_days) === "custom" ? (
                      <input
                        className="w-32 rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                        placeholder="Days"
                        value={paymentForm.usance_custom_days}
                        onChange={(e) =>
                          setPaymentForm((p) => ({
                            ...p,
                            usance_custom_days: e.target.value,
                          }))
                        }
                      />
                    ) : null}
              ...
              <input
                ...
                value={paymentForm.advising_bank}
                onChange={(e) =>
                  setPaymentForm((p) => ({
                    ...p,
                    advising_bank: e.target.value,
                  }))
                }
              />
              <input
                ...
                value={paymentForm.applicant_name}
                onChange={(e) =>
                  setPaymentForm((p) => ({
                    ...p,
              ...
              <input
                ...
                onChange={(e) =>
                  setPaymentForm((p) => ({ ...p, issue_date: e.target.value }))
                }
              />
              <input
                className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                ...
                onChange={(e) =>
                  setPaymentForm((p) => ({ ...p, expiry_date: e.target.value }))
                }
              />
              <div className="flex gap-2">
                <input
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          {paymentNotice ? (
            <span className="text-xs text-slate-500">{paymentNotice}</span>
          ) : null}
        </div>

        <div className="mt-4 space-y-2">
          {paymentLoading ? (
            <div className="text-xs text-slate-500">Loading proofs...</div>
          ) : null}
          {!paymentLoading && paymentProofs.length === 0 ? (
              ...
              >
                ...
                <div className="mt-2 grid grid-cols-1 gap-1">
                  {proof.transaction_reference ? (
                    <div>Ref: {proof.transaction_reference}</div>
                  ) : null}
                  {proof.lc_number ? <div>LC: {proof.lc_number}</div> : null}
                  {proof.lc_type ? (
                    <div>
                      LC Type: {String(proof.lc_type).toUpperCase()}
                      {proof.lc_type === "usance" && proof.usance_days
                        ? ` (${proof.usance_days} days)`
                        : ""}
                    </div>
                  ) : null}
                  {proof.amount ? (
                  ...
                  <div className="mt-2">
                    {proofDocUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[10px] font-semibold text-[#0A66C2] hover:underline"
                      >
                        Open proof document
                      </a>
                    ) : (
                    ...
                    {proof.type === "bank_transfer" ? (
                      <>
                        <button
                          type="button"
                          onClick={() =>
                            updatePaymentStatus(proof.id, "received")
                          }
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updatePaymentStatus(proof.id, "pending_check")
                          }
                          className="rounded-full bg-amber-600 px-3 py-1 text-[10px] font-semibold text-white"
                        >
                          Pending check
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            updatePaymentStatus(proof.id, "not_received")
                          }
                          className="rounded-full bg-rose-600 px-3 py-1 text-[10px] font-semibold text-white"
                        >
                          Not received
                        </button>
                      </>
                      ...
                      <>
                        <button
                          type="button"
                          onClick={() =>
                            updatePaymentStatus(proof.id, "accepted")
                          }
                          className="rounded-full bg-emerald-600 px-3 py-1 text-[10px] font-semibold text-white"
                        >
                          Accept
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updatePaymentStatus(proof.id, "pending_review")
                          }
                          className="rounded-full bg-amber-600 px-3 py-1 text-[10px] font-semibold text-white"
                        >
                          Pending review
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updatePaymentStatus(proof.id, "rejected")
                          }
                          className="rounded-full bg-rose-600 px-3 py-1 text-[10px] font-semibold text-white"
                        >
                          Reject
      ...
      <div className="mt-5 rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500 dark:text-slate-300">
              Recorded calls are stored for dispute resolution and security
              (project.md).
            </div>
          </div>
          <div className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-white/5 dark:text-slate-200">
            {callsLoading
              ? "Loading..."
              : `${callItems.filter((c) => c.recording_url).length} available`}
          </div>
        </div>
        ...
        <div className="mt-3 space-y-3">
          {callItems.map((call) => {
            ...
            const canPlay = Boolean(
              call.recording_status === "available" && url,
            );
            return (
              <div
                key={call.id}
                className="rounded-xl shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-3 dark:bg-black/20"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                      {safeDash(call.title) || "Call session"}
                    </div>
                    <div className="mt-1 text-xs text-slate-600 dark:text-slate-300">
                      {safeDash(call.status)} -{" "}
                      {call.created_at
                    ...
                    <video
                      ...
                      onPlay={async () => {
                        try {
                          ...
                          if (!token) return;
                          await apiRequest(
                            `/calls/${encodeURIComponent(call.id)}/recording/viewed`,
                            { method: "POST", token },
                          );
                        } catch {
                          // silent
      ...
      <div className="mt-5 rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 text-sm text-slate-700">
          ...
          <div className="break-all text-xs text-slate-600">
            Hash: {safeDash(selected.artifact?.pdf_hash)}
          </div>
          <div className="text-xs text-slate-600">
            Signer IDs: Buyer{" "}
            {safeDash(selected.artifact?.signer_ids?.buyer_id)}{" "}
            <span className="mx-1">-</span> Factory{" "}
            {safeDash(selected.artifact?.signer_ids?.factory_id)}
          </div>
          <div className="text-xs text-slate-600">
            Signature timestamps: Buyer{" "}
            {safeDash(selected.artifact?.signature_timestamps?.buyer_signed_at)}{" "}
            <span className="mx-1">-</span> Factory{" "}
            {safeDash(
              selected.artifact?.signature_timestamps?.factory_signed_at,
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4">
        ...
        {auditLoading ? (
          ...
          <div className="mt-3 rounded-lg shadow-borderless dark:shadow-borderlessDark bg-amber-50 p-3 text-xs text-amber-800">
            {auditError}
          </div>
        ) : auditLog.length ? (
          <div className="mt-3 space-y-2 text-xs text-slate-600">
            {auditLog.map((entry) => (
              <div
                key={entry.id || `${entry.timestamp}-${entry.note}`}
                className="rounded-lg shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-3"
              >
    ...
    <div className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-10 text-center text-sm text-slate-600">
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#020617] dark:text-slate-100 transition-colors duration-500 ease-in-out">
      <div className="mx-auto max-w-7xl p-4 sm:p-6">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-xs font-semibold text-gtBlue">Vault</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              to="/owner"
              className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-slate-800 ring-1 ring-slate-200/70 transition hover:bg-slate-50 active:scale-95 dark:bg-white/5 dark:text-slate-100 dark:ring-white/10 dark:hover:bg-white/8"
            >
              Dashboard
            </Link>
            <Link
            ...
            <button
              ...
              className="rounded-full bg-gtBlue px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-gtBlueHover active:scale-95 disabled:bg-slate-200 disabled:text-slate-500"
            >
              New draft
            </button>
          </div>
        ...
        {forbidden ? (
          <AccessDeniedState message={error || "Access denied."} />
        ) : null}
        {!forbidden && error ? (
          <div className="mb-4 rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-rose-50 p-4 text-sm font-semibold text-rose-700">
            {error}
          </div>
        ) : null}
        ...
        {!forbidden ? (
          <div className="secure-grid grid grid-cols-1 gap-6 lg:grid-cols-12">
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="h-7 w-20 rounded-full relative overflow-hidden bg-slate-200/80 dark:bg-white/5 after:content-[''] after:absolute after:inset-0 after:translate-x-[-140%] after:pointer-events-none after:opacity-70 dark:after:opacity-90 after:animate-skeleton after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.28)_45%,transparent_70%)] dark:after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.16)_45%,transparent_70%)]" />
                        </div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          <div className="h-6 w-24 rounded-full relative overflow-hidden bg-slate-200/80 dark:bg-white/5 after:content-[''] after:absolute after:inset-0 after:translate-x-[-140%] after:pointer-events-none after:opacity-70 dark:after:opacity-90 after:animate-skeleton after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.28)_45%,transparent_70%)] dark:after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.16)_45%,transparent_70%)]" />
                          <div className="h-6 w-24 rounded-full relative overflow-hidden bg-slate-200/80 dark:bg-white/5 after:content-[''] after:absolute after:inset-0 after:translate-x-[-140%] after:pointer-events-none after:opacity-70 dark:after:opacity-90 after:animate-skeleton after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.28)_45%,transparent_70%)] dark:after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.16)_45%,transparent_70%)]" />
                          <div className="h-6 w-20 rounded-full relative overflow-hidden bg-slate-200/80 dark:bg-white/5 after:content-[''] after:absolute after:inset-0 after:translate-x-[-140%] after:pointer-events-none after:opacity-70 dark:after:opacity-90 after:animate-skeleton after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.28)_45%,transparent_70%)] dark:after:bg-[linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.16)_45%,transparent_70%)]" />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="hidden lg:col-span-7 lg:block">{detailPanel}</div>
          </div>
        ) : null}
        ...
        {draftOpen ? (
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-10 w-[min(40rem,92vw)] -translate-x-1/2 rounded-3xl bg-white p-6 shadow-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-xs font-semibold text-[#0A66C2]">
                    New
                  </div>
                  ...
                  <div className="mt-1 text-xs text-slate-600">
                    Banking references are optional and should be used only for
                    fraud prevention.
                  </div>
                </div>
                <button
              ...
              <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <input
                  ...
                  placeholder="Title"
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm outline-none"
                />
                <input
                  value={draftForm.buyer_name}
                  onChange={(e) =>
                    setDraftForm((p) => ({ ...p, buyer_name: e.target.value }))
                  }
                  placeholder="Buyer name"
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm outline-none"
                />
                <input
                  value={draftForm.factory_name}
                  onChange={(e) =>
                    setDraftForm((p) => ({
                      ...p,
                ...
                />
                <input
                  value={draftForm.buyer_id}
                  onChange={(e) =>
                    setDraftForm((p) => ({ ...p, buyer_id: e.target.value }))
                  }
                  placeholder="Buyer user ID"
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm outline-none"
                />
                <input
                  value={draftForm.factory_id}
                  onChange={(e) =>
                    setDraftForm((p) => ({ ...p, factory_id: e.target.value }))
                  }
                  placeholder="Factory user ID"
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm outline-none"
                />
                <div className="hidden sm:block" />

                <input
                  value={draftForm.bank_name}
                  onChange={(e) =>
                    setDraftForm((p) => ({ ...p, bank_name: e.target.value }))
                  }
                  placeholder="Bank name (optional)"
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm outline-none"
                />
                <input
                  value={draftForm.beneficiary_name}
                  onChange={(e) =>
                    setDraftForm((p) => ({
                      ...p,
                      beneficiary_name: e.target.value,
                ...
                />
                <input
                  value={draftForm.transaction_reference}
                  onChange={(e) =>
                    setDraftForm((p) => ({
                      ...p,
                      transaction_reference: e.target.value,
                  ...
                  className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm outline-none sm:col-span-2"
                />
              </div>
