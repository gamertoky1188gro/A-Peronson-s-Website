	...
	return (
		<div class="flex min-h-[60vh] items-center justify-center px-4">
			<div class="mx-auto max-w-md text-center">
				<div class="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800">
					<SearchX class="h-10 w-10 text-slate-400" />
				</div>
				<h1 class="text-2xl font-semibold text-slate-900 dark:text-white">User not found</h1>
				<p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
					This profile doesn't exist or you may not have access.
				</p>
				<Link
					to="/"
					class="mt-6 inline-flex items-center gap-2 rounded-2xl bg-sky-600 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-sky-500/20 hover:bg-sky-500"
				>
			...
			<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
				<div class="mb-5 flex items-center justify-between gap-3">
					...
					<div class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
						<Shield class="h-3.5 w-3.5" /> Role:{" "}
						<span class="font-medium text-slate-700 dark:text-slate-200">{displayRole}</span>
					</div>
				</div>
					...
					<div class="w-full space-y-6">
									...
									<div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
										<div class="flex items-end gap-4">
											...
											<div class="pb-1 text-white">
												...
												{headline ? <p class="mt-1 text-sm text-slate-200/90">{headline}</p> : null}
												<div class="mt-3 flex flex-wrap gap-2">
													{badges.map((badge) => (
														<Pill key={badge.label} tone={badge.tone}>
															<badge.icon class="h-3.5 w-3.5" /> {badge.label}
														</Pill>
													))}
												</div>
											</div>
						...
						{bio ? (
							<SoftCard>
								...
								<p class="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">{bio}</p>
							</SoftCard>
						) : null}
						...
						{company || industry || country || email ? (
							<SoftCard>
								...
								<div class="mt-3 grid gap-3 sm:grid-cols-2">
									{company ? (
										<div class="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
											<Building2 class="h-4 w-4 shrink-0 text-sky-500" />
											<div>
												<div class="text-xs text-slate-500 dark:text-slate-400">Company</div>
									...
									{industry ? (
										<div class="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
											<Globe2 class="h-4 w-4 shrink-0 text-sky-500" />
											<div>
												<div class="text-xs text-slate-500 dark:text-slate-400">Industry</div>
												<div class="text-sm font-medium text-slate-900 dark:text-white">
													{industry}
												</div>
											</div>
									...
									{country ? (
										<div class="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
											<MapPin class="h-4 w-4 shrink-0 text-sky-500" />
											<div>
												<div class="text-xs text-slate-500 dark:text-slate-400">Country</div>
												<div class="text-sm font-medium text-slate-900 dark:text-white">
													{country}
												</div>
											</div>
									...
									{email ? (
										<div class="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
											<Mail class="h-4 w-4 shrink-0 text-sky-500" />
											<div>
							...
							<div class="mt-4 space-y-3">
								...
								{target.verified === undefined ? null : (
									<div class="flex items-center justify-between text-sm">
										...
										<span
											class={`inline-flex items-center gap-1 font-medium ${target.verified ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}
										>
											{target.verified ? <BadgeCheck class="h-3.5 w-3.5" /> : null}
											{target.verified ? "Verified" : "Unverified"}
										</span>
									</div>
								)}
								<div class="flex items-center justify-between text-sm">
									...
									<span class="font-medium text-slate-900 dark:text-white">{joinedYear}</span>
								</div>
							</div>
						</SoftCard>
					</aside>
