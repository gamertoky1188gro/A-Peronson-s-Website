...
function Pill({ children, tone = "default" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${tones[tone] || tones.default}`}>
      {children}
    </span>
  );
}
...
export default function ProfilePage() {
  ...
  if (role === "buying_house") return <Navigate to={`/buying-house/${id}`} replace />;

  const profile = target.profile || {};
  const displayName = target.name || "User";
  const displayRole = (role || "").replace(/_/g, " ");
  const avatarImage = target.avatar_url || profile.avatar_url || profile.profile_image || profile.avatar || "";
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-5 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
            <Shield className="h-3.5 w-3.5" /> Role: <span className="font-medium text-slate-700 dark:text-slate-200">{displayRole}</span>
          </div>
        </div>
          ...
          <div className="w-full space-y-6">
            ...
            >
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                    <div className="flex items-end gap-4">
                      ...
                      <div className="pb-1 text-white">
                        ...
                        {headline ? (
                          <p className="mt-1 text-sm text-slate-200/90">{headline}</p>
                        ) : null}
              ...
              <div className="grid gap-4 p-5 pt-12 sm:grid-cols-2 lg:grid-cols-4 lg:pt-14">
                <Metric label="Trust" value={
                  <span className="inline-flex items-center gap-2">
                    {target.verified ? <BadgeCheck className="h-4 w-4 text-emerald-500" /> : <Shield className="h-4 w-4 text-slate-400" />}
            ...
            {bio ? (
              <SoftCard>
                <h3 className="text-sm font-semibold tracking-wide text-slate-900 dark:text-slate-100">About</h3>
            ...
            {company || industry || country || email ? (
              <SoftCard>
                <h3 className="text-sm font-semibold tracking-wide text-slate-900 dark:text-slate-100">Details</h3>
                  ...
                  {company ? (
                    <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-slate-900 dark:text-white">{company}</div>
                      </div>
                    </div>
                  ) : null}
                  {industry ? (
                    <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-slate-900 dark:text-white">{industry}</div>
                      </div>
                    </div>
                  ) : null}
                  {country ? (
                    <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-900/70">
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-slate-900 dark:text-white">{country}</div>
                      </div>
                    </div>
                  ) : null}
                  {email ? (
          ...
          <aside className="space-y-6">
            <SoftCard>
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold tracking-wide text-slate-900 dark:text-slate-100">Account</h3>
              </div>
              <div className="mt-4 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Role</span>
                  <span className="font-medium capitalize text-slate-900 dark:text-white">{displayRole}</span>
                </div>
                {target.verified !== undefined ? (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-500 dark:text-slate-400">Status</span>
                    <span className={`inline-flex items-center gap-1 font-medium ${target.verified ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}>
