    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-[0_18px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/60 dark:bg-slate-900/70 dark:shadow-none dark:ring-white/10">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              Access pages based on your role (Buyer, Factory, Buying House,
              Admin).
            </p>
          </div>
          <BackButton
            onClick={handleBack}
            className="text-sm text-slate-600 hover:text-slate-900 dark:text-slate-200 dark:hover:text-white bg-transparent px-2 py-1 rounded-none"
          />
        </div>
        ...
        <form onSubmit={handleLogin} className="mt-6 space-y-4">
          ...
          {error ? (
            <p className="text-sm text-red-500 dark:text-rose-300">{error}</p>
          ) : null}

          {/* Primary CTA: uses brand blue color */}
          <button
            disabled={loading}
            className="w-full px-4 py-3 rounded-lg bg-(--gt-blue) hover:bg-(--gt-blue-hover) text-white disabled:opacity-70 transition"
          >
          ...
          <p className="mt-6 text-sm text-gray-600 dark:text-slate-300">
            New here*{" "}
            <Link className="text-(--gt-blue) hover:underline" to="/signup">
              Create account
            </Link>
          </p>
        </form>
