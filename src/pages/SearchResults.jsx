...
function createDefaultFilters(searchParams) {
  return {
    ...
    handlesMultipleFactories:
      searchParams.get("handlesMultipleFactories") === "true",
...
function parseRoleSeatsParam(value) {
  ...
  return raw
    .split(",")
    .map((part) => {
      const [roleRaw, seatsRaw] = String(part || "")
        .split(":")
        .map((s) => (s || "").trim());
...
function getFacetCount(counts = {}, label = "") {
  ...
  const matchKey = Object.keys(counts).find(
...
function buildQueryString({
  ...
  if (Array.isArray(category) ? category.length : category)
    params.set("category", toCsv(category));
  ...
  if (includePriority && filters.priorityOnly)
    params.set("priorityOnly", "true");

  // Advanced filters (premium only)
  if (includeAdvanced) {
    if (hasFilterValue(filters.fabricType))
      params.set("fabricType", toCsv(filters.fabricType));
    ...
    if (filters.sizeRange === "Custom" && filters.sizeRangeCustom)
      params.set("sizeRangeCustom", filters.sizeRangeCustom);
    if (hasFilterValue(filters.colorPantone))
      params.set("colorPantone", toCsv(filters.colorPantone));
    if (hasFilterValue(filters.customization))
      params.set("customization", toCsv(filters.customization));
    if (filters.sampleAvailable) params.set("sampleAvailable", "true");
    if (filters.sampleLeadTime)
      params.set("sampleLeadTime", filters.sampleLeadTime);
    if (hasFilterValue(filters.certifications))
      params.set("certifications", toCsv(filters.certifications));
    if (hasFilterValue(filters.incoterms))
      params.set("incoterms", toCsv(filters.incoterms));
    if (hasFilterValue(filters.paymentTerms))
      params.set("paymentTerms", toCsv(filters.paymentTerms));
    if (hasFilterValue(filters.documentReady))
      params.set("documentReady", toCsv(filters.documentReady));
    if (filters.auditScoreMin)
      params.set("auditScoreMin", filters.auditScoreMin);
    ...
    if (filters.permissionSection)
      params.set("permissionSection", filters.permissionSection);
    if (filters.permissionSectionEdit)
      params.set("permissionSectionEdit", "true");
    if (hasFilterValue(filters.languageSupport))
      params.set("languageSupport", toCsv(filters.languageSupport));
    if (filters.capacityMin) params.set("capacityMin", filters.capacityMin);
    if (hasFilterValue(filters.processes))
      params.set("processes", toCsv(filters.processes));
    if (filters.yearsInBusinessMin)
      params.set("yearsInBusinessMin", filters.yearsInBusinessMin);
    if (filters.responseTimeMax)
      params.set("responseTimeMax", filters.responseTimeMax);
    if (filters.teamSeatsMin) params.set("teamSeatsMin", filters.teamSeatsMin);
    if (
      filters.roleSeats &&
      Array.isArray(filters.roleSeats) &&
      filters.roleSeats.length
    ) {
      const rs = serializeRoleSeats(filters.roleSeats);
      if (rs) params.set("roleSeats", rs);
    }
    if (filters.handlesMultipleFactories)
      params.set("handlesMultipleFactories", "true");
    if (hasFilterValue(filters.exportPort))
      params.set("exportPort", toCsv(filters.exportPort));
        ...
        return (
          ...
          >
            ...
            {Number.isFinite(Number(count)) ? (
              <span
                className={`ml-1 text-[10px] ${selected ? "text-white/80" : "text-slate-400"}`}
              >
                ({count})
              </span>
            ) : null}
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {options.map((option) => {
        ...
        const selected =
          selectedValue === optValue || (!selectedValue && !optValue);
        return (
          <button
...
function RangeSlider({
  ...
  const format =
    typeof formatValue === "function" ? formatValue : (v) => `${v}${suffix}`;
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-[11px] text-slate-500">
        <span>
          {Number.isFinite(minValue) ? format(minValue) : format(min)}
        </span>
        <div className="h-px flex-1 bg-slate-200" />
        <span>
          {Number.isFinite(maxValue) ? format(maxValue) : format(max)}
        </span>
      </div>
      <div className="flex items-center gap-3">
        <input
          ...
          onChange={(event) =>
            onChange(String(event.target.value || ""), valueMax)
          }
          disabled={disabled}
          className="w-full"
        />
        <input
          ...
          onChange={(event) =>
            onChange(valueMin, String(event.target.value || ""))
          }
          disabled={disabled}
          className="w-full"
        />
...
export default function SearchResults() {
  ...
  const canAdvancedFilters = hasEntitlement(
    sessionUser,
    "advanced_search_filters",
  );
  const canEarlyAccess = hasEntitlement(
    sessionUser,
    "early_access_verified_factories",
  );
  const canPriorityAccessRequests = hasEntitlement(
    sessionUser,
    "buyer_request_priority_access",
  );
  const canPriorityAccessCompanies = hasEntitlement(
    sessionUser,
  ...
  const [category, setCategory] = useState(() =>
    parseCsvParam(searchParams.get("category")),
  );
  ...
  const [activePreset, setActivePreset] = useState(() =>
    normalizePresetKey(localStorage.getItem(PRESET_STORAGE_KEY)),
  );
  ...
  const hasAdvancedFiltersFromUrl = useMemo(
    () =>
      ADVANCED_FILTER_KEYS.some(
        (key) =>
          key !== "priorityOnly" && hasFilterValue(searchParams.get(key)),
      ),
    [searchParams],
  );
  ...
  useEffect(() => {
    ...
    if (!inDev) return;
    const validation = validateCoreFilterRenderKeys(
      renderedDefaultCoreFilterKeys,
    );
    if (!validation.isValid) {
      console.warn(
        "[SearchResults] Invalid default core filter configuration.",
        validation,
      );
    }
  }, [renderedDefaultCoreFilterKeys]);
  ...
  useEffect(() => {
    ...
    const loadEarlyVerified = async () => {
      ...
      try {
        ...
        setEarlyVerifiedFactories([]);
        setEarlyVerifiedError(
          err.message || "Unable to load early verified factories",
        );
      }
    };
    loadEarlyVerified();
  ...
  useEffect(() => {
    const storedPreset = normalizePresetKey(
    ...
    const rolePreset = normalizePresetKey(
  ...
  const moqRangeValues = useMemo(
    () => parseRangeValue(filters.moqRange),
    [filters.moqRange],
  );
  const priceRangeValues = useMemo(
    () => parseRangeValue(filters.priceRange),
    [filters.priceRange],
  );
  ...
  const totalResults =
    (Number(requestsTotal) || 0) + (Number(companiesTotal) || 0);
  ...
  const autoSaveAlert = useCallback(
    async (candidate) => {
      ...
      try {
        await apiRequest("/search/alerts", {
          ...
          token,
          body: {
            query: candidate.query || "saved-search",
            filters: {
              category: toCsv(candidate.category),
              ...candidate.filters,
              auto: true,
            },
          },
        ...
        if (err?.status === 429) {
          setAlertFeedback(
            "Daily auto-alert quota reached. Search still ran normally.",
          );
        } else if (err?.message) {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      const includePriorityRequests =
        Boolean(filters.priorityOnly) &&
        activeTab !== "companies" &&
        canPriorityAccessRequests;
      const includePriorityCompanies =
        Boolean(filters.priorityOnly) &&
        activeTab !== "requests" &&
        canPriorityAccessCompanies;
      ...
      const mergedFacets =
        activeTab === "requests"
          ...
          : activeTab === "companies"
            ? prodFacets
            : mergeFacetCounts(reqFacets, prodFacets);
      flushSync(() => {
        setFacets(mergedFacets || {});
      });

      const mergedCapabilities = reqRes?.capabilities ||
        prodRes?.capabilities || { filters: { advanced: false } };
      flushSync(() => {
      ...
      const candidate = hasActiveFilters
        ? { query: q, category, filters }
        : null;
      flushSync(() => {
      ...
      if (reqRes?.quota) {
        if (reqRes.quota.unlimited) {
          ...
          setQuotaMessage(
            `Search quota remaining today: ${reqRes.quota.remaining}`,
          );
        }
      }
      ...
      if (
        ...
        const fingerprint = hashString(
          JSON.stringify({ q, category, filters, tab: activeTab }),
        );
        recordLeadSource({
          type: "search",
          id: fingerprint,
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const mergedCapabilities =
          reqRes?.capabilities || prodRes?.capabilities;
  ...
  useEffect(() => {
    const activeAdvancedKeys = hasAdvancedAccess
      ...
      : [];
    const activeCoreKeys = DEFAULT_CORE_FILTER_KEYS.filter((key) =>
      key === "category" ? category.length > 0 : hasFilterValue(filters[key]),
    );
    const payload = {
      query: query.trim(),
      categories: category,
  ...
  useEffect(() => {
    const depth =
      supplierAdvancedOpen || productAdvancedOpen
        ? 3
        : productMoreOpen || supplierMoreOpen || advancedFiltersOpen
          ? 2
          : filtersOpen
            ? 1
            : 0;
  ...
  useEffect(
    () => () => {
      if (
        dirtyFilterSinceSearchRef.current &&
        !lastSearchMetadataRef.current.searched
      ) {
  ...
  const loadRecentViews = useCallback(async () => {
    try {
      const data = await apiRequest("/products/views/me?cursor=0&limit=5", {
        token,
      });
  ...
  useEffect(() => {
    const keys = [
      ...new Set(
        companies.map((c) => String(c.profile_key || "")).filter(Boolean),
      ),
    ];
    if (!keys.length) {
    ...
    apiRequest(
      `/ratings/search?profile_keys=${encodeURIComponent(keys.join(","))}`,
      { token },
    )
  ...
  function updateAdvancedFilter(key, value) {
    if (!hasAdvancedAccess) {
      setUpgradePrompt(
        "Advanced filters require a Premium plan. Upgrade to unlock these filters.",
      );
      return;
    }
  ...
  function addPantone(value) {
    ...
    if (!cleaned) return;
    updateAdvancedFilter("colorPantone", [
      ...new Set([...(filters.colorPantone || []), cleaned]),
    ]);
    setPantoneDraft("");
  }
  ...
  function addRoleSeat() {
    ...
    const existing = Array.isArray(filters.roleSeats)
      ? filters.roleSeats || []
      : [];
    const next = existing.filter(
      (e) => String(e?.role || "").toLowerCase() !== role.toLowerCase(),
    );
  ...
  function removePantone(value) {
    updateAdvancedFilter(
      "colorPantone",
      (filters.colorPantone || []).filter((entry) => entry !== value),
    );
  }
  ...
  async function saveAlert(presetLabel = "") {
    ...
    try {
      ...
      setAlertFeedback(
        `Search saved. Remaining alert quota today: ${result?.quota?.remaining ?? "-"}`,
      );
    } catch (err) {
  ...
  function listLocalPresets() {
    try {
      return PRESET_KEYS.map((k) => {
        ...
        if (!raw) return null;
        try {
          return { key: k, data: JSON.parse(raw) };
        } catch {
          return null;
        }
      }).filter(Boolean);
  ...
  function deleteLocalPreset(presetKey) {
    try {
      ...
      if (localStorage.getItem(PRESET_STORAGE_KEY) === presetKey)
  ...
  function presetFallback(presetKey) {
    ...
    return {
      query: "",
      category: [],
      filters: { orgType: "buying_house", verifiedOnly: true },
    };
  }
  ...
  function applyPreset(presetKey) {
    ...
    try {
      const raw = localStorage.getItem(
        `gt_search_preset_${normalizedPresetKey}`,
      );
      const preset = raw
        ? JSON.parse(raw)
        : presetFallback(normalizedPresetKey);
      ...
      if (preset?.filters) {
        setFilters((prev) => ({
          ...prev,
          ...preset.filters,
          fabricType: Array.isArray(preset.filters.fabricType)
            ? preset.filters.fabricType
            : parseCsvParam(preset.filters.fabricType),
          colorPantone: Array.isArray(preset.filters.colorPantone)
            ? preset.filters.colorPantone
            : parseCsvParam(preset.filters.colorPantone),
          customization: Array.isArray(preset.filters.customization)
            ? preset.filters.customization
            : parseCsvParam(preset.filters.customization),
          certifications: Array.isArray(preset.filters.certifications)
            ? preset.filters.certifications
            : parseCsvParam(preset.filters.certifications),
          incoterms: Array.isArray(preset.filters.incoterms)
            ? preset.filters.incoterms
            : parseCsvParam(preset.filters.incoterms),
          paymentTerms: Array.isArray(preset.filters.paymentTerms)
            ? preset.filters.paymentTerms
            : parseCsvParam(preset.filters.paymentTerms),
          documentReady: Array.isArray(preset.filters.documentReady)
            ? preset.filters.documentReady
            : parseCsvParam(preset.filters.documentReady),
          languageSupport: Array.isArray(preset.filters.languageSupport)
            ? preset.filters.languageSupport
            : parseCsvParam(preset.filters.languageSupport),
          processes: Array.isArray(preset.filters.processes)
            ? preset.filters.processes
            : parseCsvParam(preset.filters.processes),
          exportPort: Array.isArray(preset.filters.exportPort)
            ? preset.filters.exportPort
            : parseCsvParam(preset.filters.exportPort),
        }));
      }
      localStorage.setItem(PRESET_STORAGE_KEY, normalizedPresetKey);
      setActivePreset(normalizedPresetKey);
      setAlertFeedback(
        `Loaded ${normalizedPresetKey.replace("_", " ")} preset.`,
      );
    } catch {
  ...
  async function createServerPresetFromCurrent(name, shared = false) {
    ...
    try {
      const payload = {
        ...
        filters: { query, category, ...filters },
  ...
  async function createServerPresetFromLocal(presetKey) {
    try {
      ...
      const payload = {
        name: `${presetKey.replace("_", " ")} preset`,
        filters: {
          query: parsed.query || "",
  ...
  function applyServerPreset(preset) {
    try {
      ...
      setQuery(data.query || "");
      const presetCategory = Array.isArray(data.category)
        ? data.category
        : parseCsvParam(data.category);
      setCategory(presetCategory);
      if (data) {
        setFilters((prev) => ({
          ...prev,
          ...data,
          fabricType: Array.isArray(data.fabricType)
            ? data.fabricType
            : parseCsvParam(data.fabricType),
          colorPantone: Array.isArray(data.colorPantone)
            ? data.colorPantone
            : parseCsvParam(data.colorPantone),
          customization: Array.isArray(data.customization)
            ? data.customization
            : parseCsvParam(data.customization),
          certifications: Array.isArray(data.certifications)
            ? data.certifications
            : parseCsvParam(data.certifications),
          incoterms: Array.isArray(data.incoterms)
            ? data.incoterms
            : parseCsvParam(data.incoterms),
          paymentTerms: Array.isArray(data.paymentTerms)
            ? data.paymentTerms
            : parseCsvParam(data.paymentTerms),
          documentReady: Array.isArray(data.documentReady)
            ? data.documentReady
            : parseCsvParam(data.documentReady),
          languageSupport: Array.isArray(data.languageSupport)
            ? data.languageSupport
            : parseCsvParam(data.languageSupport),
          processes: Array.isArray(data.processes)
            ? data.processes
            : parseCsvParam(data.processes),
          exportPort: Array.isArray(data.exportPort)
            ? data.exportPort
            : parseCsvParam(data.exportPort),
        }));
      }
  ...
  async function updateServerPreset(presetId) {
    ...
    try {
      const name = window.prompt(
        "Rename preset (leave blank to keep current name)",
        "",
      );
      ...
      await apiRequest(`/presets/${encodeURIComponent(presetId)}`, {
        method: "PATCH",
        token,
        body,
      });
  ...
  function openChatNotice(name, leadSource, journeyContext = {}) {
    ...
    if (journeyContext?.productId)
      params.set("product_id", journeyContext.productId);
    if (journeyContext?.requirementId)
      params.set("requirement_id", journeyContext.requirementId);
    ...
    if (token) {
      apiRequest("/workflow/journeys", {
        ...
        .then((journey) => {
          ...
          return apiRequest(
            `/workflow/journeys/${encodeURIComponent(journey.id)}/transition`,
            {
              method: "POST",
              token,
              body: {
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (filters.industry)
      chips.push({
        ...
        onRemove: () => setFilters((prev) => ({ ...prev, industry: "" })),
      });
    if (filters.country)
      chips.push({
        key: "country",
    ...
    if (
      filters.incoterms &&
      Array.isArray(filters.incoterms) &&
      filters.incoterms.length
    )
      chips.push({
        key: "incoterms",
        label: `Incoterms: ${filters.incoterms.join(", ")}`,
        onRemove: () => setFilters((prev) => ({ ...prev, incoterms: [] })),
      });
    if (filters.auditDate)
      chips.push({
        key: "auditDate",
        label: `Last audit: ${filters.auditDate}`,
        onRemove: () => setFilters((prev) => ({ ...prev, auditDate: "" })),
      });
    ...
    if (filters.orgType)
      chips.push({
        key: "orgType",
        label: `Account: ${filters.orgType.replace("_", " ")}`,
        onRemove: () => setFilters((prev) => ({ ...prev, orgType: "" })),
      });
    ...
    if (filters.priceRange) {
      ...
      chips.push({
        key: "priceRange",
        label,
        onRemove: () => setFilters((prev) => ({ ...prev, priceRange: "" })),
      });
    }
    if (filters.auditScoreMin)
      chips.push({
        key: "auditScoreMin",
        label: `Audit score ≥ ${filters.auditScoreMin}`,
        onRemove: () => setFilters((prev) => ({ ...prev, auditScoreMin: "" })),
      });
    if (filters.hasPermissionMatrix)
      chips.push({
        key: "hasPermissionMatrix",
        label: "Role-based access",
        onRemove: () =>
          setFilters((prev) => ({ ...prev, hasPermissionMatrix: false })),
      });
    if (filters.permissionSection)
      chips.push({
        key: "permissionSection",
        label: `Permission: ${filters.permissionSection}${filters.permissionSectionEdit ? " (edit)" : ""}`,
        onRemove: () =>
          setFilters((prev) => ({
            ...prev,
            permissionSection: "",
            permissionSectionEdit: false,
          })),
      });
    if (
      filters.roleSeats &&
      ...
      (filters.roleSeats || []).forEach((entry) => {
        ...
        chips.push({
          key: `roleSeats-${entry.role}`,
          label,
          onRemove: () =>
            setFilters((prev) => ({
              ...prev,
              roleSeats: (prev.roleSeats || []).filter(
                (e) => e.role !== entry.role,
              ),
            })),
        });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#020617] dark:text-slate-100 transition-colors duration-500 ease-in-out">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="rounded-2xl bg-white/70 p-4 shadow-sm ring-1 ring-slate-200/60 backdrop-blur-md dark:bg-slate-950/40 dark:ring-white/10">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {categoryOptions.map((option) => (
              ...
              >
                {option}
                {Number.isFinite(
                  Number(getFacetCount(facetCounts.category, option)),
                ) ? (
                  <span
                    className={`ml-1 text-[10px] ${category.includes(option) ? "text-white/80" : "text-slate-400"}`}
                  >
          ...
          {estimateLoading ||
          estimateError ||
          estimateTotals.requests !== null ||
          estimateTotals.companies !== null ? (
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-3">
              <div className="rounded-2xl bg-[#ffffff] p-4 shadow-sm ring-1 ring-slate-200/70 dark:bg-slate-900/40 dark:ring-white/10">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Core filters are visible first
                </p>
                <div
                  className="mt-3 grid grid-cols-1 gap-2"
                  data-testid="default-core-filter-bar"
                  data-core-filter-count={renderedDefaultCoreFilterKeys.length}
                >
                  <select
                    value={filters.industry}
                    onChange={(e) =>
                      updateCoreFilter("industry", e.target.value)
                    }
                  ...
                  <div className="rounded-xl bg-white px-3 py-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <select
                        ...
                        onChange={(e) =>
                          updateCoreFilter("priceCurrency", e.target.value)
                        }
                      ...
                      <div className="flex-1">
                        <RangeSlider
                          ...
                          onChange={(min, max) =>
                            updateRangeFilter("priceRange", min, max)
                          }
                          suffix=""
                  ...
                  <div className="rounded-xl bg-white px-3 py-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                    ...
                    <div className="mt-2">
                      <ChipGroup
                        ...
                        onChange={(values) =>
                          updateCoreFilter("incoterms", values)
                        }
                        disabled={false}
                  ...
                  {productMoreOpen ? (
                    <>
                      <input
                        ...
                        onChange={(e) =>
                          updateCoreFilter("country", e.target.value)
                        }
                      ...
                      <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                        <input
                          ...
                          onChange={(e) =>
                            updateCoreFilter("verifiedOnly", e.target.checked)
                          }
                          className="h-4 w-4"
                        />
                        Verified only
              ...
              <div
                className={`rounded-2xl p-4 ring-1 shadow-sm${premiumLocked ? " bg-amber-50 ring-amber-200 dark:bg-amber-500/10 dark:ring-amber-500/30" : " bg-[#ffffff] ring-slate-200/70 dark:bg-slate-900/40 dark:ring-white/10"}`}
                data-has-advanced-url-filters={
                  hasAdvancedFiltersFromUrl ? "true" : "false"
                }
              >
                <div className="flex items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      Core visible first, attributes under More filters
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="mt-3 grid grid-cols-1 gap-2">
                  ...
                  {supplierMoreOpen ? (
                    <select
                      ...
                      onChange={(e) =>
                        updateCoreFilter("leadTimeMax", e.target.value)
                      }
                      className="rounded-xl bg-white px-3 py-2 text-sm text-slate-800 ring-1 ring-slate-200/70 focus:outline-none focus:ring-2 focus:ring-[rgba(10,102,194,0.35)] dark:bg-white/5 dark:text-slate-100 dark:ring-white/10"
                    >
                ...
                {advancedFiltersOpen ? (
                  <div className="mt-3 grid grid-cols-1 gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        filterMode === "product"
                          ? setProductAdvancedOpen((prev) => !prev)
                          : setSupplierAdvancedOpen((prev) => !prev)
                      }
                      className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10"
                    >
                      {(
                    ...
                    {filterMode === "product" ? (
                      productAdvancedOpen ? (
                        <>
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2">
                              <ChipGroup
                                ...
                                onChange={(values) =>
                                  updateAdvancedFilter("fabricType", values)
                                }
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateAdvancedFilter("sizeRange", e.target.value)
                            }
                            disabled={premiumLocked}
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2 flex gap-2">
                              <input
                                value={pantoneDraft}
                                onChange={(e) =>
                                  setPantoneDraft(e.target.value)
                                }
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2 space-y-2">
                              {CUSTOMIZATION_OPTIONS.map((opt) => {
                                const checked =
                                  Array.isArray(filters.customization) &&
                                  filters.customization.includes(opt);
                                return (
                                  <label
                                    key={opt}
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  updateAdvancedFilter("sampleLeadTime", "")
                                }
                                disabled={
                                  premiumLocked || !filters.sampleLeadTime
                                }
                            ...
                            <div className="mt-2 flex items-center gap-3">
                              <input
                                ...
                                value={
                                  Number.isFinite(
                                    Number(filters.sampleLeadTime),
                                  )
                                    ? Number(filters.sampleLeadTime)
                                    : SAMPLE_LEAD_TIME_MAX_DAYS
                                }
                                onChange={(e) =>
                                  updateAdvancedFilter(
                                    "sampleLeadTime",
                            ...
                            <div className="mt-1 text-[10px] text-slate-400">
                              {filters.sampleLeadTime
                                ? `Up to ${filters.sampleLeadTime} days`
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2">
                              <ChipGroup
                                ...
                                onChange={(values) =>
                                  updateAdvancedFilter("certifications", values)
                                }
                                disabled={premiumLocked}
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2">
                              <ChipGroup
                                ...
                                values={filters.incoterms}
                                onChange={(values) =>
                                  updateAdvancedFilter("incoterms", values)
                                }
                                disabled={premiumLocked}
                                counts={facetCounts.incoterms}
                              />
                        ...
                        <p className="text-[11px] text-slate-500">
                          Open advanced block to configure product attributes.
                        </p>
                      )
                    ) : supplierAdvancedOpen ? (
                      <>
                        <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                          ...
                          <div className="mt-2">
                            <ChipGroup
                              ...
                              onChange={(values) =>
                                updateAdvancedFilter("paymentTerms", values)
                              }
                              disabled={premiumLocked}
                        ...
                        <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                          ...
                          <div className="mt-2">
                            <ChipGroup
                              ...
                              onChange={(values) =>
                                updateAdvancedFilter("documentReady", values)
                              }
                              disabled={premiumLocked}
                        ...
                        <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                updateAdvancedFilter("auditScoreMin", "")
                              }
                          ...
                          <div className="mt-1 text-[10px] text-slate-400">
                            {filters.auditScoreMin
                              ? `Min score: ${filters.auditScoreMin}`
                              : "Any (move slider to set)"}
                          </div>
                        </div>
                        <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                          ...
                          <div className="mt-2">
                            <ChipGroup
                              ...
                              onChange={(values) =>
                                updateAdvancedFilter("languageSupport", values)
                              }
                              disabled={premiumLocked}
                        ...
                        <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                          ...
                          <div className="mt-2">
                            <ChipGroup
                              ...
                              onChange={(values) =>
                                updateAdvancedFilter("processes", values)
                              }
                              disabled={premiumLocked}
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2">
                              <BucketChips
                                ...
                                onChange={(value) =>
                                  updateAdvancedFilter("responseTimeMax", value)
                                }
                                disabled={premiumLocked}
                              />
                            </div>
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2">
                              <BucketChips
                                ...
                                onChange={(value) =>
                                  updateAdvancedFilter("teamSeatsMin", value)
                                }
                                disabled={premiumLocked}
                              />
                            </div>
                            ...
                            <div className="mt-2">
                              <div className="flex gap-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setRoleSeatDraftRole(e.target.value)
                                  }
                                  disabled={premiumLocked}
                                  className="flex-1 rounded-lg bg-white px-3 py-2 text-xs text-slate-800 ring-1 ring-slate-200/70 disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-[rgba(10,102,194,0.35)] dark:bg-white/5 dark:text-slate-100 dark:ring-white/10"
                                />
                                <input
                                  ...
                                  onChange={(e) =>
                                    setRoleSeatDraftSeats(e.target.value)
                                  }
                                  disabled={premiumLocked}
                                  className="w-24 rounded-lg bg-white px-3 py-2 text-xs text-slate-800 ring-1 ring-slate-200/70 disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-[rgba(10,102,194,0.35)] dark:bg-white/5 dark:text-slate-100 dark:ring-white/10"
                                />
                                  ...
                                  filters.roleSeats.map((entry) => (
                                    ...
                                    >
                                      <div className="text-[11px] text-slate-700">
                                        {entry.role}: {entry.seats || 0} seats
                                      </div>
                                      <div>
                                        <button
                                          type="button"
                          ...
                          <div className="rounded-xl bg-white p-3 text-xs text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/10">
                            ...
                            <div className="mt-2">
                              <ChipGroup
                                ...
                                onChange={(values) =>
                                  updateAdvancedFilter("exportPort", values)
                                }
                                disabled={premiumLocked}
                      ...
                      <p className="text-[11px] text-slate-500">
                        Open advanced block to configure supplier/account
                        attributes.
                      </p>
                    )}
                  </div>
                  ...
                  <p className="mt-3 text-[11px] text-slate-500 dark:text-slate-400">
                    Advanced filters are hidden to keep search simple. Use "More
                    filters" when needed.
                  </p>
                )}
              </div>
              ...
              <div className="rounded-2xl bg-[#ffffff] p-4 shadow-sm ring-1 ring-slate-200/70 dark:bg-slate-900/40 dark:ring-white/10">
                ...
                <div className="mt-2 space-y-2 text-[11px] text-slate-600 dark:text-slate-300">
                  <p>
                    Supplier filters use profile data such as main processes,
                    export ports, and years in business.
                  </p>
                  <p>
                    Distance radius uses coordinates. Use “Use my location” to
                    fill lat/lng quickly. If a supplier has no coordinates, we
                    fall back to country matching.
                  </p>
                  <p>
                    Premium filters are optional. Core filters always remain
                    free and unlimited.
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-[#ffffff] p-4 shadow-sm ring-1 ring-slate-200/70 dark:bg-slate-900/40 dark:ring-white/10">
                ...
                <div className="mt-2 space-y-2 text-[11px] text-slate-600 dark:text-slate-300">
                  {quotaMessage ? (
                    ...
                    <p>Run a search to see quota status.</p>
                  )}
                  {upgradePrompt ? (
                    <p className="text-amber-800 bg-amber-50 shadow-borderless dark:shadow-borderlessDark rounded-xl p-2">
                      {upgradePrompt}
                    </p>
                  ) : null}
                  {alertFeedback ? (
                    <p className="text-sky-800 bg-sky-50 shadow-borderless dark:shadow-borderlessDark rounded-xl p-2">
                      {alertFeedback}
                    </p>
                  ) : null}
                </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    <button
                      ...
                      className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-100 dark:ring-white/10"
                    >
                      Buyer
                    </button>
                    <button
                      type="button"
                      onClick={() => applyPreset("buying_house")}
                      className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-200/70 dark:bg-white/5 dark:text-slate-100 dark:ring-white/10"
                    >
                      Buying house
                    </button>
                ...
                <div className="mt-3">
                  <button
                    type="button"
                    onClick={() => setManagePresetsOpen((p) => !p)}
                    className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-200/70"
                  >
                    Manage presets
                  </button>
                  {managePresetsOpen ? (
                    <div className="mt-2 space-y-2">
                      {listLocalPresets().length ? (
                        listLocalPresets().map((p) => (
                          <div
                            key={p.key}
                            className="flex items-center justify-between gap-2 rounded-xl bg-white p-2 ring-1 ring-slate-200/70"
                          >
                            <div className="min-w-0 text-xs text-slate-700">
                              {p.key.replace("_", " ")} preset
                            </div>
                            <div className="flex gap-2">
                              <button
                                type="button"
                                onClick={() => applyPreset(p.key)}
                                className="rounded-full bg-gtBlue px-3 py-1 text-[11px] font-semibold text-white"
                              >
                                Load
                              </button>
                              <button
                                type="button"
                                onClick={() => shareLocalPreset(p)}
                                className="rounded-full px-3 py-1 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-200/70"
                              >
                                Share
                              </button>
                              <button
                                type="button"
                                onClick={() =>
                                  createServerPresetFromLocal(p.key)
                                }
                                className="rounded-full px-3 py-1 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-200/70"
                              >
                                Copy to server
                              ...
                              <button
                                type="button"
                                onClick={() => deleteLocalPreset(p.key)}
                                className="rounded-full px-3 py-1 text-[11px] font-semibold text-rose-600 ring-1 ring-rose-200/30"
                              >
                                Delete
                              </button>
                            </div>
                        ...
                        <div className="text-xs text-slate-500">
                          No local presets saved. Use the preset buttons above
                          to save one.
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <div className="mt-2 space-y-2">
                          {serverPresetsLoading ? (
                            <div className="text-xs text-slate-500">
                              Loading server presets...
                            </div>
                          ) : serverPresets.length ? (
                            serverPresets.map((sp) => (
                              <div
                                key={sp.id}
                                className="flex items-center justify-between gap-2 rounded-xl bg-white p-2 ring-1 ring-slate-200/70"
                              >
                            ...
                            <div className="text-xs text-slate-500">
                              No server presets. Save your current search to
                              create one.
                            </div>
                          )}
                ...
                {autoSaveCandidate ? (
                  <div className="mt-4 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-3 text-[11px] text-slate-600 dark:bg-white/5 dark:text-slate-200">
                    <p className="font-semibold text-slate-700 dark:text-slate-100">
                      Save this search as a preset
                    </p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => savePreset("buyer")}
                        className="rounded-full bg-gtBlue px-3 py-1 text-[11px] font-semibold text-white"
                      >
                        Buyer
                      </button>
                      <button
                        type="button"
                        onClick={() => savePreset("buying_house")}
                        className="rounded-full bg-gtBlue px-3 py-1 text-[11px] font-semibold text-white"
                      >
                        Buying house
                      </button>
                      ...
                      <button
                        ...
                        className="rounded-full px-3 py-1 text-[11px] font-semibold text-slate-600 ring-1 ring-slate-200/70 dark:text-slate-200 dark:ring-white/10"
                      >
                        Dismiss
                      </button>
                    </div>
                  ...
                  {activeTab === "all" || activeTab === "requests" ? (
                    <div className="space-y-3">
                      {requests.map((r, idx) => {
                        ...
                        const profileRoute = roleToProfileRoute(
                          author.role,
                          author.id,
                        );
                        const requestType = String(
                          r.request_type || "garments",
                        ).toLowerCase();
                        const specs =
                          r.specs && typeof r.specs === "object" ? r.specs : {};
                        const isCertified =
                          String(
                            author.order_certification_status || "",
                          ).toLowerCase() === "certified";
                        const quoteDeadline = r.quote_deadline
                          ? new Date(r.quote_deadline)
                          : null;
                        ...
                        const maxSuppliers = Number.isFinite(
                          Number(r.max_suppliers),
                        )
                          ? Number(r.max_suppliers)
                          : null;
                        const specLabel =
                          requestType === "textile"
                            ? [
                                specs.material_type || r.category,
                                specs.unit || "",
                              ]
                                .filter(Boolean)
                                .join(" - ")
                            : [specs.gender_target || "", specs.season || ""]
                                .filter(Boolean)
                                .join(" - ");
                        return (
                          <motion.div
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                ...
                                {specLabel || quoteDeadline || expiresAt ? (
                                  <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-slate-600 dark:text-slate-300">
                                    ...
                                    {quoteDeadline ? (
                                      <span className="rounded-full bg-sky-50 px-2 py-1 text-sky-700 dark:bg-sky-500/10 dark:text-sky-200">
                                        Quote by{" "}
                                        {quoteDeadline.toLocaleDateString()}
                                      </span>
                                    ) : null}
                                    {expiresAt ? (
                                      <span
                                        className={`rounded-full px-2 py-1${isExpired ? "bg-rose-50 text-rose-700 dark:bg-rose-500/15 dark:text-rose-200" : "bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200"}`}
                                      >
                                        {isExpired
                                          ? "Expired"
                              ...
                              <div className="flex flex-col gap-2 shrink-0">
                                <Link
                                  to={profileRoute}
                                  className="rounded-full px-3 py-2 text-xs font-semibold text-slate-700 ring-1 ring-slate-200/70 hover:bg-slate-50 active:scale-95 text-center dark:text-slate-100 dark:ring-white/10 dark:hover:bg-white/5"
                                >
                                  Open profile
                                </Link>
                                <button
                  ...
                  {activeTab === "all" || activeTab === "companies" ? (
                    <div className="space-y-3">
                      {companies.map((p, idx) => {
                        ...
                        const rating =
                          ratingsByProfileKey?.[p.profile_key] || null;
                        const isCertified =
                          String(
                            author.order_certification_status || "",
                          ).toLowerCase() === "certified";
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {p.boost_active ? (
                                    <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-1 text-[10px] font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-200">
                                      Boosted{" "}
                                      {p.boost_multiplier &&
                                      p.boost_multiplier !== 1
                                        ? `x${p.boost_multiplier}`
                                        : ""}
                                    </span>
                                  ) : null}
                                  {author.country ? (
                                  ...
                                  {author.role ? (
                                    <span className="text-[11px] text-slate-500 dark:text-slate-400 uppercase">
                                      -{" "}
                                      {String(author.role).replaceAll("_", " ")}
                                    </span>
                                  ) : null}
                                </div>
                                <p className="mt-2 text-sm text-slate-800 dark:text-slate-100 font-semibold">
                                  {p.title || "Product"}
                                </p>
          ...
          <aside className="col-span-12 xl:col-span-3 space-y-4">
            {isBuyer ? (
              <div className="rounded-2xl bg-[#ffffff] p-4 shadow-sm ring-1 ring-slate-200/60 dark:bg-slate-900/50 dark:ring-slate-800">
                <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                  Early verified factories
                </p>
                <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                  Premium-only early access list
                </p>
                {!canEarlyAccess ? (
                  <div className="mt-3 rounded-xl bg-amber-50 p-3 text-xs text-amber-800 ring-1 ring-amber-200/70 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-500/30">
                    Upgrade to Premium to unlock early access to newly verified
                    factories.
                    <div className="mt-2">
                      <Link
                        to="/pricing"
                        className="text-[11px] font-semibold text-gtBlue hover:underline"
                      >
                  ...
                  <div className="mt-3 space-y-2">
                    {earlyVerifiedFactories.length ? (
                      earlyVerifiedFactories.slice(0, 6).map((row) => (
                        <Link
                          key={row.id}
                ...
                {recentViews.length ? (
                  recentViews.map((row) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setQuickViewItem({ ...row.product, author: row.author })
                      }
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400">
                    No views yet. Use "Quick view" on a product.
                  </div>
                )}
              </div>
              <div className="mt-3">
                <Link
                  to="/notifications"
                  className="text-xs font-semibold text-gtBlue hover:underline"
                >
                  Open full history
                </Link>
              </div>
            ...
            {premiumLocked ? (
              <div className="rounded-2xl p-4 ring-1 ring-amber-200 bg-amber-50 dark:bg-amber-500/10 dark:ring-amber-500/30">
                ...
                <p className="mt-1 text-xs text-amber-800 dark:text-amber-200/90">
                  Upgrade to Premium to unlock advanced filters. Core filters
                  remain unlimited on the free plan.
                </p>
              </div>
            ) : null}
          </aside>
