...
import AccessDeniedState from "../components/AccessDeniedState.jsx";
		...
		<div class="flex flex-wrap gap-1.5">
			{permissions.length > 0 ? (
				permissions.map((perm) => (
					<span
						key={perm}
						class="rounded-full border border-sky-500/20 bg-sky-500/10 px-2.5 py-0.5 text-xs font-medium text-sky-700 dark:text-sky-200"
					>
						{perm}
					</span>
				))
...
function Modal({ title, children, onClose, footer }) {
	return (
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
			<div class="w-full max-w-4xl rounded-3xl border border-white/10 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950">
				<div class="flex items-start justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
					...
					<button
						type="button"
						onClick={onClose}
						class="rounded-full border border-slate-200 p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 dark:border-slate-700 dark:hover:bg-slate-800 dark:hover:text-white"
						aria-label="Close modal"
					>
						<X class="h-4 w-4" />
					</button>
				</div>
				<div data-lenis-prevent={true} class="max-h-[75vh] overflow-y-auto px-6 py-5">
					{children}
				</div>
				{footer ? (
					<div class="border-t border-slate-200 px-6 py-4 dark:border-slate-800">{footer}</div>
				) : null}
			</div>
		</div>
	);
...
function InfoCard({ title, value }) {
	return (
		<div class="rounded-3xl border border-slate-200 bg-slate-50 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/40">
			<div class="text-xs font-semibold uppercase tracking-[0.2em] text-sky-600 dark:text-sky-300">
				{title}
			</div>
			<div class="mt-2 text-sm font-medium text-slate-900 dark:text-white">{value}</div>
		</div>
	);
}
...
export default function MemberManagement() {
	...
	async function loadMembers() {
		...
		try {
			...
			setCreateForm((prev) => ({
				...prev,
				permission_matrix:
					Object.keys(prev.permission_matrix || {}).length > 0
	...
	const filtered = useMemo(() => {
		...
		return members.filter((m) =>
			[m.name, m.username, m.member_id, m.role, m.status].join(" ").toLowerCase().includes(q),
		);
	}, [members, search]);
	...
	async function handleResetPassword(memberId) {
		...
		setSuccess("");
		try {
			const data = await apiRequest(`${MEMBER_API_BASE}/${memberId}/reset-password`, {
				method: "POST",
				token,
			});
	...
	async function handleDeactivateOrRemove(memberId, remove = false) {
		...
		setSuccess("");
		try {
			await apiRequest(`${MEMBER_API_BASE}/${memberId}?remove=${remove ? "true" : "false"}`, {
				method: "DELETE",
				token,
			});
	...
	if (forbidden || !canManageMembers) {
		return (
			<div class="min-h-screen bg-gradient-to-br from-sky-50 via-white to-blue-50 text-slate-900 dark:from-slate-950 dark:via-slate-950 dark:to-sky-950/40 dark:text-white">
	...
	return (
		<div class="min-h-screen bg-gradient-to-br from-sky-50 via-white to-blue-50 text-slate-900 dark:from-slate-950 dark:via-slate-950 dark:to-sky-950/40 dark:text-white">
			<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
				<div class="rounded-[2rem] border border-white/60 bg-white/80 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
					<div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
						<div>
							<div class="mb-2 inline-flex rounded-full border border-sky-500/20 bg-sky-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-sky-700 dark:text-sky-200">
								/member-management
							</div>
							<h1 class="text-3xl font-black tracking-tight text-slate-900 dark:text-white sm:text-4xl">
								Member Management
							</h1>
							<p class="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300 sm:text-base">
						...
						<div class="flex flex-col gap-3 sm:flex-row">
							<div class="relative w-full sm:w-80">
								<input
									...
									class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 pl-11 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
								/>
								<Search class="pointer-events-none absolute inset-y-0 left-4 h-4 w-4 text-slate-400" />
							</div>
							<button
								type="button"
								onClick={() => {
								...
								class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 px-5 py-3 font-semibold text-white shadow-lg shadow-sky-500/25 transition hover:brightness-110"
							>
								+ Add New Member
							</button>
						</div>
					...
					<div class="mt-6 grid gap-3 md:grid-cols-3">
						...
						<InfoCard
							title="Team access"
							value={canTeamAccess ? "Premium permissions enabled" : "Premium permissions locked"}
						/>
						<InfoCard
							title="Members"
					...
					{canTeamAccess ? null : (
						<div class="mt-5 rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-100">
							Team/agent access management is a Premium feature on the{" "}
							<strong>{String(constraints.plan || "current").toUpperCase()}</strong> plan. Upgrade
							to edit permissions and access controls.
						</div>
					)}

					{error ? (
						<div class="mt-5 rounded-2xl border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm font-medium text-rose-800 dark:text-rose-100">
							{error}
						</div>
					) : null}
					{success ? (
						<div class="mt-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-800 dark:text-emerald-100">
							{success}
						</div>
					) : null}

					<ScrollReveal as="section">
						<div class="mt-6 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
									...
									<thead class="bg-slate-50 dark:bg-slate-900/70">
										<tr>
											{["Name", "Username", "Member ID", "Role", "Status", "Actions"].map((h) => (
												<th
													key={h}
													class="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"
												>
													{h}
												</th>
											))}
									...
									<tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-800 dark:bg-slate-950">
										{loading ? (
											...
											filtered.map((m) => (
												<tr
													key={m.id}
													class="transition hover:bg-sky-50/50 dark:hover:bg-slate-900/60"
												>
													<td class="px-6 py-5">
														<div>
													...
													<td class="px-6 py-5 text-sm text-slate-700 dark:text-slate-200">
														{m.username}
													</td>
													<td class="px-6 py-5 text-sm font-medium text-slate-700 dark:text-slate-200">
			...
			{showCreate && (
					...
					footer={
						<div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
							<button
								...
								class="rounded-2xl border border-slate-200 px-5 py-2.5 font-medium text-slate-700 transition hover:bg-slate-100 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900"
							>
								Close
							</button>
							<button
								...
								class="rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 px-5 py-2.5 font-semibold text-white shadow-lg shadow-sky-500/25 transition hover:brightness-110"
							>
								Create
							</button>
						</div>
				...
				>
					<form class="space-y-6" onSubmit={handleCreateMember}>
						<div class="grid gap-4 md:grid-cols-2">
							<label class="space-y-2">
								...
								<input
									...
									class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
							...
							<label class="space-y-2">
								...
								<div class="w-full rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm font-mono text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
						...
						<div class="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900/40">
							<p class="text-sm font-medium text-slate-700 dark:text-slate-200">
								Role: {constraints.default_role || "agent"} (fixed)
							</p>
							{canTeamAccess ? null : (
								<div class="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-900 dark:text-amber-100">
									Team/agent access management requires the{" "}
									<strong>{String(constraints.plan || "current").toUpperCase()}</strong> plan.
									Upgrade to edit permissions and access controls.
								</div>
							)}
						</div>
						...
						<div>
							<div class="mb-3 flex items-center justify-between">
								...
								<span class="text-sm text-slate-500 dark:text-slate-400">view/edit per module</span>
							</div>
						...
						{getConflictMessage(createForm.permissions) ? (
							<div class="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4 text-sm font-medium text-rose-800 dark:text-rose-100">
								{getConflictMessage(createForm.permissions)}
							</div>
						) : null}
					</form>
			...
			{Boolean(activePermissionMember) && (
				<Modal
...
function PermissionSelector({
	...
	return (
		<div class={disabled ? " opacity-60" : ""}>
			<div class="mb-3 text-base font-semibold text-slate-900 dark:text-white">Permissions</div>
			<div class="grid grid-cols-2 gap-3">
				{validPermissions.map((perm) => (
					<label
						key={perm}
						class="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 transition hover:border-sky-400 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200"
					>
						<input
							type="checkbox"
			...
			{Boolean(conflict) && (
				<div class="mt-3 rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4 text-sm font-medium text-rose-800 dark:text-rose-100">
					{conflict}
				</div>
			)}
		</div>
					...
					return (
						...
						>
							<div class="mb-3 flex items-center justify-between">
								...
								{isMembers ? (
									<span class="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-600 dark:bg-slate-800 dark:text-slate-300">
										Forced false
									</span>
								) : null}
							</div>
							<div class="grid grid-cols-2 gap-2">
								<label class="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 transition hover:border-sky-400 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200">
									<input
										type="checkbox"
										checked={isMembers ? false : Boolean(current.view)}
								...
								<label class="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 transition hover:border-sky-400 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200">
									<input
										type="checkbox"
										checked={isMembers ? false : Boolean(current.edit)}
										disabled={disabled || isMembers}
										onChange={(e) =>
											onChange({
												...matrix,
...
const MemberEditor = function MemberEditor({
	member,
	constraints,
	getConflictMessage,
	canTeamAccess,
	...
	return (
		<div class="space-y-6">
			<div class="grid gap-4 md:grid-cols-2">
				<label class="space-y-2">
					<span class="text-sm font-medium text-slate-700 dark:text-slate-200">Member name</span>
					<input
						value={form.name}
				...
				<label class="space-y-2">
					<span class="text-sm font-medium text-slate-700 dark:text-slate-200">Member ID</span>
					<input
				...
				<label class="space-y-2">
					...
					<select
						...
						class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
					>
			...
			<div>
				<div class="mb-3 flex items-center justify-between">
					<h4 class="text-base font-semibold text-slate-900 dark:text-white">Permission matrix</h4>
					<span class="text-sm text-slate-500 dark:text-slate-400">view/edit per module</span>
				</div>
				<PermissionMatrixEditor
