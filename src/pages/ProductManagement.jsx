...
export default function ProductManagement() {
  ...
  function isInternalMediaUrl(value) {
    ...
    return (
      ...
      normalized.startsWith("uploads/") ||
      normalized.includes("server/uploads/")
    );
  }
  ...
  async function syncProductMedia(nextForm) {
    ...
    try {
      ...
      setForm((prev) => ({
        ...prev,
        image_urls: Array.isArray(updated.image_urls)
          ? updated.image_urls
          : prev.image_urls,
      ...
      setMediaGallery(
        Array.isArray(updated.image_gallery) ? updated.image_gallery : [],
      );
      trackClientEvent("product_media_updated", {
        entityType: "product",
        entityId: updated.id,
      });
    } catch (err) {
  ...
  async function syncProductVideo(nextVideoUrl) {
    ...
    try {
      ...
      trackClientEvent("product_video_updated", {
        entityType: "product",
        entityId: updated.id,
      });
    } catch (err) {
  ...
  async function handleRemoveMedia(entry) {
    if (!entry) return;
    const nextUrls = (form.image_urls || []).filter(
      (url) => url !== entry.source_path,
    );
    const nextCover =
      form.cover_image_url === entry.source_path
        ? nextUrls[0] || ""
    ...
    setMediaGallery((prev) =>
      prev.filter((item) => item.source_path !== entry.source_path),
    );
    if (entry.document_id) {
      try {
        await apiRequest(
          `/documents/${encodeURIComponent(entry.document_id)}`,
          { method: "DELETE", token },
        );
      } catch {
    ...
    await syncProductMedia(nextForm);
    trackClientEvent("product_image_removed", {
      entityType: "product",
      entityId: editing?.id || "",
    });
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        ...
        {notice ? (
          <div className="bg-emerald-50 shadow-borderless dark:shadow-borderlessDark rounded-2xl p-4 text-sm text-emerald-800">
            {notice}
          </div>
        ) : null}
        {error ? (
          <div className="bg-rose-50 shadow-borderless dark:shadow-borderlessDark rounded-2xl p-4 text-sm text-rose-800">
            {error}
          </div>
        ) : null}
        ...
        <div className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-4 text-xs text-slate-600">
          ...
          <p className="mt-2">
            ...
            /uploads/... URLs are allowed. Pending or rejected media stays
            hidden from buyers.
          </p>
          <p className="mt-1">
            Use Draft to keep items private while preparing your gallery; switch
            ...
            {items.map((p) => (
              ...
              >
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  Video status:{" "}
                  {String(p.video_review_status || "approved").replaceAll(
                    "_",
                    " ",
                  )}
                </p>
                <p className="mt-1 text-[11px] text-slate-500">
                  Content review:{" "}
                  {String(p.content_review_status || "approved").replaceAll(
                    "_",
                    " ",
                  )}
                </p>
                ...
                <div className="mt-3 flex gap-2">
                  <button
                    onClick={() => openEdit(p)}
                    className="flex-1 rounded-full shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => remove(p.id)}
                    className="flex-1 rounded-full shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50"
                  >
                    Delete
                  </button>
                </div>
          ...
          <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-borderless dark:shadow-borderlessDark shadow-2xl overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <input
                ...
                value={form.moq}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, moq: e.target.value }))
                }
              />
              <input
                className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
              ...
              <input
                className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                placeholder="Video URL (internal /uploads/...)"
              ...
              />
              <textarea
                className="md:col-span-2 rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm min-h-[120px]"
              ...
              {advancedOpen ? (
                <>
                  <input
                    className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                    placeholder="Fabric GSM (optional)"
                  ...
                  <input
                    className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                    placeholder="Size range (optional)"
                  ...
                  <input
                    className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                    placeholder="Color / Pantone (optional)"
                    value={form.color_pantone}
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        color_pantone: e.target.value,
                      }))
                    }
                  />
                  <input
                    className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                    placeholder="Customization capabilities (optional)"
                    value={form.customization_capabilities}
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        customization_capabilities: e.target.value,
                      }))
                    }
                  />
                  <input
                    className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm"
                    placeholder="Sample available (yes/no)"
                    value={form.sample_available}
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        sample_available: e.target.value,
                      }))
                    }
                  />
                  <input
                    ...
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        sample_lead_time_days: e.target.value,
                      }))
                    }
                  />
                </>
              ...
              <div className="md:col-span-2 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-4 space-y-3">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    Upload MP4/WEBM inside GarTexHub or paste an internal
                    /uploads/... URL. External links are blocked.
                  </p>
                </div>

                {!editing?.id ? (
                  <div className="text-[11px] text-slate-500">
                    Save the product first to upload a video.
                  </div>
                ) : (
                  <>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      ...
                      <span className="text-[11px] text-slate-500">
                        {videoBusy ? "Uploading..." : "MP4/WEBM only"}
                      </span>
                    </div>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      <input
                    ...
                    {videoNotice ? (
                      <p className="text-[11px] text-rose-600">{videoNotice}</p>
                    ) : null}
                    {editing?.video_review_status ? (
                      <p className="text-[11px] text-slate-500">
                        Status:{" "}
                        {String(editing.video_review_status).replaceAll(
                          "_",
                          " ",
                        )}
                      </p>
                ...
                {!editing?.id ? (
                  ...
                  <>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      ...
                      />
                      <span className="text-[11px] text-slate-500">
                        {mediaBusy ? "Uploading..." : "PNG/JPG only"}
                      </span>
                    </div>
                    ...
                    {mediaNotice ? (
                      <p className="text-[11px] text-rose-600">{mediaNotice}</p>
                    ) : null}

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {mediaGallery.map((entry) => (
                        <div
                          key={entry.source_path || entry.url}
                          className="rounded-xl shadow-borderless dark:shadow-borderlessDark bg-white p-2 space-y-2"
                        >
                          {entry.url ? (
                            <img
                              src={entry.url}
                          ...
                          {entry.flags?.length ? (
                            <p className="text-[10px] text-rose-600">
                              Flags: {entry.flags.slice(0, 2).join(", ")}
                            </p>
                          ) : null}
            ...
            <div className="px-5 py-4 shadow-dividerT dark:shadow-dividerTDark bg-white flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <label className="flex items-center gap-2 text-[11px] text-slate-600">
                <input
                  ...
                  className="h-4 w-4"
                />
                I confirm this product media contains no music or prohibited
                instruments.
              </label>
              <div className="flex justify-end gap-2">
                ...
                <button
                  disabled={saving || !complianceChecked}
                  onClick={save}
                  className="rounded-full bg-[#0A66C2] px-4 py-2 text-xs font-semibold text-white hover:bg-[#004182] disabled:opacity-60"
                >
                  {saving ? "Saving..." : "Save"}
                </button>
              </div>
