  ...
  if (!canManage) {
    return (
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-borderless dark:shadow-borderlessDark p-6">
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="bg-white rounded-2xl shadow-borderless dark:shadow-borderlessDark p-4 flex items-start justify-between gap-3">
          <div>
        ...
        {notice ? <div className="bg-emerald-50 shadow-borderless dark:shadow-borderlessDark rounded-2xl p-4 text-sm text-emerald-800">{notice}</div> : null}
        {error ? <div className="bg-rose-50 shadow-borderless dark:shadow-borderlessDark rounded-2xl p-4 text-sm text-rose-800">{error}</div> : null}

        <div className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-4 text-xs text-slate-600">
        ...
        <div className="bg-white rounded-2xl shadow-borderless dark:shadow-borderlessDark p-4">
            ...
            {items.map((p) => (
              <div key={p.id} className="rounded-2xl shadow-borderless dark:shadow-borderlessDark bg-white p-4">
                ...
                {p.content_review_status === 'rejected' ? (
                  <div className="mt-2 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-rose-50 p-3 text-[11px] text-rose-700">
                ...
                {p.content_review_status === 'pending_review' ? (
                  <div className="mt-2 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-amber-50 p-3 text-[11px] text-amber-700">
                ...
                <div className="mt-3 flex gap-2">
                  <button onClick={() => openEdit(p)} className="flex-1 rounded-full shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button>
                  <button onClick={() => remove(p.id)} className="flex-1 rounded-full shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50">Delete</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {modalOpen ? (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-borderless dark:shadow-borderlessDark shadow-2xl overflow-hidden">
            <div className="px-5 py-4 shadow-dividerB dark:shadow-dividerBDark">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-3">
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Title" value={form.title} onChange={(e) => setForm((prev) => ({ ...prev, title: e.target.value }))} />
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Industry (optional)" value={form.industry} onChange={(e) => setForm((prev) => ({ ...prev, industry: e.target.value }))} />
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Category (e.g. Shirts)" value={form.category} onChange={(e) => setForm((prev) => ({ ...prev, category: e.target.value }))} />
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Material (e.g. Cotton)" value={form.material} onChange={(e) => setForm((prev) => ({ ...prev, material: e.target.value }))} />
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="MOQ" value={form.moq} onChange={(e) => setForm((prev) => ({ ...prev, moq: e.target.value }))} />
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Price range (optional)" value={form.price_range} onChange={(e) => setForm((prev) => ({ ...prev, price_range: e.target.value }))} />
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Lead time (days)" value={form.lead_time_days} onChange={(e) => setForm((prev) => ({ ...prev, lead_time_days: e.target.value }))} />
              <select className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" value={form.status} onChange={(e) => setForm((prev) => ({ ...prev, status: e.target.value }))}>
                <option value="published">Published (visible to buyers)</option>
                <option value="draft">Draft (private)</option>
              </select>
              <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Video URL (internal /uploads/...)" value={form.video_url} onChange={(e) => setForm((prev) => ({ ...prev, video_url: e.target.value }))} />
              <textarea className="md:col-span-2 rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm min-h-[120px]" placeholder="Description" value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))} />
              ...
              {advancedOpen ? (
                <>
                  <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Fabric GSM (optional)" value={form.fabric_gsm} onChange={(e) => setForm((prev) => ({ ...prev, fabric_gsm: e.target.value }))} />
                  <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Size range (optional)" value={form.size_range} onChange={(e) => setForm((prev) => ({ ...prev, size_range: e.target.value }))} />
                  <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Color / Pantone (optional)" value={form.color_pantone} onChange={(e) => setForm((prev) => ({ ...prev, color_pantone: e.target.value }))} />
                  <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Customization capabilities (optional)" value={form.customization_capabilities} onChange={(e) => setForm((prev) => ({ ...prev, customization_capabilities: e.target.value }))} />
                  <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Sample available (yes/no)" value={form.sample_available} onChange={(e) => setForm((prev) => ({ ...prev, sample_available: e.target.value }))} />
                  <input className="rounded-xl shadow-borderless dark:shadow-borderlessDark px-3 py-2 text-sm" placeholder="Sample lead time (days)" value={form.sample_lead_time_days} onChange={(e) => setForm((prev) => ({ ...prev, sample_lead_time_days: e.target.value }))} />
                </>
              ) : null}

              <div className="md:col-span-2 rounded-xl shadow-borderless dark:shadow-borderlessDark bg-slate-50 p-4 space-y-3">
                <div>
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      ...
                      <button
                        ...
                        disabled={videoBusy}
                        className="rounded-full shadow-borderless dark:shadow-borderlessDark px-4 py-2 text-xs font-semibold text-slate-600 disabled:opacity-60"
                      >
                        Remove video
                      </button>
                    </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {mediaGallery.map((entry) => (
                        <div key={entry.source_path || entry.url} className="rounded-xl shadow-borderless dark:shadow-borderlessDark bg-white p-2 space-y-2">
                          {entry.url ? (
            ...
            <div className="px-5 py-4 shadow-dividerT dark:shadow-dividerTDark bg-white flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setModalOpen(false)} className="rounded-full shadow-borderless dark:shadow-borderlessDark px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>
