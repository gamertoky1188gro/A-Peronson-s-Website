...
function TreeNode({ task, onToggle, depth = 0 }) {
	...
	return (
		<div class="select-none">
			<div
				class={`flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors ${
			...
			>
				{hasChildren ? (
					...
					>
						{isOpen ? <ChevronDown class="w-4 h-4" /> : <ChevronRight class="w-4 h-4" />}
					</button>
				) : (
					<span class="w-6" />
				)}
				...
				>
					{(allDone || task.isCompleted) && <Check class="w-3 h-3" />}
				</button>

				<span class={`font-medium ${task.isCompleted ? "line-through text-slate-400" : ""}`}>
					{task.title}
				</span>
				...
				{task.issues && task.issues.length > 0 && (
					<span class="text-xs text-orange-500 ml-auto">{task.issues.length} issues</span>
				)}
			</div>

			{hasChildren && isOpen && (
				<div class="border-l border-slate-200 dark:border-slate-700 ml-6">
					{task.children.map((child) => (
						<TreeNode key={child.id} task={child} onToggle={onToggle} depth={depth + 1} />
					))}
				</div>
			)}
		</div>
...
export default function TaskTracker() {
	...
	const totalTasks = tasks.reduce((acc, t) => acc + 1 + (t.children?.length || 0), 0);
	const completedTasks = tasks.reduce(
		(acc, t) =>
			acc + (t.isCompleted ? 1 : 0) + (t.children?.filter((c) => c.isCompleted).length || 0),
		0,
	);
	...
	return (
		<div class="min-h-screen bg-slate-50 dark:bg-[#0b1220] py-8 px-4">
			<div class="max-w-4xl mx-auto">
				<div class="flex items-center justify-between mb-6">
					<div>
						...
						<p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Task Progress Tracker</p>
					</div>
					<div class="flex items-center gap-4">
						<div class="text-right">
							...
							<div class="w-32 h-2 bg-slate-200 dark:bg-slate-700 rounded-full mt-1">
								<div
				...
				<ScrollReveal as="section">
					<div class="space-y-2">
						{filteredTasks.map((task) => (
							...
							>
								<div
									class="flex items-center gap-2 p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50"
								...
								>
									...
									>
										{(task.children?.every((c) => c.isCompleted) || task.isCompleted) && (
											<Check class="w-4 h-4" />
										)}
									</button>
									<span class="font-semibold text-lg flex-1">{task.title}</span>
								...
								{!collapsedRoots[task.id] && (
									<div class="px-4 pb-4">
										{task.description && (
											<p class="text-sm text-slate-600 dark:text-slate-400 mb-3 px-2">
												{task.description}
											</p>
										)}
										<div class="border-l border-slate-200 dark:border-slate-700 ml-3">
											{task.children?.map((child) => (
												<TreeNode key={child.id} task={child} onToggle={toggleTask} depth={1} />
											))}
										</div>
									</div>
								)}
