...
export default function CallInterface() {
  ...
  const effectiveMatchId =
    callDetails?.match_id || callDetails?.context?.chat_thread_id || matchId;

  const localName = user?.name || user?.email || "You";
  const remoteParticipant =
    participants.find((p) => p.id && p.id !== user?.id) || null;
  const remoteName =
  ...
  const connectionBadge = useMemo(() => {
    ...
    const rtcFailed =
      rtcConnectionState === "failed" || rtcIceState === "failed";
    const rtcConnecting =
      ["new", "connecting"].includes(rtcConnectionState) ||
  ...
  const buildMediaGate = (error) => {
    if (!window.isSecureContext) {
      return {
        ...
        message:
          "Open this app on https:// (or localhost). Then refresh and try again.",
        actionLabel: null,
      };
    }
    ...
    (token) => {
      ...
      pc.oniceconnectionstatechange = () => {
        ...
        if (pc.iceConnectionState === "failed") {
          setStatusMessage(
            "ICE negotiation failed (TURN server may be required).",
          );
  ...
  const requestMediaPermissions = useCallback(() => {
    ...
    ensureLocalStream()
      ...
      .catch(() =>
        pushToast("Please allow camera & microphone for calls.", "error"),
      );
  ...
  useEffect(() => {
    ...
    const shouldAutoScroll =
      firstLoad || (isChatOpenRef.current && (lastIsOwn || nearBottom));
    ...
    async function initPermissions() {
      ...
      if (!navigator?.permissions?.query) {
        ensureLocalStream().catch(() =>
          logger.warn("No permissions API, stream attempt failed"),
        );
        return;
      }
      ...
      try {
        ...
        ensureLocalStream().catch(() =>
          logger.warn("Permission-granted stream request failed"),
        );
      } catch {
        ensureLocalStream().catch(() =>
          logger.warn("Exception during stream request"),
        );
      }
    }
  ...
  useEffect(() => {
    ...
    tryAnswerPendingOffer().catch(() =>
      logger.warn("Failed to answer pending offer"),
    );
        ...
        if (payload.type === "joined_call_room") {
          ...
          if (payload.should_offer) {
            ...
            tryStartOffer().catch((error) => {
              safeSetStatus(
                `Unable to start call: ${error?.message || "offer failed"}`,
              );
            });
          }
          return;
        ...
        if (payload.type === "webrtc_signal" && payload.signal) {
          ...
          if (signal.type === "offer") {
            ...
            try {
              ...
              safeSetStatus(
                `Offer handling failed: ${error?.message || "unknown error"}`,
              );
            }
            ...
            try {
              ...
              safeSetStatus(
                `Answer handling failed: ${error?.message || "unknown error"}`,
              );
            }
    ...
    setIsSpeakerMuted((prev) => {
      ...
      if (remoteVideoRef.current) {
        ...
        if (!next) {
          ...
          if (playAttempt && typeof playAttempt.catch === "function") {
            playAttempt.catch(() =>
              logger.warn("Remote video play after unmute failed"),
            );
          }
        }
      }
  ...
  const startCallRecording = useCallback(async () => {
    ...
    try {
      ...
      const drawFrame = () => {
        ...
        recordingCleanupRef.current.raf =
