...
export const fetchAdminConfig = createAsyncThunk("config/fetchAdminConfig", async (_, { rejectWithValue }) => {
  try {
...
export const fetchInventory = createAsyncThunk("config/fetchInventory", async (_, { rejectWithValue }) => {
  try {
...
export const fetchUiConfig = createAsyncThunk("config/fetchUiConfig", async (_, { rejectWithValue }) => {
  try {
...
export const fetchCapabilities = createAsyncThunk("config/fetchCapabilities", async (moduleId, { rejectWithValue }) => {
  const endpoint =
    moduleId === "infra"
      ...
      : moduleId === "network"
        ...
        : moduleId === "ultra-security"
          ...
          : null;
  if (!endpoint) return rejectWithValue("Invalid module");
  try {
...
export const fetchActions = createAsyncThunk("config/fetchActions", async (_, { rejectWithValue }) => {
  try {
...
export const fetchActionGroups = createAsyncThunk("config/fetchActionGroups", async (_, { rejectWithValue }) => {
  try {
...
export const fetchRoles = createAsyncThunk("config/fetchRoles", async (_, { rejectWithValue }) => {
  try {
    const [rolesData, rolesList] = await Promise.all([
      ...
      apiRequest("/admin/config/roles-list", { method: "GET", token: getToken() }),
    ]);
...
const configSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchAdminConfig.pending, (state) => { state.loading = true; state.error = null; })
      ...
      .addCase(fetchInventory.pending, (state) => { state.inventoryLoading = true; })
      ...
      .addCase(fetchInventory.rejected, (state) => { state.inventoryLoading = false; })
      .addCase(fetchUiConfig.pending, (state) => { state.uiConfigLoading = true; })
      .addCase(fetchUiConfig.fulfilled, (state, action) => {
        state.uiConfigLoading = false;
        state.uiConfig = {
          chart_palette: action.payload?.chart_palette?.length >= 2 ? action.payload.chart_palette : DEFAULT_PIE_PALETTE,
      ...
      .addCase(fetchUiConfig.rejected, (state) => { state.uiConfigLoading = false; })
      .addCase(fetchCapabilities.pending, (state) => { state.capabilitiesLoading = true; })
      .addCase(fetchCapabilities.fulfilled, (state, action) => {
        state.capabilitiesLoading = false;
        state.capabilities = action.payload || [];
      })
      .addCase(fetchCapabilities.rejected, (state) => { state.capabilitiesLoading = false; })
      .addCase(fetchActions.pending, (state) => { state.actionsLoading = true; })
      .addCase(fetchActions.fulfilled, (state, action) => {
        state.actionsLoading = false;
        state.actions = action.payload || [];
      })
      .addCase(fetchActions.rejected, (state) => { state.actionsLoading = false; })
      .addCase(fetchActionGroups.pending, (state) => { state.actionGroupsLoading = true; })
      ...
      .addCase(fetchActionGroups.rejected, (state) => { state.actionGroupsLoading = false; })
      .addCase(fetchRoles.pending, (state) => { state.roleConfigLoading = true; })
      ...
      .addCase(fetchRoles.rejected, (state) => { state.roleConfigLoading = false; });
  },
});
