  ...
  extraReducers: (builder) => {
    builder
        ...
        state.uiConfig = {
          chart_palette:
            action.payload?.chart_palette?.length >= 2
              ...
              : DEFAULT_PIE_PALETTE,
          section_metrics:
            action.payload?.section_metrics || DEFAULT_SECTION_METRICS,
          empty_states:
            action.payload?.empty_states || DEFAULT_EMPTY_STATE_COPY,
        };
      })
      ...
      .addCase(fetchRoles.fulfilled, (state, action) => {
        ...
        state.roleConfig = {
          known_roles:
            action.payload.known_roles || initialState.roleConfig.known_roles,
          allowed_roles:
            action.payload.allowed_roles ||
            initialState.roleConfig.allowed_roles,
