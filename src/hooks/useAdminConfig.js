...
const ULTRA_CAPABILITIES_DEFAULT = [
  "Zero-Trust Access Controls", "Role-Based Permissions", "Audit Logging",
  "Encrypted Data Storage", "Multi-Factor Authentication", "Real-Time Threat Detection",
  "Compliance Management", "Secure API Gateway",
];
...
export function useAdminFullConfig() {
  ...
  const inventory = state.config?.inventory || DEFAULT_ADMIN_PANEL_FALLBACK_INVENTORY;
  ...
  const infraCapabilities = state.config?.infra_capabilities?.length
    ? state.config.infra_capabilities : INFRA_CAPABILITIES_DEFAULT;
  const networkCapabilities = state.config?.network_capabilities?.length
    ? state.config.network_capabilities : NETWORK_CAPABILITIES_DEFAULT;
  const ultraCapabilities = state.config?.ultra_capabilities?.length
    ? state.config.ultra_capabilities : ULTRA_CAPABILITIES_DEFAULT;
...
export function useInventory() {
  ...
  return {
    inventory: inventory.length > 0 ? inventory : DEFAULT_ADMIN_PANEL_FALLBACK_INVENTORY,
    loading,
  };
}
...
export function useCapabilities(moduleId) {
  const dispatch = useDispatch();
  const { capabilities, capabilitiesLoading: loading } = useSelector((s) => s.config);
...
export function useActionGroups() {
  const dispatch = useDispatch();
  const { actionGroups: groups, actionGroupsLoading: loading } = useSelector((s) => s.config);
...
export function useRoleConfig() {
  const dispatch = useDispatch();
  const { roleConfig, roleConfigLoading: loading } = useSelector((s) => s.config);
  ...
  return {
    ...
    rolesList: roles,
    buyerBenefits: roles.find((r) => r.role_key === "buyer")?.benefits || BUYER_BENEFITS_DEFAULT,
    factoryBenefits: roles.find((r) => r.role_key === "factory")?.benefits || FACTORY_BENEFITS_DEFAULT,
    buyingHouseBenefits: roles.find((r) => r.role_key === "buying_house")?.benefits || BUYING_HOUSE_BENEFITS_DEFAULT,
  };
}
