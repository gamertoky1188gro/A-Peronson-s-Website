...
const DEFAULT_CMS_WEEKLY_TREND = [
  { name: 'Mon', value: 24 }, { name: 'Tue', value: 38 }, { name: 'Wed', value: 29 },
  { name: 'Thu', value: 57 }, { name: 'Fri', value: 44 }, { name: 'Sat', value: 66 }, { name: 'Sun', value: 52 },
]
const DEFAULT_ULTRA_MINI_CHART_POINTS = [22, 28, 24, 34, 30, 46, 40, 54, 50, 66, 58, 72]
...
const KNOWN_ROLES = ['buyer', 'factory', 'buying_house', 'owner', 'admin', 'agent']
const INFRA_CAPABILITIES_DEFAULT = [
  { capability_id: 'system_health', title: 'System Health & Performance Monitoring', count: 5, icon_name: 'Activity', subtitle: 'Real-time signals, resource visibility, and operational pulse.' },
  { capability_id: 'os_maintenance', title: 'OS & Software Maintenance', count: 4, icon_name: 'Server', subtitle: 'Safe updates, package checks, and controlled maintenance flows.' },
]
const NETWORK_CAPABILITIES_DEFAULT = [
  { capability_id: 'monitoring', title: 'Real-Time Monitoring & Visibility', count: 4, icon_name: 'Activity', subtitle: 'Interface health, packet loss, latency heatmap.' },
]
const ULTRA_CAPABILITIES_DEFAULT = [
  { capability_id: 'zero_trust', title: 'Zero-Trust Access Controls', count: 8, icon_name: 'Lock', subtitle: 'Mandatory MFA, session timeout, IP whitelisting.' },
]
const BUYER_BENEFITS_DEFAULT = ['Access to factory network', 'Request quotations', 'Track orders']
const FACTORY_BENEFITS_DEFAULT = ['Receive RFQs', 'Manage products', 'Track shipments']
const BUYING_HOUSE_BENEFITS_DEFAULT = ['Manage multiple buyers', 'Commission tracking', 'Supplier network']
...
export function useAdminFullConfig() {
  ...
  const refetch = useCallback(async () => {
    setLoading(true)
    try {
      const data = await apiRequest('/admin/config/total-config', { method: 'GET' })
      setConfig(data)
      setError(null)
    } catch (e) {
  ...
  const infraCapabilities = config?.infra_capabilities?.length ? config.infra_capabilities : INFRA_CAPABILITIES_DEFAULT
  const networkCapabilities = config?.network_capabilities?.length ? config.network_capabilities : NETWORK_CAPABILITIES_DEFAULT
  const ultraCapabilities = config?.ultra_capabilities?.length ? config.ultra_capabilities : ULTRA_CAPABILITIES_DEFAULT
  const actions = config?.actions || []
  const rolesList = config?.roles?.roles || []
  
  const knownRoles = roles?.known_roles || KNOWN_ROLES
  const allowedRoles = roles?.allowed_roles || DEFAULT_ADMIN_PANEL_ALLOWED_ROLES
  const piePalette = uiConfig?.chart_palette?.length >= 2 ? uiConfig.chart_palette : DEFAULT_PIE_PALETTE
...
export function useInventory() {
  const [inventory, setInventory] = useState(DEFAULT_ADMIN_PANEL_FALLBACK_INVENTORY)
  const [loading, setLoading] = useState(true)
  ...
  useEffect(() => {
    async function fetchInventory() {
      setLoading(true)
      try {
        const data = await apiRequest('/admin/config/inventory', { method: 'GET' })
  ...
  useEffect(() => {
    async function fetchUiConfig() {
      ...
      try {
        ...
        setUiConfig({
          chart_palette: data?.chart_palette?.length >= 2 ? data.chart_palette : DEFAULT_PIE_PALETTE,
...
export function useCapabilities(moduleId) {
  ...
  useEffect(() => {
    async function fetchCapabilities() {
      setLoading(true)
      try {
        const endpoint = moduleId === 'infra' ? '/admin/config/infra-capabilities' :
                        moduleId === 'network' ? '/admin/config/network-capabilities' :
                        moduleId === 'ultra-security' ? '/admin/config/ultra-capabilities' : null
        if (endpoint) {
...
export function useRoleConfig() {
  const [roles, setRoles] = useState({ known_roles: KNOWN_ROLES, allowed_roles: DEFAULT_ADMIN_PANEL_ALLOWED_ROLES, roles: [] })
  const [loading, setLoading] = useState(true)
  ...
  return {
    ...
    buyerBenefits: roles.roles?.find(r => r.role_key === 'buyer')?.benefits || BUYER_BENEFITS_DEFAULT,
    factoryBenefits: roles.roles?.find(r => r.role_key === 'factory')?.benefits || FACTORY_BENEFITS_DEFAULT,
    buyingHouseBenefits: roles.roles?.find(r => r.role_key === 'buying_house')?.benefits || BUYING_HOUSE_BENEFITS_DEFAULT,
  }
}
