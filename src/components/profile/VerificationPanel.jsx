			...
			<div className="mt-4">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
					{(summary?.required_checklist || []).map((row) => (
						<div
							key={row.key}
							className="flex items-center justify-between gap-2 rounded-xl bg-white px-3 py-2 ring-1 ring-slate-200/70 dark:bg-white/5 dark:ring-white/10"
						>
							<span className="text-xs font-semibold text-slate-800 dark:text-slate-100">
								{row.label}
							</span>
							<span
								className={`text-xs font-bold${row.submitted ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}
							>
