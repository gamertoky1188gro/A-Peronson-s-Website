                  ...
                  {!searchLoading && !searchError && searchResults.map((result) => (
                    <div key={result.id} className="mb-1 rounded-lg border border-slate-100 px-2 py-2 last:mb-0 dark:border-slate-800">
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{result.name}</p>
                      ...
                      <div className="mt-2 flex gap-2">
                        <button
                          disabled={result.following || actionBusyKey === `follow:${result.id}`}
                        ...
                        >
                          {result.following ? 'Following' : 'Follow (optional)'}
                        </button>
                        <button
                          disabled={['friends', 'requested'].includes(result.friend_status) || actionBusyKey === `friend:${result.id}`}
                        ...
                        >
                          {result.friend_status === 'incoming' ? 'Accept Friend' : 'Add Friend'}
                        </button>
                      </div>
                    </div>
                  ))}
