  ...
  try {
    ...
    const formatted = lines
      .map((line) => {
        ...
        const isSelfClosing =
          /\/>$/.test(line) || /^<\*xml/.test(line) || /^<!/.test(line);
...
export default function AttachmentPreviewModal({
  ...
  const fileExt = useMemo(
    () => safeExt(file?.name) || safeExt(file?.url),
    [file?.name, file?.url],
  );
  ...
  useEffect(() => {
    ...
    async function highlight() {
      ...
      try {
        ...
        const targetLang =
          kind === "xml" ? "markup" : prismLanguageForExt(fileExt) || "markup";
  ...
  useEffect(() => {
    ...
    function sheetToRows(sheet, utils) {
      ...
      const rawRows =
        utils.sheet_to_json(sheet, { header: 1, raw: false, defval: "" }) || [];
            ...
            <div className="space-y-3">
              {!videoError ? (
                ...
                <div className="rounded-xl shadow-borderless dark:shadow-borderlessDark bg-white/5 p-4 text-sm text-slate-200">
                  This video format can't be previewed in your browser. Please
            ...
            <div className="space-y-3">
              {officeIsPrivate ? (
                <div className="rounded-xl shadow-borderless dark:shadow-borderlessDark bg-yellow-500/10 p-4 text-sm text-yellow-100">
                  Microsoft Office Web Viewer needs a publicly reachable URL.
                  Your file URL looks local/private ({absoluteFileUrl}). Please
                  use Cloudflare Tunnel / a public domain, or download the file.
                </div>
              ) : null}
                  ...
                  {spreadsheetState.sheetNames.length > 1 ? (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          selectSpreadsheetSheet(event.target.value)
                        }
                    ...
                    <table className="min-w-full text-[12px] text-slate-100 shadow-borderless dark:shadow-borderlessDark">
                      <tbody>
                        {(spreadsheetState.rows || []).map((row, rowIndex) => (
                          <tr
                            key={rowIndex}
                            className={rowIndex === 0 ? "bg-white/5" : ""}
                          >
