		...
		<div class="space-y-3">
			<div class="flex items-center gap-3">
				<button
					...
					disabled={uploading}
					class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
				>
					{uploading ? (
				...
				{uploading && <UploadProgressBar progress={uploadProgress} class="w-40" />}
				<input
					ref={fileInputRef}
					type="file"
			...
			{error && (
				<div class="rounded-xl bg-rose-50 p-3 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-200">
					{error}
				</div>
			)}
			...
			{value && (
				<div class="flex items-center gap-3">
					<img
						src={value}
						alt="Preview"
						class="h-16 w-16 rounded-xl object-cover ring-1 ring-slate-200/60 dark:ring-slate-800"
						onError={(e) => {
							e.target.style.display = "none";
						}}
					/>
					<span class="text-xs text-slate-500">Image set</span>
				</div>
			)}
		</div>
	);
