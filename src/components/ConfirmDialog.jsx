...
export default function ConfirmDialog({
	...
	return (
		<AnimatedModal
			open={open}
			onClose={onClose}
			class="w-[90vw] max-w-sm p-6 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700"
		>
			...
			<div class="flex gap-3 justify-end">
				<button
					type="button"
					onClick={onClose}
					class="px-4 py-2 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
				>
					{cancelLabel}
				</button>
				<button
					...
					class={`px-4 py-2 rounded-xl text-sm font-medium text-white transition-colors ${
						destructive ? "bg-red-500 hover:bg-red-600" : "bg-sky-500 hover:bg-sky-600"
					}`}
				>
					{confirmLabel}
				</button>
