...
const score = (0.7 * completeness) + (0.3 _ fitScore)
return { score: Math.max(0, Math.min(1, Math.round(score _ 100) / 100)), missing, fitScore }
}
...
if (!requirement?.moq) {
const moqVal = extractNumberFromText(lower, /\bmoq\b[^\d]{0,10}(\d[\d,]_)/i)
?? extractNumberFromText(lower, /\bminimum order\b[^\d]{0,10}(\d[\d,]_)/i)
if (moqVal) {
...
const qtyVal = extractNumberFromText(lower, /\b(qty|quantity|pcs|pieces)\b[^\d]{0,10}(\d[\d,]\_)/i)
if (qtyVal) {
