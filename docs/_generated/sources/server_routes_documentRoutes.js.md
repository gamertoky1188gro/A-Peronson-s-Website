...
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 50 _ 1024 _ 1024 } })
const router = Router()
...
// Provider webhook (no auth) - validate with ESIGN*WEBHOOK_SECRET
router.post('/contracts/:contractId/sign-callback', express.raw({ type: '*/\_', limit: '1mb' }), createContractSignCallback)
