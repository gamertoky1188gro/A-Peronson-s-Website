...
try {
const entityType = req.body?.entity*type || 'verification'
...
const retentionMs = Number(process.env.ESIGN_WEBHOOK_NONCE_RETENTION_MS || 24 * 60 _ 60 _ 1000)
...
try {
logInfo('esign*webhook_received', { contractId: req.params.contractId, hasSignature: Boolean(signatureHeader), hasSecret: Boolean(incomingSecretHeader) })
} catch {
void 0
}
