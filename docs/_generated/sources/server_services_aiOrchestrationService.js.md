...
11 | }
12 |
13 | const DEFAULT*CONFIDENCE_THRESHOLD = getEnvFloat('AI_HANDOFF_THRESHOLD', 0.65)
...
171 | const * = extractedRequirements || {}
...
177 | if (!approval?.approved) return { sent: false, status: 'manual*required', message: 'Draft not approved.' }
178 |
...
196 | const cutoff = Date.now() - (60 * 60 \_ 1000)
