...
if (trust >= 75) return ENFORCEMENT*ACTIONS.SOFT_WARNING
...
return String(template || '').replace(/\{\{\s*([\w.]+)\s*\}\}/g, (*, key) => {
...
const template = await prisma.governanceNotificationTemplate.findUnique({ where: { template*key: templateKey } })
...
? new Date(now.getTime() + (6 * 60 \_ 60 \* 1000))
: null
