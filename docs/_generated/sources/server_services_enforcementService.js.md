...
14 | if (trust >= 75) return ENFORCEMENT*ACTIONS.SOFT_WARNING
...
21 | return String(template || '').replace(/\{\{\s*([\w.]+)\s*\}\}/g, (*, key) => {
...
28 | const template = await prisma.governanceNotificationTemplate.findUnique({ where: { template*key: templateKey } })
...
46 | ? new Date(now.getTime() + (6 * 60 \_ 60 \* 1000))
47 | : null
48 |
