...
const baseOrigin = API_BASE.replace(/\/api\/_$/, '')
return `${baseOrigin}${pdfPath.startsWith('/') ? '' : '/'}${pdfPath}`   67 | }
...
try {
const data = await apiRequest(`/payment-proofs?contract*id=${encodeURIComponent(contractId)}`, { token })
...
<div className="font-semibold">{String(proof.type || '').replace('*', ' ').toUpperCase()}</div>
<div className="rounded-full bg-white px-2 py-1 text-[10px] font-semibold text-slate-600 ring-1 ring-slate-200">
{proof.status || 'pending'}
</div>
</div>
<div className="mt-2 grid grid-cols-1 gap-1">
{proof.transaction*reference ? <div>Ref: {proof.transaction_reference}</div> : null}
