...
const POLICY*FILE = 'org_ops_policies.json'
...
const stageKey = sanitizeString(String(stage || ''), 60).toLowerCase().replace(/\s+/g, '*')
const minutes = Math.max(1, Number(value || DEFAULT*STAGE_TARGETS[stageKey] || 60))
...
const leadStatus = sanitizeString(String(lead?.status || 'new'), 60).toLowerCase().replace(/\s+/g, '*')
const targetMinutes = Math.max(1, Number(policy?.sla*targets_by_stage?.[leadStatus] || DEFAULT_STAGE_TARGETS[leadStatus] || 60))
...
? `time_breach_stage*${breachedTimer.timer.stage}`
