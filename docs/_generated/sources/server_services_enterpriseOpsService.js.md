...
10 | const POLICY*FILE = 'org_ops_policies.json'
...
58 | const stageKey = sanitizeString(String(stage || ''), 60).toLowerCase().replace(/\s+/g, '*')
59 | const minutes = Math.max(1, Number(value || DEFAULT*STAGE_TARGETS[stageKey] || 60))
...
212 | const leadStatus = sanitizeString(String(lead?.status || 'new'), 60).toLowerCase().replace(/\s+/g, '*')
213 | const targetMinutes = Math.max(1, Number(policy?.sla*targets_by_stage?.[leadStatus] || DEFAULT_STAGE_TARGETS[leadStatus] || 60))
...
335 | ? `time_breach_stage*${breachedTimer.timer.stage}`
