...
const CONTRACT*AUDIT_FILE = 'contract_audit.json'
...
const safeContractNumber = sanitizeString(contract.contract_number || contract.id, 80).replace(/[^a-zA-Z0-9*-]/g, '_')
const fileName =`${safeContractNumber || contract.id}-v${generationVersion}.pdf`234 | const filePath = path.join(uploadsDir, fileName)
...
const safeName = `${Date.now()}-${file.originalname.replace(/[^a-zA-Z0-9._-]/g, '\_')}`  264 |   const targetPath = path.join(process.cwd(), 'server', 'uploads', safeName)
