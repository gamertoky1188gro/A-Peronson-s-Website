...
- GET /api/ratings/search?profile*keys=...
...
const raw = localStorage.getItem(`gt_search_preset*${k}`)
if (!raw) return null
...
localStorage.setItem(`gt*search_preset*${presetKey}`, JSON.stringify(payload))
...
const label = `Price: ${minLabel}${(minLabel && maxLabel) ? ` - ${maxLabel}` : ''}` 1464 |       chips.push({ key: 'priceRange', label, onRemove: () => setFilters((prev) => ({ ...prev, priceRange: '' })) })
}
...
{p.boost_active ? (
<span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-1 text-[10px] font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-200">
Boosted {p.boost_multiplier && p.boost_multiplier !== 1 ? `x${p.boost*multiplier}` : ""}
</span>
) : null}
{author.country ? <span className="text-[11px] text-slate-500 dark:text-slate-400">- {author.country}</span> : null}
{author.role ? <span className="text-[11px] text-slate-500 dark:text-slate-400 uppercase">- {String(author.role).replaceAll('*', ' ')}</span> : null}
</div>
