...
const FX*REFRESH_INTERVAL_MS = 60 * 60 _ 1000
...
const JWT*SECRET = process.env.JWT_SECRET || 'mvp-dev-secret'
...
setInterval(() => {
runLeadReminderSweep().catch((error) => logError('lead*reminder_sweep_failed', error))
}, 5 * 60 \_ 1000)
server.listen(PORT, () => {
