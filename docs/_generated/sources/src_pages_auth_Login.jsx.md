...
{/_ Login card container (max width keeps form readable). _/}
...
{/_ Controlled form: React state is the single source of truth for inputs. _/}
...
{/_ Password is required; actual auth validation happens server-side. _/}
...
{/_ Inline error (only renders when there is an error string). _/}
{error ? <p className="text-sm text-red-500">{error}</p> : null}

{/_ Primary CTA: uses brand blue color _/}
...
{/_ Secondary nav: send user to signup if they do not have an account yet. _/}
<p className="mt-6 text-sm text-gray-600">
New here_ <Link className="text-[var(--gt-blue)] hover:underline" to="/signup">Create account</Link>
</p>
</div>
</div>
)
