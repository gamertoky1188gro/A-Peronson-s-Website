...
_ Member/Team system (Phase 2)
_ - "Members" are stored as real user rows in `users.json` with `role: "agent"`.
_ - This enables single-field Agent login: Email OR Agent ID (member_id).
_ - Legacy support: if `members.json` exists with old rows, we migrate them into users.json on demand.
\*/
