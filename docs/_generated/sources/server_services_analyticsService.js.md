...
18 | const SEARCH*TREND_MIN_EVENTS = 25
...
108 | if (h < 1) return `${Math.round(h * 60)}m`109 | if (h < 24) return`${Math.round(h)}h`
110 | return `${Math.round(h / 24)}d`111 | }
112 |
...
281 | ?`${buyersActive.size}:${suppliersActive.size}`282 | :`${buyersActive.size}:0`
283 |
...
549 | const key = `${sourceType}:${sourceId || 'unknown'}`550 | const existing = leadSources.get(key) || {
...
576 | } else {
577 | label = entry.source_type.replace(/*/g, ' ')
578 | }
579 | }
...
593 | avg_response_hours: Math.round(avgResponseHours _ 10) / 10,
...
621 | const retentionMs = Math.max(1, Number(governance.retention_days || 365)) _ 24 _ 60 _ 60 _ 1000
...
710 | const last30 = now - 30 _ 24 _ 60 _ 60 _ 1000
711 | const prev30 = now - 60 _ 24 _ 60 _ 60 \* 1000
...
874 | rawReport.org_scope = orgScopeId ? `org:${orgScopeId.slice(0, 6)}**_`: 'org:global'
875 |
...
922 | rawReport.org_scope = orgScopeId ?`org:${orgScopeId.slice(0, 6)}_**`: 'org:unknown'
923 |
...
1058 | ]
1059 |
1060 | const signedContracts = myContracts.filter((c) => String(c.lifecycle*status || '').toLowerCase() === 'signed').length
...
1090 | }
1091 |
1092 | const orgId = role === 'agent' ? String(user.org*owner_id || '') : String(user.id || '')
