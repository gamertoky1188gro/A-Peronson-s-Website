...
const bucket = map.get(key) || { field, value: raw, user*ids: [] }
...
average_redemption_usd: redemptionsForCode.length ? Math.round((total / redemptionsForCode.length) _ 100) / 100 : 0,
...
acc[key].redeemed_total_usd = Math.round((acc[key].redeemed_total_usd + row.redeemed_total_usd) _ 100) / 100
