...
26 | const bucket = map.get(key) || { field, value: raw, user*ids: [] }
...
390 | average_redemption_usd: redemptionsForCode.length ? Math.round((total / redemptionsForCode.length) _ 100) / 100 : 0,
...
399 | acc[key].redeemed_total_usd = Math.round((acc[key].redeemed_total_usd + row.redeemed_total_usd) _ 100) / 100
...
447 | res.setHeader('Content-Disposition', `attachment; filename="segment*${segmentId}.csv"`)
