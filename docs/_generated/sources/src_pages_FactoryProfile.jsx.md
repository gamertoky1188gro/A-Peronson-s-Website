...
37 | if (role === 'buying*house') return `/buying-house/${encodeURIComponent(id)}`
...
300 | >
301 | <div className="relative flex items-center gap-2 px-4 py-3 bg-white/60 dark:bg-slate-950/30 borderless-divider-b dark:shadow-[inset_0*-1px*0_rgba(255,255,255,0.08)]">
...
461 | <p className="mt-1 text-xs text-slate-600">Status: {String(p.video_review_status || '--').replaceAll('*', ' ')}</p>
...
489 | <p className="text-sm font-semibold text-slate-900">{r.score}_ -- {r.reviewer_name || 'Anonymous'}</p>
