...
838 | const level = Math.min(1, rms _ 3.2)
839 | smoothing = smoothing _ 0.82 + level _ 0.18
840 |
...
858 | }
859 | try { source?.disconnect?.() } catch { /_ ignore _/ }
860 | try { analyser?.disconnect?.() } catch { /_ ignore _/ }
861 | try { ctx?.close?.() } catch { /_ ignore _/ }
...
1292 | const pad = 22
1293 | const pipW = Math.round(canvas.width _ 0.28)
1294 | const pipH = Math.round(canvas.height _ 0.28)
...
1582 | {/_ Remote Participant Label _/}
...
1654 | </div>
1655 |
1656 | {/_ Floating Call Controls _/}
...
1685 | style={!isMuted && micLevel > 0.02 ? { boxShadow: `0 0 ${10 + micLevel _ 26}px rgba(34,211,238,${0.18 + micLevel * 0.35})` } : undefined}
 1686 |               >
 ...
 1723 |                   style={{ transformOrigin: 'bottom', transform: `scaleY(${Math.max(0.18, base + micLevel _ 0.9)})` }}
1724 | />
1725 | ))}
1726 | </div>
...
1731 | </div>
1732 |
1733 | {/_ Right Side: Chat Sidebar \*/}
1734 | {isChatOpen ? (
