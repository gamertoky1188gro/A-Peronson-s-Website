...
{/_ Centered panel to keep the message readable and focused. _/}
...
{/_ Show the blocked route (fallback to generic wording if missing). _/}
You do not have permission to access <strong>{location.state?.from || 'this page'}</strong>.
</p>
{/_ Primary actions: re-auth or return to a safe default page. \*/}
