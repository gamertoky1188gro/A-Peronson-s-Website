...
if (!fs.existsSync(callsUploadRoot)) fs.mkdirSync(callsUploadRoot, { recursive: true })
...
const safeCallId = String(req.params.callId || 'call').replace(/[^a-zA-Z0-9_-]/g, '\_')
cb(null, `${safeCallId}-${Date.now()}${safeExt}`)
},
}),
limits: { fileSize: 120 _ 1024 _ 1024 },
})
