...
510 | const status = String(row?.review*status || row?.verification_status || '').toLowerCase()
...
656 | revenue_estimate_usd: premiumUsers * Number(config.pricing?.premium*usd || 0),
...
662 | restricted*usd: Math.round(walletRestricted * 100) / 100,
663 | credits*issued_usd: Math.round(totalCreditsIssued * 100) / 100,
