...
const status = String(row?.review*status || row?.verification_status || '').toLowerCase()
...
revenue_estimate_usd: premiumUsers * Number(config.pricing?.premium*usd || 0),
...
restricted*usd: Math.round(walletRestricted * 100) / 100,
credits*issued_usd: Math.round(totalCreditsIssued * 100) / 100,
