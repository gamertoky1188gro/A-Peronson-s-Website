...
70 | const entityType = raw.feed*type === 'buyer_request' ? 'buyer_request' : 'company_product'
...
86 | accountType: accountType ? String(accountType).replaceAll('*', ' ') : (isBuyerRequest ? 'Buyer' : 'Company'),
87 | rolePath,
88 | },
89 | verified: Boolean(raw.author?.verified || raw.verified),
90 | createdAt: formatRelativeTime(raw.created*at),
...
477 | <p className="text-xs text-slate-500">{user?.role ? user.role.replaceAll('*', ' ') : 'Account'}</p>
478 | </div>
479 | </div>
480 |
...
621 | transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1], delay: idx _ 0.05 }}
622 | >
623 | <FeedItemCard
624 | item={item}
