...
10 | const AUTO*RATING_DAYS = Number(process.env.AUTO_RATING_DAYS || 7)
...
131 | const marginError = 1.96 _ (stdDev / Math.sqrt(sampleSize))
132 | const normalizedConfidence = Math.max(0, Math.min(1, (sampleSize / (sampleSize + 6)) _ (1 - (stdDev / 2.5))))
133 |
134 | return {
135 | sample*size: sampleSize,
...
195 | responseTimes.push((end - start) / (1000 * 60 _ 60))
196 | }
197 |
...
229 | const rounded = Math.round(score _ 10) / 10
...
244 | row.profile*key === normalizedProfile
...
360 | const cutoffMs = Math.max(1, AUTO_RATING_DAYS) * 24 _ 60 _ 60 \_ 1000
