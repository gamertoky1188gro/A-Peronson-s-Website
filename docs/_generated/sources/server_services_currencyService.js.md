...
55 | 1,
56 | Number(row?.staleToleranceMinutes || row?.staleThresholdHours * 60 || DEFAULT*STALE_TOLERANCE_MINUTES),
57 | ),
58 | }
59 | } catch {
60 | return {
...
144 | if (!response.ok) throw new Error(`fx_http*${response.status}`)
...
219 | rate: 1,
220 | currency*from: fromCurrency,
