...
1,
Number(row?.staleToleranceMinutes || row?.staleThresholdHours * 60 || DEFAULT*STALE_TOLERANCE_MINUTES),
),
}
} catch {
return {
...
if (!response.ok) throw new Error(`fx_http*${response.status}`)
