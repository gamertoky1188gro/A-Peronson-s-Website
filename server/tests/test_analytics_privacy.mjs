...
function testSuppressLabeledItems() {
  const raw = {
    ...
    top_categories_global: [ { label: 'A', count: 3 }, { label: 'B', count: 1 } ],
    price_range_demand: [ { bucket: '0-5', count: 2 } ],
    monthly_demand_trend: [],
  }

  const config = { enabled: true, min_cohort_size: 5, geo_granularity: 'country', date_granularity: 'month', retention_days: 365 }
  const result = sanitizePlatformAnalytics(raw, config)
  // suppressed values should be present (insufficient_data bucket)
  assert(result.report.top_categories_global.some((r) => r.count === 0 || r.bucket === 'insufficient_data' || r.count >= 0))
