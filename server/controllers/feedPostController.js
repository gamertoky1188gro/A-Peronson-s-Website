import { createFeedPost, deleteFeedPost, listFeedPosts, updateFeedPost } from '../services/feedPostService.js'
...
export async function patchFeedPost(req, res) {
  try {
    const row = await updateFeedPost(req.user, req.params.postId, req.body || {})
    return res.json(row)
  } catch (error) {
    return handleControllerError(res, error)
  }
