...
export async function listDisputesAdmin(req, res) {
  ...
  const disputes = reports.filter(
    (r) => String(r.entity_type || "") === "contract_dispute",
  );
...
export async function listPaymentProofsAdmin(req, res) {
  ...
  const items = sortByDateDesc(rows).map((proof) => ({
    ...
    evidence_present: Boolean(
      proof.document_url || proof.file_url || proof.attachment_url,
    ),
  }));
  return res.json({ items });
}
...
export async function listCouponReport(req, res) {
  ...
  const byCode = codeRows.map((code) => {
    const redemptionsForCode = redemptionRows.filter(
      (r) => String(r.code_id) === String(code.id),
    );
    const total = redemptionsForCode.reduce(
      (sum, r) => sum + Number(r.amount_usd || 0),
      0,
    );
    const uniqueUsers = new Set(
      redemptionsForCode.map((r) => String(r.user_id)),
    ).size;
    ...
    return {
      ...
      average_redemption_usd: redemptionsForCode.length
  ...
  const byCampaign = byCode.reduce((acc, row) => {
    ...
    acc[key].redeemed_total_usd =
      Math.round((acc[key].redeemed_total_usd + row.redeemed_total_usd) * 100) /
      100;
  ...
  return res.json({
    ...
    campaigns: Object.values(byCampaign).map((row) => ({
      ...
      unique_users:
        row.unique_users instanceof Set
          ? row.unique_users.size
          : row.unique_users || 0,
    })),
  });
}
...
export async function exportEmailSegmentAdmin(req, res) {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="segment_${segmentId}.csv"`,
  );
  return res.send(csv);
}
