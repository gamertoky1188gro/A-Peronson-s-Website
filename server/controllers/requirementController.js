...
export async function searchRequirements(req, res) {
  ...
  const results = all
      ...
      return {
        ...
        author: buyer
          ? {
              ...
              avg_response_hours:
                responseTimeByOwner.get(String(buyer.id)) ?? null,
              order_certification_status: certification?.status || "",
              permission_matrix:
                profile?.permission_matrix || buyer?.permission_matrix || null,
    ...
    .filter((r) => {
      if (combinedIdSet) {
        if (!combinedIdSet.has(String(r.id))) return false;
      ...
      if (
        wantedOrgType &&
        String(r.author?.role || "").toLowerCase() !== wantedOrgType
      )
        return false;
      if (
        wantedCountry &&
        String(r.author?.country || "").toLowerCase() !== wantedCountry
      )
        return false;
      ...
      if (leadTimeMax !== null) {
        const timeline = parseNumber(
          r.timeline_days || r.delivery_timeline || "",
        );
      ...
      if (
        wantedAuditDate &&
        String(r.audit_date || "").toLowerCase() !== wantedAuditDate
      )
        return false;
      ...
      if (sampleLeadTimeMax !== null) {
        const sampleLead = parseNumber(
          r.sample_lead_time_days || r.sample_timeline || "",
        );
      ...
      if (roleSeatsMap && Object.keys(roleSeatsMap).length) {
        const profileRoleSeats =
          r.author?.role_seats || r.author?.roleSeats || {};
        const permMemberSeats =
          r.author?.permission_matrix?.members?.seats || null;
      ...
      if (handlesMultipleFactoriesFilter !== null) {
        if (
          Boolean(r.author?.handles_multiple_factories) !==
    ...
    (acc, row) => {
      ...
      if (customization)
        acc.customization[customization] =
          (acc.customization[customization] || 0) + 1;
