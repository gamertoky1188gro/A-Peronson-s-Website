...
export async function createSupportTicketController(req, res) {
  ...
  return res.status(201).json({ ticket: ticketResult.ticket, message: ticketResult.initial_message })
}
...
export async function listSupportTicketMessagesController(req, res) {
  ...
  if (String(ticket.user_id) !== String(req.user.id)) return res.status(403).json({ error: 'Forbidden' })
...
export async function postSupportTicketMessageController(req, res) {
  ...
  if (!ticketId || !message) return res.status(400).json({ error: 'message is required' })
  const entry = await appendSupportTicketMessage(ticketId, req.user, message)
  if (entry === 'forbidden') return res.status(403).json({ error: 'Forbidden' })
