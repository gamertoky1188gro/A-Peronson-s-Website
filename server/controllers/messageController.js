...
export async function sendMessage(req, res) {
  ...
  if (!allowed)
    return res
      .status(403)
      .json({ error: "Only connected friends can message in this thread" });
  ...
  try {
    ...
    return res.status(error.status || 400).json({
      ...
      retry_after_seconds:
        Number(error?.policy?.retry_after_seconds || 0) || undefined,
...
export async function uploadMessageAttachment(req, res) {
  ...
  if (!allowed) {
    ...
    return res
      .status(403)
      .json({ error: "Only connected friends can upload in this thread" });
  }
...
export async function inbox(req, res) {
  ...
  return res.json(
    await tieredInbox(
      [...new Set([...matchIds, ...friendMatchIds])],
      req.user.id,
    ),
  );
}
...
export async function updatePolicyConfig(req, res) {
  ...
  if (!["admin", "owner"].includes(role))
    return res
      .status(403)
      .json({ error: "Only admins can update communication policy config" });
...
export async function listPolicyReviewQueue(req, res) {
  ...
  if (!["admin", "owner"].includes(role))
    return res
      .status(403)
      .json({ error: "Only admins can access policy review queue" });
...
export async function markPolicyFalsePositive(req, res) {
  ...
  if (!["admin", "owner"].includes(role))
    return res
      .status(403)
      .json({ error: "Only admins can mark false positives" });
...
export async function listMessagePolicyQueueInspector(req, res) {
  ...
  if (!["admin", "owner"].includes(role))
    return res
      .status(403)
      .json({ error: "Only admins can access policy queue inspector" });
...
export async function updateSenderReputation(req, res) {
  ...
  if (!["admin", "owner"].includes(role))
    return res
      .status(403)
      .json({ error: "Only admins can adjust sender reputation" });
...
export async function weeklyPolicyDecisionQualityReport(req, res) {
  ...
  if (!["admin", "owner"].includes(role))
    return res
      .status(403)
      .json({ error: "Only admins can access policy quality reports" });
