...
export async function register(req, res) {
  ...
  return res
    .status(201)
    .json({ user: { ...sanitizeUser(user), entitlements }, token });
}
...
export async function passkeyLoginVerify(req, res) {
  try {
    ...
    return res
      .status(err.status || 400)
      .json({ error: err.message || "Passkey login failed" });
  }
}
...
export async function passkeyList(req, res) {
  try {
    ...
    return res
      .status(err.status || 400)
      .json({ error: err.message || "Unable to load passkeys" });
  }
}
...
export async function passkeyRemove(req, res) {
  try {
    ...
    return res
      .status(err.status || 400)
      .json({ error: err.message || "Unable to remove passkey" });
  }
}
