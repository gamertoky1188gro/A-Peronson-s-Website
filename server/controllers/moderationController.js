...
export async function updateModerationProduct(req, res) {
  ...
  if (!productId || !nextStatusRaw) return res.status(400).json({ error: 'productId and status are required' })
  ...
  rows[idx] = {
    ...
    content_review_reason: nextStatus === 'rejected' ? (reason || current.content_review_reason || 'Content standards violation.') : '',
  ...
  const notifyMessage = nextStatus === 'approved'
  ...
  if (rows[idx].company_id) {
    await createNotification(rows[idx].company_id, {
      ...
      meta: { review_status: nextStatus, reason: rows[idx].content_review_reason },
    })
  }
