...
export async function postEvent(req, res) {
  ...
  const entityType = sanitizeString(
    String(req.body?.entity_type || req.body?.entityType || ""),
    80,
  );
  const entityId = sanitizeString(
    String(req.body?.entity_id || req.body?.entityId || ""),
    160,
  );
  const clientId = sanitizeString(
    String(req.body?.client_id || req.body?.clientId || ""),
    120,
  );
  const metadata = sanitizeMetadata(req.body?.metadata);
  let country = sanitizeString(
    String(
      req.body?.country || metadata.country || countryFromHeaders(req) || "",
    ),
    60,
  );
  ...
  const allowUnknownTypes =
    String(process.env.EVENTS_ALLOW_UNKNOWN_TYPES || "").toLowerCase() ===
    "true";
  const result = await ingestEvent(
    {
      ...
      context: {
        ...
        orgOwnerId:
          sanitizeString(
            String(req.user?.org_owner_id || req.user?.id || ""),
            120,
          ) || "unknown",
        ...
        sessionId:
          sanitizeString(String(metadata.session_id || ""), 180) || "unknown",
      },
    },
  ...
  if (!result.accepted) {
    ...
    return res
      .status(400)
      .json({ error: "Event validation failed", reason: result.reason });
  }

  return res
    .status(201)
    .json({ ok: true, schema_version: result.schema_version });
}
