...
import {
  listMyProductViews,
  recordView,
} from "../services/productViewService.js";
...
function matchesMoqRange(rawRange, moqValue) {
  ...
  const moq = Number.isFinite(Number(moqValue))
    ? Number(moqValue)
    : parseNumber(moqValue);
  ...
  for (const msgs of messagesByMatch.values()) {
    ...
    const ownersInMatch = new Set(
      sorted
        .map(
          (m) =>
            ownerByMember.get(String(m.sender_id || "")) ||
            String(m.sender_id || ""),
        )
        .filter(Boolean),
    );
    ...
    for (const ownerId of ownersInMatch) {
      ...
      for (const message of sorted) {
        const senderOwner =
          ownerByMember.get(String(message.sender_id || "")) ||
...
function computeAuditScore(company = {}, certification = null) {
  ...
  const rawAudit = String(
    profile?.audit_date || company?.audit_date || "",
  ).trim();
  if (rawAudit) {
...
export async function searchProducts(req, res) {
  ...
  const estimateOnly =
    String(req.query.estimateOnly || "").toLowerCase() === "true";
  ...
  const priceCurrency = String(
    req.query.priceCurrency || req.query.currency || "",
  )
    .trim()
    .toUpperCase();
  ...
  const distanceFilterActive =
    distanceKm !== null && locationLat !== null && locationLng !== null;
  ...
  if (priceRange) {
    ...
    priceRangeBase = [minText, maxText]
  ...
  if (estimateOnly && engine === "opensearch") {
    const resolvedFacets = openSearchResult?.facets
      ? {
          ...
          category:
            openSearchResult.facets.category ||
            openSearchResult.facets.categories ||
            {},
        }
      : {};
    return res.json({
  ...
  const results = all
    .map((p) => {
      ...
      const companyPremium =
        String(company?.subscription_status || "").toLowerCase() === "premium";
      const certification = company
        ? orderCertMap.get(String(company.id))
        : null;
      ...
      const effectiveBoost =
        (Number.isFinite(paidBoostMultiplier) ? paidBoostMultiplier : 1) *
        premiumBoostMultiplier;
      const boostActive = effectiveBoost > 1;
      const priorityScore =
        (companyPremium ? 2 : 0) +
        (company?.verified ? 0.5 : 0) +
        (boostActive ? 1.25 : 0);
      ...
      return {
        ...
        author: company
          ? {
              ...
              main_processes: Array.isArray(profile?.main_processes)
                ? profile.main_processes
                : [],
              years_in_business: profile?.years_in_business || "",
              handles_multiple_factories: Boolean(
                profile?.handles_multiple_factories,
              ),
              team_seats: profile?.team_seats || "",
              export_ports: Array.isArray(profile?.export_ports)
                ? profile.export_ports
                : [],
              location_lat: profile?.location_lat ?? "",
              location_lng: profile?.location_lng ?? "",
              avg_response_hours:
                responseTimeByOwner.get(String(company.id)) ?? null,
              order_certification_status: certification?.status || "",
              permission_matrix:
                profile?.permission_matrix ||
                company?.permission_matrix ||
                null,
        ...
        boost_multiplier: Number.isFinite(effectiveBoost)
          ? Number(effectiveBoost.toFixed(2))
          : 1,
      };
    })
    .filter((p) => {
      ...
      if (wantedIndustry) {
        ...
        if (
          productIndustry !== wantedIndustry &&
          authorIndustry !== wantedIndustry
        )
          return false;
      }
      if (
        wantedOrgType &&
        String(p.author?.role || "").toLowerCase() !== wantedOrgType
      )
        return false;
      if (
        wantedCountry &&
        String(p.author?.country || "").toLowerCase() !== wantedCountry
      )
        return false;
      ...
      if (leadTimeMax !== null) {
        const lead = parseNumber(
          p.lead_time_days || p.author?.lead_time_days || "",
        );
      ...
      if (wantedFabricTypes.length > 0) {
        ...
        const hit = wantedFabricTypes.some((fabric) =>
      ...
      if (wantedCustomization.length > 0) {
        const customization = String(
          p.customization_capabilities || "",
        ).toLowerCase();
        const hit = wantedCustomization.some((entry) =>
          customization.includes(entry),
        );
        if (!hit) return false;
      }
      if (sampleAvailable) {
      ...
      if (
        wantedAuditDate &&
        String(p.author?.audit_date || "").toLowerCase() !== wantedAuditDate
      )
        return false;
      ...
      if (roleSeatsMap && Object.keys(roleSeatsMap).length) {
        const profileRoleSeats =
          p.author?.role_seats || p.author?.roleSeats || {};
        const permMemberSeats =
          p.author?.permission_matrix?.members?.seats || null;
        for (const [roleKey, minSeats] of Object.entries(roleSeatsMap)) {
          ...
          if (profileRoleSeats && profileRoleSeats[roleKey] !== undefined) {
            ...
            permMemberSeats &&
            permMemberSeats[roleKey] !== undefined
          ) {
      ...
      if (handlesMultipleFactoriesFilter !== null) {
        if (
          Boolean(p.author?.handles_multiple_factories) !==
          handlesMultipleFactoriesFilter
        )
          return false;
      }
  ...
  const facets = orderedResults.reduce(
    (acc, row) => {
      ...
      if (documentReady)
        acc.documentReady[documentReady] =
          (acc.documentReady[documentReady] || 0) + 1;
      const language = String(row.author?.language_support || "").trim();
      if (language)
        acc.languageSupport[language] =
          (acc.languageSupport[language] || 0) + 1;
      ...
      const responseBucket = bucketResponseTime(
        Number(row.author?.avg_response_hours),
      );
      acc.response_time[responseBucket] =
        (acc.response_time[responseBucket] || 0) + 1;
      const yearsBucket = bucketYearsInBusiness(row.author?.years_in_business);
      acc.years_in_business[yearsBucket] =
        (acc.years_in_business[yearsBucket] || 0) + 1;
      const seatsBucket = bucketTeamSeats(row.author?.team_seats);
      acc.team_seats[seatsBucket] = (acc.team_seats[seatsBucket] || 0) + 1;
      const handlesKey = row.author?.handles_multiple_factories
        ? "true"
        : "false";
