...
export async function getMyVerification(req, res) {
  ...
  return res.json(
    rec || { user_id: req.user.id, verified: false, missing_required: [] },
  );
}
...
export async function submitMyVerification(req, res) {
  ...
  try {
    ...
    return res
      .status(status)
      .json({ error: error?.message || "Verification data is invalid" });
  }
}
...
export async function adminApprove(req, res) {
  ...
  if (rec.verified) {
    await createNotification(req.params.userId, {
      ...
      message:
        "Your verification has been approved. Your profile is now verified.",
...
export async function renewMyVerification(req, res) {
  ...
  const hasFreeWindow =
    freeUntil &&
    Number.isFinite(freeUntil.getTime()) &&
    freeUntil.getTime() > Date.now();
  const isFirstTime = !existing?.subscription_valid_until;
  const priceUsd = hasFreeWindow
    ? 0
    : isFirstTime
      ? FIRST_MONTH_PRICE_USD
      : RENEWAL_PRICE_USD;
