...
export async function adminEmailExport(req, res) {
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="gartexhub_emails.csv"',
  );
  return res.send(csv);
}
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.on("end", () => {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="${dataset}.pdf"`,
        );
        res.end(pdfBuffer);
        resolve();
      });
      ...
      doc
        .fontSize(18)
        .text(
          `GarTexHub - ${dataset.replace(/_/g, " ").toUpperCase()} REPORT`,
          { align: "center" },
        );
      doc.moveDown(0.5);
      doc
      ...
      if (data && typeof data === "object") {
        ...
        if (rows.length > 0) {
          doc
            .fontSize(11)
            .text(`Total Items: ${rows.length}`, { align: "left" });
          doc.moveDown();
...
export async function adminDataExport(req, res) {
  ...
  if (dataset === "full_system") {
    const { getAdminMasterSummary } =
      await import("../services/adminMasterService.js");
    const { getAdminConfig } =
      await import("../services/adminConfigService.js");
