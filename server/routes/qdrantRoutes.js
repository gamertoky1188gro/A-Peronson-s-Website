...
import { getQdrantStatus, ensureQdrantCollections, reindexAll } from "../services/qdrantService.js";
...
router.get("/status", async (_req, res) => {
  try {
    ...
    return res.json({ qdrant, embeddings_configured: embeddings, reranker_configured: reranker });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});

router.post("/ensure-collections", requireAuth, requireAdminSecurity, async (_req, res) => {
  try {
