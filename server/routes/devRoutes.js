...
const BASE_URL = (process.env.APP_BASE_URL || `http://localhost:${process.env.PORT || 4000}`).replace(/\/+$/, '')
...
router.post('/esign/simulate_callback', async (req, res) => {
  try {
    const contractId = String(req.query.contractId || (req.body && req.body.contractId) || '')
    const role = String((req.query.role || (req.body && req.body.role) || 'buyer')).toLowerCase()
    if (!contractId) return res.status(400).json({ error: 'contractId required' })

    const payload = role === 'factory' ? { factory_signed: true } : { buyer_signed: true }
    ...
    const response = await axios.post(target, payloadString, { headers, timeout: 10000 })
    return res.json({ ok: true, proxiedStatus: response.status, proxiedBody: response.data })
  } catch (err) {
