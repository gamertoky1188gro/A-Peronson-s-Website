...
const examplesPath = path.join(process.cwd(), 'server', 'evals', 'examples.json')
let examples = []
try { examples = JSON.parse(fs.readFileSync(examplesPath, 'utf8')) } catch { examples = [] }

function scoreField(pred, gold) {
  if (gold === null || gold === undefined || gold === '') return null
  if (pred === null || pred === undefined || pred === '') return 0
  if (typeof gold === 'number' || typeof pred === 'number') return Number(pred) === Number(gold) ? 1 : 0
  ...
  return String(pred).toLowerCase().trim() === String(gold).toLowerCase().trim() ? 1 : 0
}
...
async function run() {
  ...
  const fieldNames = ['product_type','category','moq','target_price','timeline','incoterm','certifications']
  const totals = { }
  const hits = { }
  fieldNames.forEach((f) => { totals[f] = 0; hits[f] = 0 })
  ...
  const results = {}
  fieldNames.forEach((f) => {
    results[f] = { examples: totals[f], precision_recall_score: totals[f] ? (hits[f] / totals[f]) : null }
  })
