...
export function isSmtpConfigured() {
  return (
    isConfigured(process.env.SMTP_HOST) && isConfigured(process.env.SMTP_USER)
  );
}
...
async function getEmailConfig() {
  ...
  return {
    ...
    test_recipient: sanitizeString(
      String(emailConfig?.test_recipient || ""),
      160,
    ),
  };
}
...
function normalizeRecipients(to) {
  if (Array.isArray(to))
    return to
      .map((value) => sanitizeString(String(value || ""), 160))
      .filter(Boolean);
