...
function normalizeVideoReview(row) {
  ...
  const restricted = Boolean(
    row.video_restricted || reviewStatus !== "approved",
  );
  return {
    ...row,
...
function normalizeImageUrls(value) {
  ...
  return [
    ...new Set(
      cleaned.filter((url) => Boolean(url && isInternalMediaUrl(url))),
    ),
  ].slice(0, IMAGE_URL_LIMIT);
}
    ...
    .map((entry) => {
      ...
      if (entry && typeof entry === "object") {
        return sanitizeImageUrl(
          entry.url || entry.source_path || entry.file_path || "",
        );
      }
      return "";
    })
...
function assertInternalMediaUrls(urls = [], fieldName = "media") {
  ...
  if (bad.length) {
    const err = new Error(
      `Only internal media URLs are allowed for ${fieldName}.`,
    );
    err.status = 400;
    throw err;
  }
...
function presentProduct(product, documents = [], viewer = {}) {
  ...
  const isOwner =
    viewer?.id && String(viewer.id) === String(normalized.company_id);
  ...
  const findCover = (items) =>
    items.find(
      (entry) => entry.source_path === coverSource || entry.url === coverSource,
    );
  const coverEntry =
  ...
  return normalizeVideoReview({
    ...
    cover_image_url: canSeePending
      ? coverSource || coverSourcePath
      : coverPublicUrl,
...
async function enforceProductLimits({ owner, allProducts, nextVideoUrl = "" }) {
  ...
  if (productLimit > 0 && ownerProducts.length >= productLimit) {
    const err = new Error(
      `Free plan allows up to ${productLimit} products. Upgrade to add more.`,
    );
    err.status = 403;
    throw err;
  }
  ...
  if (nextVideoUrl) {
    const existingVideos = ownerProducts.filter((p) =>
      String(p.video_url || "").trim(),
    ).length;
...
function getVideoModerationResult({
  title = "",
  description = "",
  videoUrl = "",
}) {
...
function buildReviewReason(template = "", matched = []) {
  const base =
    template ||
    "This listing violates our content standards for modest apparel.";
...
async function evaluateClothingModeration({
  title,
  description,
  category,
  material,
...
export async function updateProductById(actor, productId, patch = {}) {
  ...
  const nextTitle =
    patch.title !== undefined
      ? sanitizeString(patch.title, 120)
      : existing.title;
  let nextDescription =
  ...
  const clothingReview = await evaluateClothingModeration({
    ...
    category:
      patch.category !== undefined
        ? sanitizeString(patch.category, 80)
        : existing.category,
    material:
      patch.material !== undefined
        ? sanitizeString(patch.material, 80)
        : existing.material,
