...
import {
  ...
  moderateTextOrRedactWithContext,
...
async function enforceConversationLock(matchId, sender) {
  ...
  if (
    requirement &&
    String(requirement.buyer_id || "") === String(sender.id || "")
  )
    return null;
  ...
  const allowed = existing
    ? [
        ...new Set([
          ...(Array.isArray(existing.allowed_users)
            ? existing.allowed_users
            : []),
          ...(Array.isArray(existing.allowed_agents)
            ? existing.allowed_agents
            : []),
        ]),
      ]
    : [];
  ...
  const err = new Error(
    "Conversation locked by another agent. Request access to proceed.",
  );
  err.status = 403;
...
function applyFriendThreadMeta(message, fallbackFriend, currentUserId) {
  ...
  const direction =
    fallbackFriend.requester_id === currentUserId ? "outgoing" : "incoming";
  return {
    ...message,
    ...
    friend_request_direction:
      fallbackFriend.type === "friend_request" ? direction : "accepted",
  };
}
...
export async function postMessage(
  ...
  if (
    policyResult.action === "soft_block" ||
    policyResult.action === "hard_block"
  ) {
    const err = new Error(
      policyResult.rejection_message ||
        "Message rejected by communication policy",
    );
    err.status = 429;
    err.code =
      policyResult.action === "hard_block"
  ...
  if (policyResult.action === "queue") {
    ...
    entry.policy_priority =
      policyResult?.decision?.queue_priority_label || null;
...
export async function tieredInbox(matchIds, currentUserId) {
  ...
  const requestMap = new Map(
    messageRequests.map((request) => [request.thread_id, request]),
  );
  ...
  const friendConnectionByMatchId = new Map(
    friendConnections.map((row) => [row.match_id, row]),
  );
        ...
        ? enrichMessage(
            {
              ...
              timestamp:
                fallbackFriend.updated_at ||
                fallbackFriend.created_at ||
