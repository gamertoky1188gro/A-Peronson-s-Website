...
function hasSplitEmailTokens(text = "") {
  ...
  const hasTld =
    /\b(com|net|org|io|me|bd|uk|co|in|pk|cn|ru|jp|sg|my|ae|sa|de|fr|it|es|nl|eu|us)\b/.test(
      l,
    );
...
function scanOutsideContact(text) {
  ...
  if (keywordHit) {
    ...
    const identifierSignal =
      /[@.]|\b(id|username|user|handle|contact|call|dm)\b/.test(l) ||
      looksLikePhone(l);
    return {
      kind: "outside_contact",
...
export function assertMessagingAllowed(user) {
  ...
  if (status === "banned") {
    const err = new Error(
      "Your account has been banned due to repeated policy violations.",
    );
    err.status = 403;
...
export async function moderateTextOrRedact({
  actor,
  text,
  entity_type = "",
  entity_id = "",
