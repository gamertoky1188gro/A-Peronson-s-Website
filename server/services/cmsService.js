...
async function listContentFiles() {
  try {
    ...
    return entries.filter(
      (e) =>
        e.isFile() &&
        [".md", ".html"].includes(path.extname(e.name).toLowerCase()),
    );
  } catch {
    return [];
  }
...
export async function getCmsState() {
  ...
  for (const file of files) {
    ...
    const item = {
      ...
      status: "published",
      updated_at:
        current.articles?.find((a) => a.id === file.name)?.updated_at || "",
  ...
  if (action === "cms.article.create") {
    ...
    if (payload?.content) {
      ...
      await fs
        .writeFile(
          path.join(
            CONTENT_DIR,
            `${entryIdFromTitle(payload.title || "article")}-${actionId}.md`,
          ),
    ...
    updated = await updateState((state) => {
      state.seo = {
        ...
        meta_description:
          payload.meta_description ?? state.seo.meta_description,
