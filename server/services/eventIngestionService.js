...
function normalizeContext(context = {}, base = {}) {
  return {
    actorType:
      sanitizeString(
        String(context.actorType || base.actorType || "unknown"),
        60,
      ) || "unknown",
    orgOwnerId:
      sanitizeString(
        String(context.orgOwnerId || base.orgOwnerId || "unknown"),
        120,
      ) || "unknown",
    entityType:
      sanitizeString(
        String(context.entityType || base.entityType || "unknown"),
        80,
      ) || "unknown",
    entityId:
      sanitizeString(
        String(context.entityId || base.entityId || "unknown"),
        160,
      ) || "unknown",
    sourceModule:
      sanitizeString(
        String(
          context.sourceModule || base.sourceModule || DEFAULT_SOURCE_MODULE,
        ),
        80,
      ) || DEFAULT_SOURCE_MODULE,
    sessionId:
      sanitizeString(
        String(context.sessionId || base.sessionId || "server"),
        180,
      ) || "server",
  };
}
...
function validateRequiredMetadata(metadata, requiredKeys = [], eventType = "") {
  ...
  if (missing.length)
    touchMetric(
      qualityState.missing_required_fields,
      `${eventType}:${missing.join(",")}`,
    );
  return missing;
}
...
export async function ingestEvent(payload = {}, options = {}) {
  ...
  const missingMetadata = validateRequiredMetadata(
    metadata,
  ...
  const enrichedMetadata = {
    ...
    schema_version: sanitizeString(
      String(
        payload.schema_version || taxonomy.version || DEFAULT_SCHEMA_VERSION,
      ),
      20,
    ),
    idempotency_key: idempotencyKey,
