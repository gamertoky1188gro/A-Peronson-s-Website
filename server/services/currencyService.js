  ...
  try {
    ...
    return {
      baseCurrency: toCurrency(
        row?.baseCurrency || row?.defaultBaseCurrency,
        DEFAULT_BASE,
      ),
...
async function persistRate(base, quote, rate, source, staleToleranceMinutes) {
  ...
  const expiresAt = new Date(
    fetchedAt.getTime() + staleToleranceMinutes * 60 * 1000,
  );
  const payload = {
    base,
    quote,
...
export async function getRate(base, quote, options = {}) {
  ...
  const staleToleranceMinutes = Math.max(
    1,
    Number(
      options.staleToleranceMinutes ||
        cfg.staleToleranceMinutes ||
        DEFAULT_STALE_TOLERANCE_MINUTES,
    ),
  );
...
export async function normalizeMoney(amount, from, toBase) {
  ...
  if (
    !rateEntry ||
    !Number.isFinite(Number(rateEntry.rate)) ||
    Number(rateEntry.rate) <= 0
  ) {
    return {
      amount: null,
      rate: null,
...
export async function normalizePriceRange({
  min,
  max,
  currency,
  baseCurrency,
