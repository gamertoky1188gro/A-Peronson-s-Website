...
export async function creditWallet({ userId, amountUsd, reason = '', ref = '', metadata = {}, restricted = false }) {
  ...
  const nextRestricted = restricted ? Math.round((currentRestricted + amount) * 100) / 100 : currentRestricted
  const nextBalance = restricted ? currentBalance : Math.round((currentBalance + amount) * 100) / 100
