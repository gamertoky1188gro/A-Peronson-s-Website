...
const DEFAULT_INTERVAL_MS = Number(
  process.env.ESIGN_WEBHOOK_RETRY_INTERVAL_MS || 60_000,
);
const MAX_ATTEMPTS = Number(process.env.ESIGN_WEBHOOK_MAX_ATTEMPTS || 5);
const BACKOFF_BASE_MS = Number(
  process.env.ESIGN_WEBHOOK_RETRY_BACKOFF_MS || 10_000,
);
      ...
      for (const item of list) {
        try {
          ...
          if (attempts >= MAX_ATTEMPTS) {
            logError("esign_webhook_failure_dropped", {
              id: item.id,
          ...
          const lastAttemptAt = Number(
            item.lastAttemptAt || item.firstFailedAt || 0,
          );
          ...
          logInfo("esign_webhook_retry_success", {
            id: item.id,
            contractId: item.contractId,
          });
        } catch (err) {
          ...
          logError("esign_webhook_retry_failed", {
            id: item.id,
            contractId: item.contractId,
