...
function sanitizeJourney(journey = {}) {
  return {
    ...
    current_state: normalizeDealLifecycleState(
    ...
    final_conversion_status:
      sanitizeString(journey.final_conversion_status, 80) || "in_progress",
    interrupted: Boolean(journey.interrupted),
    interrupted_reason: sanitizeString(journey.interrupted_reason, 240),
    rollback_logs: Array.isArray(journey.rollback_logs)
      ? journey.rollback_logs
      : [],
    invalid_transitions: Array.isArray(journey.invalid_transitions)
...
async function writeJourneys(rows = []) {
  return writeLocalJson(
    FILE,
    (Array.isArray(rows) ? rows : []).map(sanitizeJourney),
  );
}

function findJourneyByContext(rows = [], context = {}) {
  const contextMatchId = sanitizeString(
    context.chat_thread_id || context.match_id,
    120,
  );
  ...
  return (
    rows.find((row) => {
      if (contextMatchId && String(row.chat_thread_id || "") === contextMatchId)
        return true;
      if (
        contextContractId &&
        String(row.contract_id || "") === contextContractId
      )
        return true;
      if (
        contextRequirementId &&
        String(row.requirement_id || "") === contextRequirementId
      )
        return true;
      if (contextProductId && String(row.product_id || "") === contextProductId)
        return true;
      return false;
    }) || null
  );
...
function mergeContext(journey, context = {}) {
  return {
    ...journey,
    search_source:
      sanitizeString(context.search_source, 120) || journey.search_source,
    requirement_id:
      sanitizeString(context.requirement_id, 120) || journey.requirement_id,
    product_id: sanitizeString(context.product_id, 120) || journey.product_id,
    chat_thread_id:
      sanitizeString(context.chat_thread_id || context.match_id, 120) ||
      journey.chat_thread_id,
    contract_id:
      sanitizeString(context.contract_id, 120) || journey.contract_id,
    call_ids: normalizeArrayIds([
      ...(journey.call_ids || []),
      ...(Array.isArray(context.call_ids) ? context.call_ids : []),
      context.call_id,
    ]),
  };
}
    ...
    transitions: [
      {
        ...
        from_state: "",
        to_state: normalizeDealLifecycleState(
          context.initial_state || "discovered",
          "discovered",
        ),
        metadata: {
          source: sanitizeString(context.search_source, 120) || "unknown",
        },
        created_at: nowIso(),
      },
    ],
...
export async function recordJourneyEvent(
  ...
  if (safeEventType === "conversion_closed_lost") {
    ...
    journey.interrupted_reason =
      sanitizeString(metadata?.reason, 240) || "Closed as lost";
  }
...
export async function rollbackDealJourney(
  journeyId,
  toState,
  reason,
  actorId = "",
