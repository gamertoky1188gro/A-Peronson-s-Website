...
import {
  getOpenSearchStatus,
  ensureOpenSearchIndices,
  reindexAll as opensearchReindexAll,
  reindexOrg as opensearchReindexOrg,
...
async function detectInstallerSource() {
  ...
  if (dnf.ok && dnf.stdout)
    return { os: "linux", source: "dnf", available: true };
  const yum = await runCommand("command -v yum");
  if (yum.ok && yum.stdout)
    return { os: "linux", source: "yum", available: true };
  const apt = await runCommand("command -v apt");
  if (apt.ok && apt.stdout)
    return { os: "linux", source: "apt", available: true };
...
export async function getIntegrationStatus() {
  ...
  return {
    signature: {
      ...
      configured:
        isConfigured(process.env.SIGNATURE_CLIENT_ID) &&
        isConfigured(process.env.SIGNATURE_CLIENT_SECRET),
    },
    bank_validation: {
    ...
    ids_ips: {
      ...
      configured:
        isConfigured(process.env.IDS_LOG_PATH) ||
        isConfigured(process.env.IDS_API_URL),
    },
    radius: {
      configured:
        isConfigured(process.env.RADIUS_HOST) &&
        isConfigured(process.env.RADIUS_SECRET),
    },
    tacacs: {
      configured:
        isConfigured(process.env.TACACS_HOST) &&
        isConfigured(process.env.TACACS_SECRET),
    },
    rogue_ap: {
    ...
    netflow: {
      configured:
        isConfigured(process.env.NETFLOW_COLLECTOR) ||
        isConfigured(process.env.NETFLOW_API),
    },
    alert_delivery: {
      slack:
        isConfigured(process.env.SLACK_WEBHOOK_URL) ||
        (config.integrations?.webhooks || []).length > 0,
      sms:
        isConfigured(process.env.SMS_PROVIDER) &&
    ...
    backups: {
      s3:
        ...
        isConfigured(process.env.S3_ACCESS_KEY),
      gcs:
        isConfigured(process.env.GCS_BUCKET) &&
        isConfigured(process.env.GCS_ACCESS_KEY),
      spaces:
        isConfigured(process.env.SPACES_BUCKET) &&
        isConfigured(process.env.SPACES_ACCESS_KEY),
    },
    phpmyadmin: {
...
export async function runIntegrationAction(action = "", payload = {}) {
  ...
  if (!safeAction)
    return { ok: false, action: safeAction, error: "action_required" };
  ...
  if (safeAction === "opensearch.reindex_all") {
    const reset =
      data.reset === true || String(data.reset || "").toLowerCase() === "true";
