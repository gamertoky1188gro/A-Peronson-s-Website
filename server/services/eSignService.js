...
export async function createSignSession(contractId, actor) {
  ...
  const idx = docs.findIndex((d) => d.entity_type === 'contract' && String(d.id) === String(contractId))
  if (idx < 0) {
  ...
  if (process.env.ESIGN_PROVIDER_TYPE && process.env.ESIGN_PROVIDER_URL) {
    try {
      providerSession = await createProviderSignSession({ contractId, contract, actor, token })
  ...
  if (providerSession && providerSession.session_id) contract.artifact.provider_session_id = providerSession.session_id
  if (providerSession && providerSession.provider_id) contract.artifact.provider_request_id = providerSession.provider_id
  if (providerSession && providerSession.meta) contract.artifact.provider_meta = providerSession.meta
...
export async function handleSignCallback(contractId, payload = {}) {
  ...
  if (payload.buyer_signed) {
    ...
    contract.buyer_signed_at = contract.buyer_signed_at || new Date().toISOString()
  }
  if (payload.factory_signed) {
    contract.factory_signature_state = 'signed'
    contract.factory_signed_at = contract.factory_signed_at || new Date().toISOString()
  }
  ...
  try {
    if (contract.buyer_signature_state === 'signed' && contract.factory_signature_state === 'signed') {
