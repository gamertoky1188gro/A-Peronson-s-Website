...
const FEED_BOOST_CONFIG = {
  ...
  minProfileCompleteness: Number(
    process.env.FEED_BOOST_MIN_PROFILE_COMPLETENESS || 0.5,
  ),
  minActivityQuality: Number(
    process.env.FEED_BOOST_MIN_ACTIVITY_QUALITY || 0.4,
  ),
  minimumAccountAgeHours: Number(
    process.env.FEED_BOOST_MIN_ACCOUNT_AGE_HOURS || 1,
  ),
  abuseRapidWindowMinutes: Number(
    process.env.FEED_ABUSE_RAPID_WINDOW_MINUTES || 120,
  ),
  ...
  spamDuplicateRatioLimit: Number(
    process.env.FEED_SPAM_DUPLICATE_RATIO_LIMIT || 0.5,
  ),
  spamMinWordVarietyRatio: Number(
    process.env.FEED_SPAM_MIN_WORD_VARIETY || 0.35,
  ),
};
...
function computeActivityQuality(authorItemIds = [], socialRows = []) {
  ...
  const positive = relevant.filter(
    (row) =>
      row.interaction_type === "comment" || row.interaction_type === "share",
  ).length;
  const reports = relevant.filter(
    (row) => row.interaction_type === "report",
  ).length;
...
function getAgeBoostMultiplier(accountAgeDays) {
  ...
  if (accountAgeDays <= windowDays + decayDays) {
    const decayProgress =
      (accountAgeDays - windowDays) / Math.max(1, decayDays);
...
function evaluateSpamPattern(item = {}, authorItems = []) {
  ...
  const duplicateCount = normalizedItems.filter(
    (entry) => entry === text,
  ).length;
  const duplicateRatio = normalizedItems.length
    ? duplicateCount / normalizedItems.length
    : 0;
...
function evaluateRepeatedPosting(item = {}, authorItems = []) {
  ...
  return {
    postsInRapidWindow,
    suspiciousRepeatedPosting:
      postsInRapidWindow > FEED_BOOST_CONFIG.abuseRapidMaxPosts,
  };
}
...
function evaluateAntiAbuseSignals(item = {}, authorItems = []) {
  ...
  return {
    ...
    ...spamPattern,
    antiAbusePassed:
      !repeatedPosting.suspiciousRepeatedPosting && !spamPattern.lowQualitySpam,
  };
}
...
function calculateRecencyScore(itemCreatedAt) {
  const hoursOld = Math.max(
    0,
    (Date.now() - new Date(itemCreatedAt).getTime()) / (1000 * 60 * 60),
  );
...
function diversifyFeedItems(
  ...
  const dominantCategory =
    [...freq.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || "unknown";
  ...
  const explorationEvery = Math.max(
    3,
    Math.round(1 / Math.max(0.05, explorationRate)),
  );
  const output = [];
  let lastAuthorId = "";
  let lastCategory = "";
  ...
  function canPick(item) {
    ...
    const nextAuthorRun =
      authorId && authorId === lastAuthorId ? authorRun + 1 : 1;
    const nextCategoryRun =
      category && category === lastCategory ? categoryRun + 1 : 1;
  ...
  while (dominantPool.length || explorePool.length) {
    ...
    const shouldExplore =
      ...
      step % explorationEvery === explorationEvery - 1;
...
export async function getCombinedFeed({
  ...
  const requests =
    type === "products" || type === "posts"
      ? []
      : await listRequirements({ status: "open" });
  const products =
  ...
  const ranked = combined.map((item) => {
    ...
    const trustedSeller =
      verifiedContact || (Number.isFinite(avgRating) && avgRating >= 4.3);
    ...
    const discussionBoost =
      viewerVerified && discussionActive ? DISCUSSION_BOOST_MULTIPLIER : 1;
    ...
    const ageBoostMultiplier = antiAbuseEligible
      ? getAgeBoostMultiplier(accountAgeDays)
      : 1;
    const guardedAgeBoost =
      !trustedSeller && ageBoostMultiplier > 1.2 ? 1.2 : ageBoostMultiplier;
    const paidBoostMultiplier =
      paidBoostByUser.get(String(authorId || "")) || 1;
    ...
    return {
      ...
      feed_metadata: {
        ...
        ranking_components: {
          ...
          suspicious_repeated_posting:
            antiAbuseSignals.suspiciousRepeatedPosting,
          ...
          duplicate_content_ratio: roundNumber(
            clamp01(antiAbuseSignals.duplicateRatio),
          ),
          word_variety_ratio: roundNumber(
            clamp01(antiAbuseSignals.wordVarietyRatio),
          ),
        },
      },
    };
  ...
  const boostActiveCount = sortedItems.filter(
    (item) => item.feed_metadata?.boost_active,
  ).length;
  const totalItemCount = sortedItems.length;
  const newProfileBoostedCount = sortedItems.filter((item) => {
    const multiplier =
      item.feed_metadata?.ranking_components?.age_boost_multiplier || 1;
  ...
  const nextCursor =
    safeCursor + safeLimit < totalItemCount ? safeCursor + safeLimit : null;
