...
function queueRanking({ sender, reputationScore, spamScore, config, firstResponse }) {
  ...
  const premiumVerifiedPriorityScore = Number(((premiumMultiplier * verifiedMultiplier) * 100).toFixed(2))
  const basePriority = Number(reputationScore || 50) + (firstResponse ? 12 : 0)
  const riskPenalty = Math.round(Number(spamScore || 0) * 70)
  const adjusted = Math.round(basePriority * premiumMultiplier * verifiedMultiplier - riskPenalty)

  if (adjusted >= 90) return { queue_rank: 'urgent', queue_priority_label: 'P1-Urgent', queue_priority_score: adjusted, premium_verified_priority_score: premiumVerifiedPriorityScore }
...
export function evaluatePolicyContract({ sender = null, matchId = '', text = '', messages = [], config = DEFAULT_GLOBAL_CONFIG, reputationScore = 50 }) {
  ...
  const retryAfterSeconds = Math.max(1, Number(normalizedConfig.message_caps.cooldown_seconds || 30))
