...
function getContractById(contracts, id) {
  return (Array.isArray(contracts) ? contracts : []).find((row) => String(row?.id || '') === String(id)) || null
}
...
export async function createPaymentProof(actor, payload = {}) {
  const contractId = sanitizeString(payload.contract_id || payload.contractId || '', 120)
  if (!contractId) {
  ...
  if (type === 'bank_transfer') {
    const missing = requireFields(payload, ['transaction_reference', 'bank_name', 'sender_account_name', 'receiver_account_name', 'transaction_date', 'amount', 'currency'])
    if (missing.length) {
  ...
  if (type === 'lc') {
    const missing = requireFields(payload, ['lc_number', 'issuing_bank', 'advising_bank', 'applicant_name', 'beneficiary_name', 'issue_date', 'expiry_date', 'amount', 'currency'])
    if (missing.length) {
      const err = new Error(`Missing fields: ${missing.join(', ')}`)
      err.status = 400
      throw err
  ...
  const row = {
    ...
    receiver_account_name: sanitizeString(payload.receiver_account_name || '', 120),
  ...
  const counterpartyId = String(actor.id || '') === String(contract.buyer_id || '')
  ...
  await trackEvent({ type: 'payment_proof_created', actor_id: actor.id, entity_id: row.id, metadata: { contract_id: contractId, type } })
  return row
}
...
export async function updatePaymentProof(actor, proofId, payload = {}) {
  ...
  const nextStatus = String(payload.status || '').toLowerCase().trim()
  ...
  await trackEvent({ type: 'payment_proof_status_updated', actor_id: actor.id, entity_id: next.id, metadata: { status: nextStatus } })

  if (['not_received', 'rejected'].includes(nextStatus)) {
    await notifyAdmins(`Payment proof requires review for contract ${contract.contract_number || contract.id}.`, {
      proof_id: next.id,
