...
const BASE_PERMISSION_MATRIX = {
  ...
  [ACTIONS.FILTERS_ADVANCED_ACCESS]: [
    "owner",
    "admin",
    "buying_house",
    "factory",
...
function teamMatches(actor, resourceContext) {
  ...
  const actorSubTeam = String(
    actor?.profile?.sub_team || actor?.sub_team || "",
  );
...
function applyEnterprisePolicies(
  actor,
  action,
  resourceContext = {},
  baseDecision,
  ...
  if (action === ACTIONS.ANALYTICS_VIEW_AGENT && isAgent(actor)) {
    const targetAgentId = String(
      resourceContext?.target_agent_id || actor.id || "",
    );
  ...
  if (
    action === ACTIONS.CONTRACTS_APPROVE &&
    !ORG_MANAGER_ROLES.has(roleOf(actor))
  ) {
    return policyDecision({
      allowed: false,
  ...
  if (action === ACTIONS.MEMBERS_MANAGE) {
    ...
    if (
      seatCap > 0 &&
      requestedSeats > 0 &&
      activeSeats + requestedSeats > seatCap
    ) {
      return policyDecision({
        allowed: false,
...
export async function authorize(actor, action, resourceContext = {}) {
  ...
  const baseDecision = checkBasePermission(
    actor,
    requestedAction,
    resourceContext,
  );
  const decision = applyEnterprisePolicies(
    actor,
    requestedAction,
    resourceContext,
    baseDecision,
...
export function buildCapabilityPayload(actor) {
  ...
  const entries = Object.values(ACTIONS).map((action) => {
    const baseDecision = checkBasePermission(actor, action, {
      org_id: orgId,
      team,
      sub_team: subTeam,
    });
    const decision = applyEnterprisePolicies(
      actor,
      action,
      { org_id: orgId, team, sub_team: subTeam },
      baseDecision,
    );
