...
export async function createDropboxSignSession({
  contractId,
  contract,
  token,
}) {
  ...
  if (!base)
    throw new Error("ESIGN_PROVIDER_URL is not configured for Dropbox Sign");

  const apiKey =
    process.env.ESIGN_DROPBOX_SIGN_API_KEY ||
    process.env.ESIGN_PROVIDER_API_KEY;
  ...
  const returnUrl =
    process.env.ESIGN_RETURN_URL ||
    `${process.env.APP_BASE_URL || ""}/contracts/${contractId}/sign-return`;
  ...
  const body = {
    ...
    signers: [
      ...
      {
        role: "factory",
        name: contract.factory_name || "",
        email: contract.factory_email || "",
      },
    ],
  ...
  const signing_url =
    ...
    data.url ||
    data.signingUrl ||
    (data.embedded && data.embedded.sign_url);
  const session_id =
    data.session_id || data.id || data.request_id || data.signature_request_id;
