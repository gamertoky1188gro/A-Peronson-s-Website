    ...
    (rows = []) => {
      ...
      const incomingEvidence =
        patch.evidence_contract_ids !== undefined
          ...
          : null;
      const issuedBy =
        patch.issued_by !== undefined
          ? normalizeIssuedBy(patch.issued_by)
          : null;
      const note = patch.note !== undefined ? normalizeNote(patch.note) : null;
      const status =
        patch.status !== undefined ? normalizeStatus(patch.status) : null;
      ...
      updated = {
        ...
        issued_at:
          patch.issued_at !== undefined
            ? patch.issued_at
            : current.issued_at || null,
...
export async function listOrderCertifications({ status } = {}) {
  ...
  return list.filter(
    (row) => normalizeStatus(row?.status) === normalizedStatus,
  );
}
...
export async function getOrderCertificationSummary(userId) {
  ...
  return {
    ...
    evidence_contract_ids: normalizeEvidence(
      record?.evidence_contract_ids || [],
    ),
    note: record?.note || "",
...
export async function revokeOrderCertification(
  userId,
  { issuedBy, note } = {},
) {
  return upsertOrderCertification(userId, {
...
export async function recordCertificationEvidenceFromContract(contract) {
  if (
    !contract ||
    String(contract.entity_type || "").toLowerCase() !== "contract"
  )
    return [];
