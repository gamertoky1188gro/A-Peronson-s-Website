...
export async function getLinkPreview(url) {
	...
	try {
		...
		const data = {
			...
			image: Array.isArray(result.images) && result.images.length > 0 ? result.images[0] : null,
			favicon:
				Array.isArray(result.favicons) && result.favicons.length > 0 ? result.favicons[0] : null,
