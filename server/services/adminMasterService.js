  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        features: ["NetFlow/sFlow analyzer", "QoS policies", "Traffic shaping"],
      },
      {
        id: "alerting",
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        features: ["RBAC roles (Admin/Manager/Viewer)", "Audit logs"],
      },
      {
        id: "srv-auto",
        title: "Automation",
        features: ["Auto updates + patches", "Task queues"],
      },
    ],
  },
  {
...
function isPendingVerification(row) {
  const status = String(
    row?.review_status || row?.verification_status || "",
  ).toLowerCase();
...
export async function getAdminMasterSummary(user) {
  ...
  const couponRedemptionsRows = Array.isArray(couponRedemptions)
    ? couponRedemptions
    : [];
  ...
  const knowledgeRows = Array.isArray(assistantKnowledge)
    ? assistantKnowledge
    : [];
  const productRows = Array.isArray(products) ? products : [];
  const historyRows = Array.isArray(subscriptionHistory)
    ? subscriptionHistory
    : [];
  ...
  const premiumUsers = userRows.filter(
    (u) => String(u.subscription_status || "").toLowerCase() === "premium",
  ).length;
  const suspendedUsers = userRows.filter(
    (u) => String(u.status || "").toLowerCase() === "suspended",
  ).length;
  const pendingVerifications = verificationRows.filter(
    isPendingVerification,
  ).length;
  const expiringVerifications = verificationRows.filter((v) =>
    Boolean(v.expiring_soon),
  ).length;
  ...
  const contractDocs = documentRows.filter(
    (d) =>
      String(d.entity_type || "")
        .toLowerCase()
        .includes("contract") || Boolean(d.contract_number),
  );
  const pendingSignatures = contractDocs.filter(
    (d) =>
      String(d.buyer_signature_state || "").toLowerCase() !== "signed" ||
      String(d.factory_signature_state || "").toLowerCase() !== "signed",
  ).length;

  const walletBalance = userRows.reduce(
    (sum, u) => sum + Number(u.wallet_balance_usd || 0),
    0,
  );
  const walletRestricted = userRows.reduce(
    (sum, u) => sum + Number(u.wallet_restricted_usd || 0),
    0,
  );
  const totalCreditsIssued = walletRows.reduce(
    (sum, w) => sum + Number(w.amount_usd || 0),
    0,
  );

  const config = await getAdminConfig();
  const abuseThreshold = Number(
    config?.search_limits?.abusive_search_threshold || 100,
  );
  const searchAbuse = searchUsageRows.filter(
    (row) => Number(row.count || 0) >= abuseThreshold,
  ).length;
  ...
  const trafficStored = await readLocalJson("traffic_analytics.json", {
  ...
  return {
    ...
    security_context: {
      ...
      ip_allowlist: Array.isArray(adminAuth.ip_allowlist)
        ? adminAuth.ip_allowlist
        : [],
      device_allowlist: Array.isArray(adminAuth.device_allowlist)
        ? adminAuth.device_allowlist
        : [],
      step_up_required: Boolean(
        String(process.env.ADMIN_STEPUP_CODE || "").trim(),
      ),
      export_dual_confirm:
        Boolean(String(process.env.ADMIN_EXPORT_CODE_PRIMARY || "").trim()) ||
        Boolean(String(process.env.ADMIN_EXPORT_CODE_SECONDARY || "").trim()),
      exec_enabled: ["true", "1", "yes"].includes(
    ...
    summary: {
      ...
      orgs: {
        total: userRows.filter((u) =>
          ["buyer", "factory", "buying_house"].includes(
            String(u.role || "").toLowerCase(),
          ),
        ).length,
        staff: userRows.filter((u) => Boolean(u.org_owner_id)).length,
        agents: userRows.filter(
          (u) => String(u.role || "").toLowerCase() === "agent",
        ).length,
      },
      verification: {
      ...
      finance: {
        ...
        revenue_estimate_usd:
          premiumUsers * Number(config.pricing?.premium_usd || 0),
        upgrade_events: historyRows.filter((row) => row.action === "upgrade")
          .length,
        downgrade_events: historyRows.filter(
          (row) => row.action === "downgrade",
        ).length,
      },
      wallet: {
      ...
      partners: {
        ...
        active: partnerRows.filter(
          (r) => String(r.status || "").toLowerCase() === "active",
        ).length,
      },
      requests: {
        total: requirementRows.length,
        verified_only: requirementRows.filter((r) => Boolean(r.verified_only))
          .length,
      ...
      calls: {
        ...
        recordings: callRows.filter(
          (c) =>
            Boolean(c.recording_url) ||
            String(c.recording_status || "").toLowerCase() === "available",
        ).length,
      },
      messages: {
        total: messageRows.length,
        violations: violationRows.length,
        strikes: userRows.reduce(
          (sum, u) => sum + Number(u.policy_strikes || 0),
          0,
        ),
      },
      media: {
        videos_pending: productRows.filter(
          (p) =>
            Boolean(p.video_url) &&
            String(p.video_review_status || "").toLowerCase() !== "approved",
        ).length,
        docs_pending: documentRows.filter(
          (d) =>
            String(d.moderation_status || "").toLowerCase() ===
            "pending_review",
        ).length,
      },
      support: {
        tickets: supportTicketRows.length,
        open: supportTicketRows.filter(
      ...
      ai: {
        ...
        chatbot_enabled: userRows.filter((u) => Boolean(u.chatbot_enabled))
          .length,
      },
      system: {
