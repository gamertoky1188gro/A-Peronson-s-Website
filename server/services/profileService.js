...
export async function getProfileOverview(viewerId, profileUserId) {
  ...
  const [connections, requirements, products, partnerRequests] = await Promise.all([
  ...
  const relationship = viewerId === profileUserId ? { following: false, friend_status: 'self' } : connectionSnapshot(connections, viewerId, profileUserId)

  const verificationRecord = await getVerification(profileUserId)
  const verification_summary = getVerificationPublicSummary(user, verificationRecord)
  ...
  if (user.role === 'buying_house') {
    const connected = partnerRequests.filter((r) => r.status === 'connected' && (r.requester_id === profileUserId || r.target_id === profileUserId))
...
export async function getProfileRequestsPage(_viewerId, profileUserId, { cursor = 0, limit = 12 } = {}) {
  const user = await findUserById(profileUserId)
  if (!user) return 'not_found'
  if (user.role !== 'buyer') return 'invalid_role'

  const requirements = await readJson(REQUIREMENTS_FILE)
  const all = requirements.filter((r) => r.buyer_id === profileUserId).sort(sortNewest)
  const pageItems = all.slice(cursor, cursor + limit)
  const nextCursor = cursor + limit < all.length ? cursor + limit : null

  return { items: pageItems, cursor, next_cursor: nextCursor, total: all.length }
}

export async function getProfileProductsPage(_viewerId, profileUserId, { cursor = 0, limit = 12 } = {}) {
  const user = await findUserById(profileUserId)
  if (!user) return 'not_found'
...
export async function getProfilePartnerNetworkSummary(viewerId, profileUserId) {
  ...
  const [requests, users] = await Promise.all([readJson(PARTNER_REQUESTS_FILE), readJson('users.json')])
  const usersById = new Map(users.map((u) => [u.id, u]))

  const connected = requests.filter((r) => r.status === 'connected' && (r.requester_id === profileUserId || r.target_id === profileUserId))
  const factories = connected
    .map((r) => (r.requester_id === profileUserId ? r.target_id : r.requester_id))
