...
function parseMarketplaceMatchId(matchId = '') {
  ...
  return { requirementId: sanitizeString(parts[0], 120), supplierId: sanitizeString(parts[1], 120) }
}
...
function buildThreadTimeline(messages = [], usersById = new Map(), leadByMatch = new Map()) {
  ...
  for (const thread of byMatch.values()) {
    thread.messages.sort((a, b) => String(a.timestamp || '').localeCompare(String(b.timestamp || '')))
  }

  return [...byMatch.values()].sort((a, b) => String(b.last_message_at || '').localeCompare(String(a.last_message_at || '')))
}
...
export async function getCrmProfileSummary(actor, targetId, options = {}) {
  ...
  const messageRows = (Array.isArray(messages) ? messages : []).filter((m) => {
    ...
    .filter((m) => (options?.match_id ? String(m.match_id || '') === String(options.match_id) : true))
    .filter((m) => (range.from || range.to ? withinRange(m.timestamp || m.created_at, range) : true))

  const leadRows = (Array.isArray(leads) ? leads : [])
    .filter((l) => String(l.org_owner_id || '') === orgId)
    .sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')))

  const leadByMatch = new Map(leadRows.map((lead) => [String(lead.match_id || ''), lead]))
  ...
  const callRows = (Array.isArray(calls) ? calls : []).filter((c) => {
    ...
    .filter((c) => (options?.match_id ? String(c.match_id || c?.context?.chat_thread_id || '') === String(options.match_id) : true))
    .filter((c) => (range.from || range.to ? withinRange(c.created_at || c.started_at, range) : true))
  ...
  const contractRows = (Array.isArray(documents) ? documents : [])
    ...
    .filter((d) => (range.from || range.to ? withinRange(d.updated_at || d.created_at, range) : true))
  ...
  const agentOutcomes = (Array.isArray(users) ? users : [])
    .filter((u) => String(u.org_owner_id || '') === orgId && String(u.role || '').toLowerCase() === 'agent')
    .map((agent) => {
      const assigned = leadRows.filter((l) => String(l.assigned_agent_id || '') === String(agent.id))
      const closed = assigned.filter((l) => String(l.status || '') === 'closed').length
      const confirmed = assigned.filter((l) => String(l.status || '') === 'order_confirmed').length
  ...
  const previousOrders = contractItems
    .filter((row) => String(row.lifecycle_status || '').toLowerCase() === 'signed')
...
export async function getCrmRelationshipTimeline(actor, counterpartyId, options = {}) {
  const safeCounterparty = sanitizeString(String(counterpartyId || ''), 120)
  if (!safeCounterparty) return { error: 'Counterparty id required' }

  const actorOrgId = String(actor?.role || '').toLowerCase() === 'agent'
  ...
  const counterparty = allUsers.find((u) => String(u.id) === safeCounterparty) || null
  ...
  const leadRows = (Array.isArray(leads) ? leads : [])
    ...
    .filter((l) => (options?.match_id ? String(l.match_id || '') === String(options.match_id) : true))
    .sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')))

  const matchIds = new Set(leadRows.map((lead) => String(lead.match_id || '')).filter(Boolean))
  ...
  const contractRows = (Array.isArray(documents) ? documents : [])
    ...
    .filter((d) => {
      ...
      return (buyerId === actorOrgId && factoryId === safeCounterparty) || (factoryId === actorOrgId && buyerId === safeCounterparty)
    })
    .filter((d) => (range.from || range.to ? withinRange(d.updated_at || d.created_at, range) : true))
  ...
  const agentOutcomes = allUsers
    .filter((u) => String(u.org_owner_id || '') === actorOrgId && String(u.role || '').toLowerCase() === 'agent')
    .map((agent) => {
      const assigned = leadRows.filter((l) => String(l.assigned_agent_id || '') === String(agent.id))
      const closed = assigned.filter((l) => String(l.status || '') === 'closed').length
      const confirmed = assigned.filter((l) => String(l.status || '') === 'order_confirmed').length
