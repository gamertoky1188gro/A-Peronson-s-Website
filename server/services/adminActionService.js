...
function parseCsvList(value) {
  if (Array.isArray(value))
    return value
      .map((v) => sanitizeString(String(v || ""), 160))
      .filter(Boolean);
  return String(value || "")
    .split(",")
...
async function removeLocalRecord(fileName, id, idKey = "id") {
  await updateLocalJson(
    ...
    (rows = []) =>
      Array.isArray(rows)
        ? rows.filter((row) => String(row?.[idKey]) !== String(id))
        : [],
    [],
  );
  return true;
...
async function broadcastNotification({
  ...
  const filtered = users.filter((u) => {
    if (roleSet.size && !roleSet.has(String(u.role || "").toLowerCase()))
      return false;
    if (
      premiumOnly &&
      String(u.subscription_status || "").toLowerCase() !== "premium"
    )
      return false;
...
export async function performAdminAction(action, payload = {}, actor) {
  ...
  if (name === "org.split") {
    ...
    if (!orgOwnerId || !newOwnerId || memberIds.length === 0) {
      const err = new Error(
        "org_owner_id, new_owner_id and member_ids are required",
      );
      err.status = 400;
      throw err;
    }
  ...
  if (name === "org.buying_house_staff.remove") {
    ...
    await updateLocalJson(
      ...
      (current = {}) => {
        ...
        return {
          ...current,
          buying_house_staff_ids: list.filter(
  ...
  if (name === "agent.create") {
    ...
    const payloadOut = {
      ...
      permission_matrix: parseJson(
        payload.permission_matrix || payload.permission_matrix_json,
      ),
  ...
  if (name === "verification.reject") {
    ...
    const reason = sanitizeString(
      String(payload.reason || "rejected_by_admin"),
      240,
    );
    if (!userId) {
      const err = new Error("user_id is required");
  ...
  if (name === "verification.remind_expiring") {
    const threshold =
      payload.threshold_days !== undefined
        ? Math.max(1, toNumber(payload.threshold_days, 7))
        : 7;
  ...
  if (name === "verification.revoke_expired") {
    ...
    const revokedIds = (Array.isArray(before) ? before : [])
      .filter(
        (rec) =>
          rec.verified &&
          !updated.find((row) => row.user_id === rec.user_id && row.verified),
      )
      .map((rec) => rec.user_id);
    await Promise.all(
      revokedIds.map((userId) =>
        createNotification(userId, {
          ...
          entity_id: userId,
          message:
            "Verification expired because the premium subscription ended.",
  ...
  if (name === "subscription.set_plan") {
    const userId = toId(payload.user_id);
    const plan = sanitizeString(
      String(payload.plan || "free"),
      20,
    ).toLowerCase();
    if (!userId) {
      const err = new Error("user_id is required");
    ...
    const actionVerb =
      name === "partner.force_accept"
        ...
        : name === "partner.force_reject"
          ? "reject"
          : "cancel";
    const updated = await updatePartnerRequestStatus(
      admin,
      requestId,
      actionVerb,
    );
  ...
  if (name === "contract.signatures") {
    ...
    const patch = {
      ...
      is_draft:
        payload.is_draft !== undefined ? toBool(payload.is_draft) : undefined,
    };
  ...
  if (name === "message.takedown") {
    ...
    const reason = sanitizeString(
      String(payload.reason || "admin_takedown"),
      120,
    );
    if (!messageId) {
  ...
  if (name === "message.redact") {
    const messageId = toId(payload.message_id);
    const reason = sanitizeString(
      String(payload.reason || "admin_redact"),
      120,
    );
    if (!messageId) {
      const err = new Error("message_id is required");
  ...
  if (name === "content.flag") {
    ...
    const entry = await appendLocalRecord("content_flags.json", {
      entity_type: sanitizeString(
        String(payload.entity_type || "document"),
        40,
      ),
      entity_id: entityId,
  ...
  if (name === "violation.strike") {
    const userId = toId(payload.user_id);
    const reason = sanitizeString(
      String(payload.reason || "manual_strike"),
      120,
    );
  ...
  if (name === "support.ticket.create") {
    ...
    const subject = sanitizeString(
      String(payload.subject || "Support ticket"),
      160,
    );
    const note = sanitizeString(
      String(payload.note || "Created by admin"),
      1200,
    );
  ...
  if (name === "order.certification.approve") {
    ...
    const evidence = parseCsvList(
      payload.evidence_contract_ids || payload.evidence_ids,
    );
  ...
  if (name === "notification.batch.send") {
    ...
    const template = templates.find(
      (t) => String(t.id) === String(entry.template_id),
    );
    const users = await listUsers();
    ...
    const recipients = users.filter((u) => {
      ...
      if (roles.length && !roles.includes(String(u.role || "").toLowerCase()))
        return false;
      if (
        premiumOnly &&
        String(u.subscription_status || "").toLowerCase() !== "premium"
  ...
  if (name === "system.search_limits") {
    ...
    if (payload.abusive_search_threshold !== undefined)
      patch.abusive_search_threshold = Math.max(
        0,
        toNumber(payload.abusive_search_threshold),
      );
    if (Object.keys(patch).length === 0) {
  ...
  if (name === "email.test_send") {
    ...
    const subject = sanitizeString(
      String(payload.subject || "GarTexHub test email"),
      200,
    );
  ...
  if (name === "integrations.webhook.test") {
    const config = await getAdminConfig();
    const hooks = Array.isArray(config.integrations?.webhooks)
      ...
      for (const hook of hooks) {
        try {
          const res = await fetch(
            hook.url || hook.endpoint || hook.target || "",
            {
              method: "POST",
  ...
  if (name === "featured.add") {
    const entityType = sanitizeString(
      String(payload.entity_type || "product"),
      40,
    );
  ...
  if (name === "featured.remove") {
    ...
    await updateLocalJson(
      ...
      (rows = []) =>
        Array.isArray(rows)
          ? rows.filter((row) => String(row.entity_id) !== entityId)
          : rows,
      [],
    );
