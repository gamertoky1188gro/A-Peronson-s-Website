...
export async function getServerAdminState() {
  ...
  return {
    ...
    php: {
      ...
      available: phpDetected.available.length
        ? phpDetected.available
        : current.php?.available || [],
    },
    databases: databases.length ? databases : current.databases || [],
    db_admin_ui: {
      phpmyadmin_url:
        process.env.PHPMYADMIN_URL || current.db_admin_ui?.phpmyadmin_url || "",
      configured: Boolean(
        process.env.PHPMYADMIN_URL || current.db_admin_ui?.phpmyadmin_url,
      ),
    },
...
export async function performServerAdminAction(action = "", payload = {}) {
  ...
  if (action === "webserver.update_config") {
    ...
    updated = await updateState((state) =>
      appendLog(state, { id: actionId, action, at: now }),
    );
    ...
    updated = await updateState((state) => {
      const requested = String(
        payload.path || `${actionId.slice(0, 6)}.txt`,
      ).trim();
      ...
      state.files = existing
        ? (state.files || []).map((f) =>
            String(f.path) === String(safePath) ? entry : f,
          )
        : [...(state.files || []), entry];
      return appendLog(state, {
        id: actionId,
    ...
    updated = await updateState((state) => {
      ...
      state.backups.providers = (state.backups.providers || []).map(
        (provider) =>
