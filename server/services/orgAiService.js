...
const DEFAULTS = {
  ...
  ai_hallucination_threshold: parseFloat(process.env.AI_HALLUCINATION_THRESHOLD || '0.7'),
}

export async function getOrgAiSettings(orgOwnerId = '') {
  const rows = await readJson('org_ai_settings.json')
  const found = Array.isArray(rows) ? rows.find((r) => String(r.org_owner_id || '') === String(orgOwnerId || '')) : null
