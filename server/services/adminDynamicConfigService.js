...
async function getInventoryWithFallback() {
  try {
    ...
    console.error('[AdminDynamicConfig] getInventoryWithFallback error:', error.message)
    return DEFAULT_ADMIN_PANEL_FALLBACK_INVENTORY
  }
}
...
async function getInventoryByModule(moduleId) {
  try {
    ...
    console.error('[AdminDynamicConfig] getInventoryByModule error:', error.message)
    return null
  }
}
...
async function updateInventoryConfig(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateInventoryConfig error:', error.message)
    throw error
  }
}
...
async function getActionsWithFallback() {
  try {
    ...
    console.error('[AdminDynamicConfig] getActionsWithFallback error:', error.message)
    return null
  }
}
...
async function getActionsAsGroups() {
  try {
    ...
    console.error('[AdminDynamicConfig] getActionsAsGroups error:', error.message)
    return null
  }
}
...
async function updateActionsConfig(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateActionsConfig error:', error.message)
    throw error
  }
}
...
async function getCapabilitiesWithFallback(moduleId) {
  try {
    ...
    console.error('[AdminDynamicConfig] getCapabilitiesWithFallback error:', error.message)
    return null
  }
}
...
async function updateCapabilities(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateCapabilities error:', error.message)
    throw error
  }
}
...
async function getUiConfigWithFallback() {
  try {
    ...
    return {
      ...
      chart_palette: config.chart_palette?.length >= 2 ? config.chart_palette : DEFAULT_PIE_PALETTE,
      empty_states: config.empty_states || DEFAULT_EMPTY_STATE_COPY,
    }
  } catch (error) {
    console.error('[AdminDynamicConfig] getUiConfigWithFallback error:', error.message)
    return {
      section_metrics: DEFAULT_SECTION_METRICS,
      chart_palette: DEFAULT_PIE_PALETTE,
      empty_states: DEFAULT_EMPTY_STATE_COPY,
...
async function getMockDataWithFallback(dataType) {
  try {
    ...
    if (!mockData || mockData.length === 0) {
      ...
      if (dataType === 'ultra_mini_chart_points') return DEFAULT_ULTRA_MINI_CHART_POINTS
      if (dataType === 'ultra_mini_chart_kpis') return DEFAULT_ULTRA_MINI_CHART_KPIS
      return null
    }

    return mockData.map((m) => m.payload)
  } catch (error) {
    console.error('[AdminDynamicConfig] getMockDataWithFallback error:', error.message)
    return null
  }
}
...
async function getRoleConfigWithFallback() {
  try {
    ...
    return {
      ...
      allowed_roles: roles.filter((r) => r.is_admin_role).map((r) => r.role_key),
      roles: roles.map((r) => ({
        role_key: r.role_key,
        label: r.label,
        benefits: r.benefits || [],
    ...
    console.error('[AdminDynamicConfig] getRoleConfigWithFallback error:', error.message)
    return {
      known_roles: KNOWN_ROLES,
      allowed_roles: DEFAULT_ADMIN_PANEL_ALLOWED_ROLES,
    }
...
async function updateRoleConfig(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateRoleConfig error:', error.message)
    throw error
  }
}
...
async function getGovernanceConfigWithFallback() {
  try {
    ...
    console.error('[AdminDynamicConfig] getGovernanceConfigWithFallback error:', error.message)
    return DEFAULT_GOVERNANCE_CONFIG
  }
}
...
async function updateGovernanceConfig(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateGovernanceConfig error:', error.message)
    throw error
  }
}
...
async function getBrandingConfigWithFallback() {
  try {
    ...
    console.error('[AdminDynamicConfig] getBrandingConfigWithFallback error:', error.message)
    return {
      app_name: 'GarTexHub',
      admin_title: 'Admin Matrix',
      command_deck: 'Command Deck',
...
async function updateBrandingConfig(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateBrandingConfig error:', error.message)
    throw error
  }
}
...
async function getSecurityPurposesWithFallback() {
  try {
    ...
    console.error('[AdminDynamicConfig] getSecurityPurposesWithFallback error:', error.message)
    return { admin_security: 'passkey' }
  }
}
...
async function updateSecurityPurposes(data, actorId) {
  try {
    ...
    console.error('[AdminDynamicConfig] updateSecurityPurposes error:', error.message)
    throw error
  }
}

async function getAllConfig() {
  const [inventory, uiConfig, mockData, roleConfig, governanceConfig, branding, security] = await Promise.all([
...
async function getConfigHistory(configType, limit = 20) {
  try {
    ...
    console.error('[AdminDynamicConfig] getConfigHistory error:', error.message)
    return []
  }
}
