...
function normalizeUserRow(row = {}) {
  const next = { ...row };
  if (next.messaging_restricted_until === "")
    next.messaging_restricted_until = null;
...
function normalizeCommunicationPolicyConfigRow(row = {}) {
  const messageCaps =
    row?.message_caps && typeof row.message_caps === "object"
      ? row.message_caps
      : {};
  const priorityMultipliers =
    row?.priority_multipliers && typeof row.priority_multipliers === "object"
      ? row.priority_multipliers
      : {};
  const spamThresholds =
    row?.spam_thresholds && typeof row.spam_thresholds === "object"
      ? row.spam_thresholds
      : {};

  const premiumMultiplier = toFloat(
    row?.premium_multiplier,
    toFloat(priorityMultipliers.premium, 1.2),
  );
  const verifiedMultiplier = toFloat(
    row?.verified_multiplier,
    toFloat(priorityMultipliers.verified, 1.3),
  );
  ...
  return {
    ...
    max_outreach_per_window: toInt(
      row?.max_outreach_per_window ?? messageCaps.outbound_per_window,
      12,
    ),
    outreach_window_minutes: toInt(
      row?.outreach_window_minutes ?? messageCaps.window_minutes,
      15,
    ),
    cooldown_seconds: toInt(
      row?.cooldown_seconds ?? messageCaps.cooldown_seconds,
      30,
    ),
    premium_boost: toInt(
      row?.premium_boost,
    ...
    verified_boost: toInt(
      row?.verified_boost,
      Math.round((verifiedMultiplier - 1) * 100),
    ),
    keyword_risk_threshold_soft: toFloat(
      row?.keyword_risk_threshold_soft ?? spamThresholds.queue,
      0.45,
    ),
    keyword_risk_threshold_hard: toFloat(
      row?.keyword_risk_threshold_hard ?? spamThresholds.hard_block,
      0.75,
    ),
...
async function syncTable({ model, rows, keyFields, uniqueKeyName = null }) {
  ...
  const safeRows = Array.isArray(rows)
    ? rows.map((r) => stripUndefined(r))
    : [];
  const existing = await delegate.findMany();
  const existingByKey = new Map(
    existing.map((row) => [keyForRow(row, keyFields), row]),
  );
  const nextByKey = new Map(
    safeRows.map((row) => [keyForRow(row, keyFields), row]),
  );

  const missingKeys = [...existingByKey.keys()].filter(
    (k) => !nextByKey.has(k),
  );
  ...
  for (const row of safeRows) {
    const hasAllKeys = keyFields.every(
      (field) =>
        row[field] !== undefined &&
        row[field] !== null &&
        String(row[field]) !== "",
...
const FILE_HANDLERS = {
  ...
  "search_usage_counters.json": tableHandler(
    "searchUsageCounter",
    ["user_id", "action"],
    "user_id_action",
  ),
  ...
  "matches.json": tableHandler(
    "match",
    ["requirement_id", "factory_id"],
  ...
  "message_reads.json": tableHandler(
    "messageRead",
    ["match_id", "user_id"],
    "match_id_user_id",
  ),
  ...
  "communication_limits.json": {
    read: async () =>
      normalizeRows(await prisma.communicationPolicyConfig.findMany()),
    write: async (rows) =>
      syncTable({
        model: "communicationPolicyConfig",
        rows: Array.isArray(rows)
          ? rows.map(normalizeCommunicationPolicyConfigRow)
          : rows,
        keyFields: ["id"],
      }),
  },
  "message_queue.json": tableHandler("messageQueue", ["id"]),
  "message_policy_decisions.json": tableHandler("messagePolicyDecision", [
    "id",
  ]),
...
const ratingsHandler = {
  read: async () => {
    const [ratings, milestones, feedbackRequests, feedbackEvents] =
      await Promise.all([
  ...
  write: async (store = {}) => {
    ...
    const feedbackRequests = Array.isArray(store.feedback_requests)
      ? store.feedback_requests
      : [];
    const feedbackEvents = Array.isArray(store.feedback_events)
