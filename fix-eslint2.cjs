...
if (fe !== origFe) {
  ...
  console.log("Fixed FileExplorerSection.jsx");
}

// For all JSX files with framer-motion imports, wrap them in eslint-disable
const { execSync } = require("child_process");
const files = execSync("dir /s /b src\\*.jsx", { encoding: "utf8" })
  .split("\r\n")
  .filter(Boolean);
...
for (const fp of files) {
  try {
    ...
    if (fp.includes("VerificationPanel")) {
      c = c.replace(
        /\/\* eslint-disable no-unused-vars \*\/\s*\/\/ no-unused-vars \*\/  \/\/ Icon import[\s\S]*?\/\* eslint-enable no-unused-vars \*\//g,
        "",
      );
    }
...
console.log("Fixed framer-motion imports in", fixedCount, "files");
