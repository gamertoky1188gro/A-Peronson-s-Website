...
describe("App protected routes and role gates", () => {
  ...
  test("ProtectedRoute redirects unauthorized roles to access-denied", async () => {
    ...
    expect(source).toMatch(
      /!roles\.includes\(user\.role\)[\s\S]*?<Navigate to="\/access-denied"/,
    );
  });
  ...
  test("critical role arrays and gated routes exist", async () => {
    ...
    expect(source).toMatch(
      /path="\/agent"[\s\S]*?roles=\{\['buying_house', 'owner', 'admin', 'agent'\]\}/,
    );
    expect(source).toMatch(
      /path="\/search"[\s\S]*?<ProtectedRoute roles=\{AUTH_ROLES\}>/,
    );
    expect(source).toMatch(
      /path="\/access-denied" element=\{<AccessDenied \/>\}/,
    );
    expect(source).toMatch(
      /path="\/member-management"[\s\S]*?roles=\{MEMBER_MANAGEMENT_ROLES\}/,
    );
    expect(source).toMatch(
