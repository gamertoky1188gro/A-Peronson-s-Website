...
async function renderChat() {
  ...
  jest.unstable_mockModule(
    "../../src/components/chat/AttachmentPreviewModal",
    () => ({
  ...
  jest.unstable_mockModule(
    "../../src/components/chat/FileAttachmentCard",
    () => ({
  ...
  const ChatInterface = (await import("../../src/pages/ChatInterface.jsx"))
    .default;
  render(
    React.createElement(
      MemoryRouter,
...
describe("ChatInterface UI (RTL)", () => {
  test("shows sign-in error when no token", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Please sign in to view your inbox."),
      ).toBeInTheDocument();
    });
  });
});
