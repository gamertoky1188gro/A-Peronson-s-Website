...
describe('Dropbox Sign provider integration', () => {
  test('creates provider session and stores provider IDs on contract', async () => {
    ...
    axios.post = jest.fn().mockResolvedValue({ data: { signing_url: 'https://provider/sign/abc', session_id: 'sess-abc' } })

    const { createDraftContract } = await import('../../server/services/documentService.js')
    const { createSignSession } = await import('../../server/services/eSignService.js')
