describe('buyer request validation', () => {
  ...
  test('createRequirement rejects invalid open requests but allows drafts', async () => {
    ...
    await expect(reqSvc.createRequirement('buyer1', payload)).rejects.toMatchObject({ status: 400 })

    const draft = await reqSvc.createRequirement('buyer1', { ...payload, status: 'draft' })
