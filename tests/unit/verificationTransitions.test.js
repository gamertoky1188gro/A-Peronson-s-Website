...
describe('verification routes and transition lifecycle', () => {
  test('verificationRoutes exposes submit and admin review endpoints', () => {
    ...
    expect(entries).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: '/admin/:userId/approve', methods: expect.arrayContaining(['post']) }),
      expect.objectContaining({ path: '/admin/:userId/reject', methods: expect.arrayContaining(['post']) }),
    ]))
  })
  ...
  test('approve/reject transitions are stable', async () => {
    ...
    await setVerificationSubscription(userId, new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString())
