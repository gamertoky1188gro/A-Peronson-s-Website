...
async function renderSearch({ initialEntry = "/search?q=denim" } = {}) {
	...
	const SearchResults = (await import("../../src/pages/SearchResults.jsx")).default;
...
describe("SearchResults UI (RTL)", () => {
	test("hydrates query from URL params - requires full router setup", async () => {
		await renderSearch({ initialEntry: "/search?q=denim" });
		const input = screen.getByPlaceholderText("Search requests, factories, products...");
		expect(input).toHaveValue("denim");
	});

	test("filters toggle reveals core filter bar - not implemented", async () => {
