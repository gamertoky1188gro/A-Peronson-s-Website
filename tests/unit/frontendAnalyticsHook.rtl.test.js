...
const mockApiRequest = jest.fn(async (path) => {
	...
	if (path === "/analytics/company") {
		return { items: [] };
	}
	if (path === "/analytics/platform/admin") {
		return { total: 1 };
	}
	if (path === "/analytics/premium") {
		return { upsell: true };
	}
	return {};
});
...
async function setupHook({ hookDashboardError = null } = {}) {
	...
	if (!useAnalyticsDashboard) {
		useAnalyticsDashboard = (await import("../../src/hooks/useAnalyticsDashboard.js")).default;
	}
