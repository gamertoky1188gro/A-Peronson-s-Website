import { createPreset, listPresets, getPresetById, updatePreset, deletePreset } from '../../server/services/presetsService.js'
...
describe('presetsService CRUD', () => {
  ...
  test('create, list, get, update and delete preset', async () => {
    ...
    const created = await createPreset(ownerId, { name: 'My Preset', filters: { industry: 'garments' }, shared: false })
    ...
    const updated = await updatePreset(created.id, { name: 'Updated Preset', shared: true }, actor)
