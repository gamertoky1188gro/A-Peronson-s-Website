...
import { creditWallet, debitWallet, redeemCouponForUser } from '../../server/services/walletService.js'
...
describe('wallet routes and wallet/refund service behavior', () => {
  test('walletRoutes exposes me/history/redeem endpoints', () => {
    ...
    expect(entries).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: '/me/history', methods: expect.arrayContaining(['get']) }),
      expect.objectContaining({ path: '/redeem', methods: expect.arrayContaining(['post']) }),
    ]))
  })
  ...
  test('wallet charge and debit update balances', async () => {
    ...
    const credited = await creditWallet({ userId, amountUsd: 10, reason: 'topup' })
    expect(credited.wallet.balance_usd).toBe(10)

    const debited = await debitWallet({ userId, amountUsd: 3, reason: 'charge', allowRestricted: false })
  ...
  test('coupon redemption is idempotent per user/code pair', async () => {
    ...
    await expect(redeemCouponForUser({ userId, code: 'WELCOME5' })).rejects.toMatchObject({ status: 409 })
  })

  test('recordRefund stores refund audit entry', async () => {
    process.env.NODE_ENV = 'test'
    const userId = `refund-${Date.now()}`
    const entry = await recordRefund({ userId, amountUsd: 4.5, reason: 'manual_refund', ref: 'ref-1', actorId: 'admin-1' })
    const rows = await readLocalJson('refund_log.json', [])
    expect(rows.some((row) => row.id === entry.id && row.user_id === userId)).toBe(true)
  })
})
