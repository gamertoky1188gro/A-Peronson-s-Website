...
describe('Prisma schema DB constraints', () => {
  test('enforces identity-model uniqueness and required fields', async () => {
    ...
    expect(schema).toMatch(/model\s+SenderReputation\s+\{[\s\S]*?sender_id\s+String\s+@unique/)
    expect(schema).toMatch(/model\s+Verification\s+\{[\s\S]*?user_id\s+String\s+@id/)
  })
  ...
  test('enforces money-model indexes, uniqueness, and required fields', async () => {
    ...
    expect(schema).toMatch(/model\s+WalletHistory\s+\{[\s\S]*?balance_after_usd\s+Float\?/)
  })
})
