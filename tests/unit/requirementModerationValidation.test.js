...
import { createRequirement, updateRequirement } from '../../server/services/requirementService.js'
...
describe('requirement routes and service validation/moderation', () => {
  test('requirementRoutes exposes create and patch endpoints', () => {
    ...
    expect(entries).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: '/:requirementId', methods: expect.arrayContaining(['patch']) }),
    ]))
  })
  ...
  test('createRequirement enforces validation for non-draft payload', async () => {
    ...
    await expect(createRequirement('buyer-validation', {
  ...
  test('createRequirement moderates outside-contact text', async () => {
    ...
    const created = await createRequirement(buyerId, {
      ...
      incoterm: 'FOB',
      ex_factory_date: new Date(Date.now() + 20 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    expect(String(created.custom_description)).toMatch(/Removed: outside contact/i)
  })
  ...
  test('updateRequirement blocks buyer from changing assignee', async () => {
    ...
    const row = await createRequirement(buyerId, {
      ...
      incoterm: 'FOB',
      ex_factory_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
      payment_terms: 'TT 30/70',
      status: 'open',
    })

    const result = await updateRequirement(row.id, { assigned_agent_id: 'agent-x' }, { id: buyerId, role: 'buyer' })
