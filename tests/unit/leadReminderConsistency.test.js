  ...
  test("leadService reminder creation + reminder sweep stay consistent in SQL mode", async () => {
    ...
    prisma.lead = {
      findFirst: async ({ where } = {}) =>
        leads.find(
          (row) =>
            row.id === where?.id && row.org_owner_id === where?.org_owner_id,
        ) || null,
    ...
    const leadService = await import(
      `../../server/services/leadService.js?nonce=${now}`
    );
    const leadReminderService = await import(
      `../../server/services/leadReminderService.js?nonce=${now}`
    );
