...
import { ensureEntitlement, getEntitlements } from '../../server/services/entitlementService.js'
...
describe('subscription routes and entitlement boundaries', () => {
  test('subscriptionRoutes exposes self-service and admin endpoints', () => {
    ...
    expect(entries).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: '/me', methods: expect.arrayContaining(['post']) }),
      expect.objectContaining({ path: '/admin/:userId', methods: expect.arrayContaining(['post']) }),
    ]))
  })

  test('free plan user fails premium entitlement check', async () => {
    process.env.NODE_ENV = 'test'
    const user = { id: `sub-free-${Date.now()}`, role: 'factory', subscription_status: 'free' }
    await upsertSubscription(user.id, 'free', true)

    await expect(ensureEntitlement(user, 'advanced_analytics')).rejects.toMatchObject({
      status: 403,
  ...
  test('premium plan enables premium features', async () => {
    process.env.NODE_ENV = 'test'
    const user = { id: `sub-premium-${Date.now()}`, role: 'factory', subscription_status: 'free' }
