describe("aiOrchestrationService sendReply integration", () => {
	test("posts approved reply into thread when auto-reply enabled", async () => {
		...
		const _orgAi = await import("../../server/services/orgAiService.js");

		// Ensure org settings allow auto-reply for a test org
		const settings = [
			{
				id: "org-test",
				org_owner_id: "org-test",
				auto_reply_enabled: true,
				auto_reply_rate_limit_per_hour: 10,
			},
		];
		...
		const res = await svc.sendReply({
			draft: "Automated reply test",
			approval: { approved: true, match_id: "test-match-1" },
		});
