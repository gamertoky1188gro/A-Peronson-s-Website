describe("AI prefill → create → match_id persistence (integration)", () => {
  test("extracts, maps, creates requirement and persists match metadata", async () => {
    ...
    await jsonStore.writeJson("users.json", [
      { id: "buyer-e2e", role: "buyer" },
    ]);
    await jsonStore.writeJson("leads.json", [
      { id: "lead1", match_id: "match-e2e-123", org_owner_id: "buyer-e2e" },
    ]);
    ...
    await jsonStore.writeJson("org_ai_settings.json", [
      {
        id: "org-e2e",
        org_owner_id: "buyer-e2e",
        auto_reply_enabled: true,
        auto_reply_rate_limit_per_hour: 100,
      },
    ]);

    // Simulate a user description that would be sent to the AI
    const text =
      "Need 200 blue cotton shirts, moq 200, target price USD 3-4, timeline 30 days, FOB";
    ...
    const payload = {
      ...
      quantity:
        mapped.quantity || mapped.totalQuantity || String(extracted.moq || 200),
    ...
    expect(
      messages.some((m) => String(m.match_id || "") === "match-e2e-123"),
    ).toBe(true);
