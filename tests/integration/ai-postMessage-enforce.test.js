describe("messageService AI auto-reply enforcement", () => {
	test("blocks ai-origin messages when org auto-reply is disabled", async () => {
		...
		await jsonStore.writeJson("org_ai_settings.json", [
			{
				id: "org-s1",
				org_owner_id: "supplier1",
				auto_reply_enabled: false,
				auto_reply_rate_limit_per_hour: 5,
			},
		]);
		...
		let threw = false;
		try {
			await postMessage(matchId, "system:ai", "Automated reply attempt", "text", null, {
				source_label: "ai:auto_reply",
			});
		} catch (err) {
			threw = true;
			expect(err).toBeDefined();
			expect(err.code === "AI_AUTO_REPLY_DISABLED" || err.code === "AI_AUTO_REPLY_RATE_LIMIT").toBe(
				true,
			);
		}
