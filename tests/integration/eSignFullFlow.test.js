...
describe('e-sign full flow (createSignSession -> callback -> PDF)', () => {
  test('creates signing session and generates PDF after both callbacks', async () => {
    ...
    const req1 = { params: { contractId: draft.id }, body: Buffer.from(JSON.stringify({ buyer_signed: true })), headers: {}, query: {} }
    const res1 = {
      _status: 200,
      _json: null,
      status(code) { this._status = code; return this },
      json(obj) { this._json = obj; return this._json },
    }
    ...
    const req2 = { params: { contractId: draft.id }, body: Buffer.from(JSON.stringify({ factory_signed: true })), headers: {}, query: {} }
    const res2 = {
      _status: 200,
      _json: null,
      status(code) { this._status = code; return this },
