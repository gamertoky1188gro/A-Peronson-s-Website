...
try {
  ...
  console.error("Could not read file list:", e.message);
  process.exit(1);
}
...
header.push("");
header.push(
  "Format: file git url | description | linked files | lines | words | chars",
);
header.push("");
...
for (const rel of list) {
  ...
  rows.push(
    `| ${safeUrl} | ${safeDesc} | ${safeLinked} | ${lines} | ${words} | ${chars} |`,
  );
}
