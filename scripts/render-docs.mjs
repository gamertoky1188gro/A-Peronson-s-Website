...
function groupTailwindTokens(tokens) {
  ...
  const isSpacing = (t) =>
    /^(p|px|py|pt|pr|pb|pl|m|mx|my|mt|mr|mb|ml|gap|space)-/.test(t);
  const isLayout = (t) =>
  ...
  const isColor = (t) =>
    /^(bg-|from-|via-|to-|fill-|stroke-|opacity-|backdrop-|text-\[|bg-\[)/.test(
      t,
    );
  const isBorders = (t) =>
    /^(border|ring|outline|shadow|rounded|divide)/.test(t);
...
function matchApiToBackend({ endpoint, method, backendMap }) {
  ...
  const def =
    backendMap.routeDefs.find(
      (d) =>
        d.routeFile === mount.routeFile &&
        d.method === method &&
        d.routePath === remainder,
    ) || null;
...
function buildPageDoc({ page, indexedPage, styleFilesByPath, backendMap }) {
  ...
  for (const call of apiCalls) {
    ...
    backendRows.push({
      ...
      express: mount
        ? `${mount.prefix} -> ${mount.routeFile}:${mount.line}`
        : "-",
      route: def
        ? `${def.method} ${def.routePath} (${def.routeFile}:${def.line})`
        : "-",
...
function findControllerFunction({
  backendFilesByPath,
  controllerFile,
  handler,
}) {
...
async function main() {
  ...
  for (const doc of serverDocs) {
    if (doc.realtimeOnly) {
      ...
      const serverEntry = index.backend.find(
        (f) => f.path === "server/server.js",
      );
    ...
    const mounts = doc.prefixes
      .map((p) => mountsByPrefix.get(p))
      .filter(Boolean);
  ...
  await writeFileEnsured(
    path.join(REPO_ROOT, "docs", "DOCS_CONVENTIONS.md"),
    conventions,
  );
