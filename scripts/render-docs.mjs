...
const PAGE_CONFIG = [
	...
	},
	{
		...
		name: "Signup",
		file: "Signup.md",
		src: "src/pages/auth/Signup.jsx",
		access: "Public",
	},
	{
		route: "/access-denied",
		name: "AccessDenied",
		file: "AccessDenied.md",
		src: "src/pages/AccessDenied.jsx",
		access: "Public",
	},
	...
	},
	{
		route: "/search",
		name: "SearchResults",
		file: "SearchResults.md",
		src: "src/pages/SearchResults.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		route: "/buyer/:id",
		name: "BuyerProfile",
		file: "BuyerProfile.md",
		src: "src/pages/BuyerProfile.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		route: "/factory/:id",
		name: "FactoryProfile",
		file: "FactoryProfile.md",
		src: "src/pages/FactoryProfile.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		route: "/buying-house/:id",
		name: "BuyingHouseProfile",
		file: "BuyingHouseProfile.md",
		src: "src/pages/BuyingHouseProfile.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		route: "/contracts",
		name: "ContractVault",
		file: "ContractVault.md",
		src: "src/pages/ContractVault.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		...
		file: "NotificationsCenter.md",
		src: "src/pages/NotificationsCenter.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		route: "/chat",
		name: "ChatInterface",
		file: "ChatInterface.md",
		src: "src/pages/ChatInterface.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		route: "/call",
		name: "CallInterface",
		file: "CallInterface.md",
		src: "src/pages/CallInterface.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		...
		file: "VerificationPage.md",
		src: "src/pages/VerificationPage.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
	},
	{
		...
		file: "VerificationCenter.md",
		src: "src/pages/VerificationPage.jsx",
		access: "Protected",
		roles: REQUIRED_ROLES,
		aliasOf: "/verification",
	},
];
...
function groupTailwindTokens(tokens) {
	...
	const isLayout = (t) =>
		/^(flex|grid|block|inline|hidden|absolute|relative|fixed|sticky|inset|top|left|right|bottom|z|overflow|w|h|min|max|col|row|items|justify|content|place|self|order|aspect|container|float|clear|truncate|whitespace|break|line-clamp)/.test(
			t,
		);
	const isTypography = (t) =>
		/^(text-|font-|tracking-|leading-|uppercase|lowercase|capitalize|italic|not-italic|antialiased)/.test(
			t,
		);
	const isColor = (t) =>
		/^(bg-|from-|via-|to-|fill-|stroke-|opacity-|backdrop-|text-\[|bg-\[)/.test(t);
	const isBorders = (t) => /^(border|ring|outline|shadow|rounded|divide)/.test(t);
	const isInteraction = (t) =>
		interactionPrefixes.some((p) => t.startsWith(p)) ||
		/^(transition|duration-|ease-|animate-|will-change|transform|scale-|translate-|rotate-)/.test(
			t,
		);
	...
	for (const token of tokens) {
		...
		} else if (isResponsive(token)) {
			groups.responsive.push(token);
		} else if (isInteraction(token)) {
			groups.interaction.push(token);
		} else if (isBorders(token)) {
			groups.bordersRingsShadows.push(token);
		} else if (isColor(token)) {
			groups.color.push(token);
		} else if (isTypography(token)) {
			groups.typography.push(token);
		} else if (isSpacing(token)) {
			groups.spacing.push(token);
		} else if (isLayout(token)) {
			groups.layout.push(token);
		} else {
...
function explainToken(token) {
	...
	if (token.startsWith("text-")) {
		return "Text color or text sizing.";
	}
	if (token.startsWith("rounded")) {
		return "Corner radius.";
	}
	if (token.startsWith("shadow")) {
		return "Drop shadow depth (elevation).";
	}
	if (token.startsWith("ring")) {
		return "Outline ring (often used instead of borders in dark mode).";
	}
	...
	if (token.includes(":")) {
		return "Variant prefix (responsive, dark, or interaction state).";
	}
...
function getAllTokens(classStrings) {
	...
	for (const s of classStrings || []) {
		for (const t of String(s).split(/\s+/g)) {
			tokens.push(t.trim());
		}
	}
...
function buildBackendMap(backendFiles) {
	...
	for (const file of filesByPath.values()) {
		if (!(file.path.startsWith("server/routes/") && file.path.endsWith(".js"))) {
			continue;
		}
		...
		for (let i = 0; i < lines.length; i += 1) {
			...
			if (m) {
				...
				for (const n of names) {
					controllerImports.set(n, `server/controllers/${m[2]}`);
				}
			}
		}
		...
		for (let i = 0; i < lines.length; i += 1) {
			...
			const head = text.match(
				/router\.(get|post|patch|delete)\(\s*['"]([^'"]+)['"]\s*,([\s\S]*)\)\s*;?/,
			);
			if (!head) {
				continue;
			}
			...
			const handler = args.length > 0 ? args.at(-1).trim() : "";
...
function matchApiToBackend({ endpoint, method, backendMap }) {
	...
	const def =
		backendMap.routeDefs.find(
			(d) => d.routeFile === mount.routeFile && d.method === method && d.routePath === remainder,
		) || null;
...
async function readIndex() {
	...
	const frontend = [];
	for (const f of parsed.frontend) {
		frontend.push(await attachLines(f));
	}

	const styles = [];
	for (const f of parsed.styles) {
		styles.push(await attachLines(f));
	}

	const backend = [];
	for (const f of parsed.backend) {
		backend.push(await attachLines(f));
	}
...
function buildPageDoc({ page, indexedPage, styleFilesByPath, backendMap }) {
	...
	const customCssRefs = CUSTOM_CSS_NAMES.map((name) => ({
		name,
		refs: findCssRefs(styleFilesByPath, name),
	...
	md += `- **Backend interactions:** ${apiCalls.length > 0 ? "Yes (see API map below)." : "None (static page / client-only interactions)."}\n`;
	...
	for (const block of classBlocks) {
		...
		if (block.classStrings.length > 0) {
			for (const s of block.classStrings) {
				md += `- \`${s}\`\n`;
			}
			md += "\n";
		} else {
	...
	md +=
		"- **When line numbers drift:** re-run `npm run docs:generate` to refresh " +
		"`path:line`" +
		" references.\n\n";
...
function buildServerDoc({ title, routes, mounts, backendFilesByPath }) {
	...
	for (const r of routes) {
		...
		if (r.controllerFile && r.handler) {
			const fn = findControllerFunction({
				backendFilesByPath,
...
async function main() {
	...
	const serverDocs = [
		...
		{
			title: "Verification",
			prefixes: ["/api/verification"],
			out: "Verification.md",
		},
		{
			title: "Subscription",
			prefixes: ["/api/subscriptions"],
			out: "Subscription.md",
		},
		...
		{
			title: "RealtimeCommunication",
			prefixes: [],
			out: "RealtimeCommunication.md",
			realtimeOnly: true,
		},
	];
	...
	try {
		...
		if (!raw.includes("Line-numbered docs")) {
			await fs.writeFile(pagesReadmePath, raw.trimEnd() + genNote, "utf-8");
		}
	} catch {
		// ignore
	}
	...
	try {
		...
		if (!raw.includes("Line-numbered docs")) {
			await fs.writeFile(serverReadmePath, raw.trimEnd() + genNote, "utf-8");
		}
	} catch {
		// ignore
	}
