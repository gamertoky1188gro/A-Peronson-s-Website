...
async function buildIndex() {
  ...
  let products = []
  try { products = JSON.parse(await fs.readFile(productsPath, 'utf8')) } catch { products = [] }
  ...
  const serial = Object.fromEntries(Object.entries(index).map(([k, v]) => [k, [...v]]))
...
if (process.argv[1].endsWith('indexer.mjs')) {
  buildIndex().catch((e) => { console.error(e); process.exit(2) })
}
