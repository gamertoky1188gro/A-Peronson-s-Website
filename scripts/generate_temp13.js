...
function isBinary(_filename) {
	// simple check: treat common text extensions as text
	return false;
}
...
function inferDescription(filePath, content) {
	...
	if (name === "package.json") {
		return "NPM manifest (dependencies, scripts)";
	}
	...
	if (ext === ".json") {
		return "JSON data or config file";
	}
	if (ext === ".html") {
	...
	if (ext === ".js" || ext === ".mjs" || ext === ".cjs") {
		if (/react|jsx|component/i.test(content)) {
			return "JavaScript (React) component or module";
		}
		return "JavaScript source file or module";
	}
	if (ext === ".ts" || ext === ".tsx") {
		return "TypeScript source or React TypeScript component";
	}
	if (ext === ".py") {
		return "Python script/module";
	}
	if (ext === ".sh" || ext === ".ps1") {
		return "Shell/PowerShell script";
	}
	if (ext === "") {
		if (path.basename(filePath) === "Dockerfile") {
			return "Dockerfile (container build)";
		}
		if (path.basename(filePath) === "Makefile") {
			return "Makefile (build rules)";
		}
	}
	// fallback
...
function findLinkedFiles(filePath, content) {
	...
	// also pick up simple "./file.ext" references
...
lines.push("");
lines.push(
	"Format: file git url or git repo file location(https://github.com/gamertoky1188gro/A-Peronson-s-Website)| why its made/or description|linked with which files",
);
lines.push("");
