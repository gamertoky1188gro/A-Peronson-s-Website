...
function extractClassBlocks(lines) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    for (const exLine of excerptLines) {
      const q = [...exLine.matchAll(/className\s*=\s*["']([^"']+)["']/g)].map(
        (m) => m[1],
      );
...
async function indexBackend() {
  ...
  backendFiles.push(
    ...(await listFilesUnder("server/services")).filter((p) =>
      p.endsWith(".js"),
    ),
  );
  ...
  backendFiles.push(
    ...(await listFilesUnder("server/database")).filter((p) =>
      p.endsWith(".json"),
    ),
  );
  ...
  for (const rel of unique) {
    ...
    indexed.push({
      ...
      exports: lines
        ...
        .map(({ l, idx }) => ({
          line: idx + 1,
          code: snippet(lines, idx + 1, 3),
        })),
      apiMounts:
        rel === "server/server.js"
          ? lines
              .map((l, idx) => ({ l, idx }))
              .filter(
                ({ l }) =>
                  l.includes("app.use('/api/") || l.includes("app.get("),
              )
              .map(({ l, idx }) => ({
                line: idx + 1,
                code: snippet(lines, idx + 1, 2),
              }))
          : [],
      routeDefs: rel.startsWith("server/routes/")
        ? lines
            ...
            .filter(
              ({ l }) =>
                l.includes("router.get") ||
                l.includes("router.post") ||
                l.includes("router.patch") ||
                l.includes("router.delete"),
            )
            .map(({ l, idx }) => ({
              line: idx + 1,
              code: snippet(lines, idx + 1, 2),
...
async function main() {
  ...
  process.stdout.write(
    `Wrote ${toPosix(path.relative(REPO_ROOT, path.join(GENERATED_DIR, "index.json")))}\n`,
  );
}
